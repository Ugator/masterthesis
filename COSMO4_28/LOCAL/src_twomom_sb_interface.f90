...
!
!                                          |
!   new namelist parameters in seifert_pp:
!  
!   lincloud=.true./.false. -- allow/do not allow incloud nucleation
!   z_ncn_segal=2000.0m -- constant N_cn below that height, exp. decrease above
!   z1oe_ncn_segal=2000.0m -- 1/e-scale-height of exp. decrease of N_cn above z_ncn_segal
!
!
! Preprocessor flags necessary for the Seifert/Beheng 2-moment scheme (set in Makefile / Fopts):
!
! MPI_CLOUD: Einkommentieren oder beim Compileraufruf definieren,
! wenn load balancing gewuenscht.
!
! SEDI_VECTORIZED: Use vectorizable versions of the sedimenation routines.
!                  You can find the respective code in src_seifert.f90.
!
! CGP_SEARCH_VEC: Use vectorizable version of cloudy grid point search.
...
! Modules used:
 ...
 ! -------------------------------------------
  ...
  ! 2MOM modules from src_seifert.f90:
...
! UB_20100421>> von integer nach real(kind=ireals) geaendert:
REAL(KIND=ireals), PARAMETER :: &
  ttstep = 2.0_ireals,       &  ! increment for temperature in look-up table
  ssstep = 1.0_ireals           ! increment for ice supersaturation in look-up table

REAL(KIND=ireals), DIMENSION(0:100,0:100), SAVE :: &
...
REAL (KIND=ireals) ELEMENTAL FUNCTION diff_vapor(temp,pres)
IMPLICIT NONE

REAL (KIND=ireals), INTENT(IN)  :: temp, pres

REAL (KIND=ireals), PARAMETER   :: a = 8.7602e-5_ireals
REAL (KIND=ireals), PARAMETER   :: b = 1.81_ireals
...
REAL (KIND=ireals)              :: rain_mue_Dm
REAL (KIND=ireals), INTENT(IN)  :: Dm,qc
REAL    (KIND=ireals   ), PARAMETER ::  &
  zrmu0  = 2.00e+00_ireals, & ! Seifert (2008) mue-Dmv-relation 
  zrmu1  = 2.00e+01_ireals, & ! for the raindrop size distribution
  zrmu2  = 1.00e+03_ireals, & ! 
  zrmu3  = 1.10e-03_ireals, & ! 
  zrmu4  = 0.00e+00_ireals
...
!==============================================================================
!
! Functions for setting initial and boundary conditions for QNX in case
...
! Set QNC in a way that it leads to an average mass corresponding to a diameter of 10 microns:
! Input: QC in kg/m^3; Output: QNC in 1/m^3
...
FUNCTION set_qnc_from_qc_sb(qcin)
  ...
  REAL(kind=ireals) :: set_qnc_from_qc_sb(ie, je, ke)
  REAL(kind=ireals), INTENT(in) :: qcin(ie, je, ke)

  ! Diameter of mean particle mass:
  REAL(kind=ireals), PARAMETER :: Dmean = 10e-6_ireals
!  REAL(kind=ireals), PARAMETER :: Dmean = 30e-6_ireals

  REAL(kind=ireals), PARAMETER :: rhowater = 1000.0_ireals
  REAL(kind=ireals), PARAMETER :: pi = 3.14159265358979323844_ireals

  set_qnc_from_qc_sb = &
       qcin * 6.0_ireals / (pi * rhowater * Dmean**3.0_ireals)

END FUNCTION set_qnc_from_qc_sb

! For Seifert/Beheng scheme:
! Set QNI in a way that it leads to an average mass corresponding to a diameter of 100 microns:
! Input: QI in kg/m^3; Output: QNI in 1/m^3
FUNCTION set_qni_from_qi_sb(qiin)

  IMPLICIT NONE

  REAL(kind=ireals) :: set_qni_from_qi_sb(ie, je, ke)
  REAL(kind=ireals), INTENT(in) :: qiin(ie, je, ke)

  ! Diameter of mean particle mass:
  REAL(kind=ireals), PARAMETER :: Dmean = 100e-6_ireals
  ! Mass-size-relation parameter of "iceHK"
  REAL(kind=ireals), PARAMETER :: ageo  = 0.217_ireals
  REAL(kind=ireals), PARAMETER :: bgeo  = 0.302115_ireals

  set_qni_from_qi_sb = &
       qiin / 1e-10
     !  qiin / ( ( Dmean / ageo) ** (1.0_ireals / bgeo) )
...
FUNCTION set_qni_from_qi_sb_scalar(qiin)
  ...
  REAL(kind=ireals) :: set_qni_from_qi_sb_scalar
  REAL(kind=ireals), INTENT(in) :: qiin
...
! For Seifert/Beheng scheme:
! Set QNR in a way that it is consistent to the assumptions in itype_gscp == 4:
! Input: QR in kg/m^3; Output: QNR in 1/m^3
FUNCTION set_qnr_from_qr_sb(qrin)

  IMPLICIT NONE

  REAL(kind=ireals) :: set_qnr_from_qr_sb(ie, je, ke)
  REAL(kind=ireals), INTENT(in) :: qrin(ie, je, ke)

  ! Diameter of mean particle mass:
  REAL(kind=ireals), PARAMETER :: N0r = 8000.0e3_ireals
  REAL(kind=ireals), PARAMETER :: rhowater = 1000.0_ireals
  REAL(kind=ireals), PARAMETER :: pi = 3.14159265358979323844_ireals

  set_qnr_from_qr_sb = &
       N0r * ( qrin * 6.0_ireals / (pi * rhowater * N0r * gamma_fct(4.0_ireals)))**(0.25_ireals)

END FUNCTION set_qnr_from_qr_sb

! For Seifert/Beheng scheme:
! Set QNS in a way that it is consistent to the assumptions in itype_gscp == 4:
! Input: QS in kg/m^3; Output: QNS in 1/m^3
FUNCTION set_qns_from_qs_sb(qsin)

  IMPLICIT NONE

  REAL(kind=ireals) :: set_qns_from_qs_sb(ie, je, ke)
  REAL(kind=ireals), INTENT(in) :: qsin(ie, je, ke)

  ! Diameter of mean particle mass:
  REAL(kind=ireals), PARAMETER :: Dmean = 100e-6_ireals
  REAL(kind=ireals), PARAMETER :: N0s = 800.0e3_ireals
  REAL(kind=ireals), PARAMETER :: ams = 0.038_ireals
  REAL(kind=ireals), PARAMETER :: bms = 2.0_ireals

  set_qns_from_qs_sb = &
       N0s * ( qsin / ( ams * N0s * gamma_fct(bms+1.0_ireals)))**( 1.0_ireals/(1.0_ireals+bms) )
!  set_qns_from_qs_sb = &
!       qsin / ( ( Dmean / ams) ** (1.0_ireals / bms) )

END FUNCTION set_qns_from_qs_sb

! For Seifert/Beheng scheme:
! Set QNG in a way that it is consistent to the assumptions in itype_gscp == 4:
! Input: QG in kg/m^3; Output: QNG in 1/m^3
FUNCTION set_qng_from_qg_sb(qgin)

  IMPLICIT NONE

  REAL(kind=ireals) :: set_qng_from_qg_sb(ie, je, ke)
  REAL(kind=ireals), INTENT(in) :: qgin(ie, je, ke)

  ! Diameter of mean particle mass:
  REAL(kind=ireals), PARAMETER :: Dmean = 1000e-6_ireals
  REAL(kind=ireals), PARAMETER :: N0g = 4000.0e3_ireals
  REAL(kind=ireals), PARAMETER :: amg = 169.6_ireals
  REAL(kind=ireals), PARAMETER :: bmg = 3.1_ireals

  set_qng_from_qg_sb = &
       N0g * ( qgin / ( amg * N0g * gamma_fct(bmg+1.0_ireals)))**( 1.0_ireals/(1.0_ireals+bmg) )

!  set_qng_from_qg_sb = &
!       qgin / ( ( Dmean / amg) ** (1.0_ireals / bmg) )
...
!==============================================================================
!                                              
! Two-moment mixed-phase bulk microphysics for LM 
!       
...
! Description:
!
! The subroutine is the interface between LM and the original KAMM2 modules, 
! which are provided by src_seifert.f90. A major difference between LM and 
...
!
! details can be found in src_seifert.F90 and in the following references
!
...
!!!!! dann auch die ssw, ssi ueberall nachberechnen.
  ...
  REAL(KIND=ireals), PARAMETER :: qnc_const = 1e8_ireals    ! Assumed constant QNC (1/m^3) in case of nuc_c_typ = 0 
  REAL(KIND=ireals), PARAMETER :: lewfaktor = 1.0_ireals
  LOGICAL, PARAMETER :: lgshed_ascm = .false.
  LOGICAL, PARAMETER :: lhshed_ascm = .false.
  REAL(KIND=ireals), PARAMETER :: tshift_clfr_sbcm = 0.0_ireals
  ...
  REAL(KIND=ireals), PARAMETER :: z_ncn_segal = 2000.0
  REAL(KIND=ireals), PARAMETER :: z1oe_ncn_segal = 2000.0
  ...
  INTEGER(KIND=iintegers) :: i,j,k,ii,jj,kk,n,nt_2mom,nt_sedi,igridpoints,izstat,kkk
  INTEGER, SAVE  :: firstcall, firstcall_init
  REAL(KIND=ireals) :: wmax,qvmax,qcmax,qrmax,qimax,qsmax,qgmax,qhmax,&
    &                  precmax,rhomax,rhomin,ncmax,nimax,dzmin,tmax,tmin
  REAL(KIND=ireals) :: q_vap_new,q_vap_old
  REAL(KIND=ireals) :: q_liq_new,q_liq_old
  REAL(KIND=ireals) :: q_ice_new,q_ice_old
  REAL(KIND=ireals) :: q_v, rho_v, x_v, hlp, dt_sedi,e_v,T_a,qerr, mrho

  INTEGER, DIMENSION(0:ie*je*ke)           :: ilm,jlm,klm
  REAL(KIND=ireals), DIMENSION(ie,je,ke) :: rain_r,rain_g,rain_s,rain_i,rain_h
  REAL(KIND=ireals), DIMENSION(ie,je,ke) :: adz,rhocorr,ssi,ssw,zml

  REAL(KIND=ireals),    PARAMETER :: eps  = 1e-15
...
! Voher war das an den Output der Max./Min.-Werte der QX gekoppelt.
  LOGICAL, PARAMETER :: debug_maxrain = .FALSE.
...
! field for inst. microphys. conv. rates     (kg/kg*s)
  ...
  REAL(KIND=ireals) :: dum1, dum2, dum3, dum4, dum5, dum6, dum7

  REAL(KIND=ireals), PARAMETER :: wcb_min=0.1, scb_min=0.0e0, scb_max=-0.0e0
...
!----------------
  REAL (KIND=ireals), POINTER :: &
    ...
    qg  (:,:,:) => NULL(),         & ! QG at nx
    qh  (:,:,:) => NULL(),         & ! QH at nx
    qnc (:,:,:) => NULL(),         & ! QNCLOUD at nx
    qnr (:,:,:) => NULL(),         & ! QNRAIN at nx
    qni (:,:,:) => NULL(),         & ! QNICE at nx
    qns (:,:,:) => NULL(),         & ! QNSNOW at nx
    qng (:,:,:) => NULL(),         & ! QNGRAUPEL at nx
    qnh (:,:,:) => NULL()            ! QNHAIL at nx
...
!------------------------------------------------------------------------------
  ...
  IF (izerror /= 0) THEN
    ...
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNCLOUD', ptr_tlev = nx, ptr = qnc)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNRAIN', ptr_tlev = nx, ptr = qnr)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNICE', ptr_tlev = nx, ptr = qni)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNSNOW', ptr_tlev = nx, ptr = qns)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNGRAUPEL', ptr_tlev = nx, ptr = qng)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNHAIL', ptr_tlev = nx, ptr = qnh)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
...
! UB_20090227<<
  ...
  ! Limit QX and QNX after the advection routines at all gridpoints:
  ...
  ! For example, in situations with high QNC at gridpoints with very low QC, the advection routine
  ! led to ever increasing values of QNC above 10^34 with nothing to stop it, which is the maximum value which fits into
  ! the grib data routines of DWD. This inevitably leads to a model crash for no apparent physical reason.
  !
  ! Clip values of QX < 0 and then clip QNX values to their allowed range:

  qv(:,:,:)  = MAX(qv(:,:,:),  0.0_ireals)
  qc(:,:,:)  = MAX(qc(:,:,:),  0.0_ireals)
  qnc(:,:,:) = MAX(qnc(:,:,:), 0.0_ireals)
  qr(:,:,:)  = MAX(qr(:,:,:),  0.0_ireals)
  qnr(:,:,:) = MAX(qnr(:,:,:), 0.0_ireals)
  IF (itype_gscp >= 1000) THEN
    qi(:,:,:)  = MAX(qi(:,:,:),  0.0_ireals)
    qni(:,:,:) = MAX(qni(:,:,:), 0.0_ireals)
    qs(:,:,:)  = MAX(qs(:,:,:),  0.0_ireals)
    qns(:,:,:) = MAX(qns(:,:,:), 0.0_ireals)
    qg(:,:,:)  = MAX(qg(:,:,:),  0.0_ireals)
    qng(:,:,:) = MAX(qng(:,:,:), 0.0_ireals)
  END IF
  IF (itype_gscp >= 2000) THEN
    qh(:,:,:)  = MAX(qh(:,:,:),  0.0_ireals)
    qnh(:,:,:) = MAX(qnh(:,:,:), 0.0_ireals)
  END IF
...
!CDIR COLLAPSE
  ...
  CALL calrho ( t(:,:,:,nx), pp(:,:,:,nx), &
       qv(:,:,:), qc(:,:,:),&
       qrs(:,:,:), p0(:,:,:), &
       rho(:,:,:), ie, je, ke, r_d, r_v/r_d-1.0_ireals)
...
cv_d     =  cp_d - r_d
cv_v     =  (rcpv + 1.0_ireals) * cp_d - r_v
cv_l     =  (rcpl + 1.0_ireals) * cp_d
cv_i     =  2060.0_ireals
...
!!$    zttfac  = 1.0_ireals / cv_d  ! isochoric
!!$    zpptfac = 1.0_ireals / cv_d  ! isochoric
!!$    zlhTfac = 1.0_ireals         ! T-dependent lh_v, lh_s and lh_f and the Qm-term
!!$  ELSE
...
!!$    !       This is the original formulation of the COSMO equation system:
    zttfac  = cpdr
    zpptfac = 0.0_ireals  ! zero out the pp-tendency
    zlhTfac = 0.0_ireals  ! zero out the T-dependence of lh_v, lh_s and lh_f and the Qm-term


!!$  END IF

  IF (debug_maxvalloc) THEN
    wmax  = global_maxval(w(its:ite,jts:jte,kts:kte,nx))
    qvmax = global_maxval(qv(its:ite,jts:jte,kts:kte))
    qcmax = global_maxval(qc(its:ite,jts:jte,kts:kte))
    qrmax = global_maxval(qr(its:ite,jts:jte,kts:kte))
    qimax = global_maxval(qi(its:ite,jts:jte,kts:kte))
    qsmax = global_maxval(qs(its:ite,jts:jte,kts:kte))
    qgmax = global_maxval(qg(its:ite,jts:jte,kts:kte))  
    qhmax = global_maxval(qh(its:ite,jts:jte,kts:kte))  
    ncmax = global_maxval(qnc(its:ite,jts:jte,kts:kte))  
    tmax = global_maxval(t(its:ite,jts:jte,kts:kte,nx))  
    IF (isIO()) THEN
      WRITE (*,*) TRIM(yzroutine)//": output 1"
      WRITE(*,'(A10,10A11)')   '   ', 'w','qv','qc','qr','qi','qs','qg','qh','nc','t'
      WRITE(*,'(A10,10E11.3)') '   ', wmax,qvmax,qcmax,qrmax,qimax,qsmax,qgmax,qhmax,ncmax,tmax
    ENDIF
    
    wmax  = global_minval(w(its:ite,jts:jte,kts:kte,nx))
    qvmax = global_minval(qv(its:ite,jts:jte,kts:kte))
    qcmax = global_minval(qc(its:ite,jts:jte,kts:kte))
    qrmax = global_minval(qr(its:ite,jts:jte,kts:kte))
    qimax = global_minval(qi(its:ite,jts:jte,kts:kte))
    qsmax = global_minval(qs(its:ite,jts:jte,kts:kte))
    qgmax = global_minval(qg(its:ite,jts:jte,kts:kte))  
    qhmax = global_minval(qh(its:ite,jts:jte,kts:kte))  
    ncmax = global_minval(qnc(its:ite,jts:jte,kts:kte))  
    nimax = global_minval(qni(its:ite,jts:jte,kts:kte))
    tmax = global_minval(t(its:ite,jts:jte,kts:kte,nx))
    IF (isIO()) THEN
      WRITE(*,'(10x,a)') 'Minimum Values:'
      WRITE(*,'(A10,10A11)')   '   ', 'w','qv','qc','qr','qi','qs','qg','qh','ni','t'
...
!MB is loadbalancing possible in case of aerosol interaction??
          ...
          IF ( lincloud .AND. (w(ii,jj,kk,nx) > wcb_min .AND. ssw(ii,jj,kk) >= scb_min &
              & .AND.  ssw(ii,jj,kk) > ssw(ii,jj,MIN(kk+1,kte))))  THEN
            sendbuf(j+19) = w(ii,jj,kk,nx)
          ELSE IF (.NOT.lincloud .AND. (w(ii,jj,kk,nx) > wcb_min .AND. ssw(ii,jj,kk) >= scb_min .AND. &
               ssw(ii,jj,MIN(kk+1,kte)) < scb_max)) THEN
            sendbuf(j+19) = w(ii,jj,kk,nx)
          ELSE
            sendbuf(j+19) = 0.0
          ENDIF
...
!MB Need to implement a cloud diagnosis e.g. cloudact=1 new cloud; cloudact=2 cloud base; cloudact=3 incloud 
        ...
        IF ( lincloud .AND. (w(ii,jj,kk,nx) > wcb_min .AND. ssw(ii,jj,kk) >= scb_min &
          ...
          w_cb(i,j,k) = w(ii,jj,kk,nx)

#ifdef COSMOART
          IF(ssw(ii,jj,MIN(kk+1,kte)) < scb_max)    THEN   ! cloud base
              cloudact(i,j,k)=2 
          ELSE IF(ssw(ii,jj,kk) > ssw(ii,jj,MIN(kk+1,kte))) THEN  ! incloud activation
              cloudact(i,j,k)=3
          ENDIF
#endif
...
#ifdef COSMOART
!IF  n_cloud 'to low' do activation as if new
...
!            endif
    ...
    ! Finally, clip values of QX < 0 and then clip QNX values to their allowed range:
...
!!$     should be 0.0:
    ...
    WHERE (qc < eps) qnc = 0.0_ireals
    WHERE (qr < eps) qnr = 0.0_ireals
    IF (itype_gscp >= 1000) THEN
      WHERE (qi < eps) qni = 0.0_ireals
      WHERE (qs < eps) qns = 0.0_ireals
      WHERE (qg < eps) qng = 0.0_ireals
    END IF
    IF (itype_gscp >= 2000) THEN
      WHERE (qh < eps) qnh = 0.0_ireals
    END IF


    rain_r = MAX(rain_r, 0.0_ireals)
    rain_i = MAX(rain_i, 0.0_ireals)
    rain_s = MAX(rain_s, 0.0_ireals)
    rain_g = MAX(rain_g, 0.0_ireals)
    rain_h = MAX(rain_h, 0.0_ireals)
...
!!$    IF (debug.and.isIO()) WRITE (*,*) TRIM(yzroutine)//": deallocation"
  ...
  IF (debug_maxrainloc) THEN
    qvmax = global_maxval_2d(prr_gsp+prs_gsp+prg_gsp+prh_gsp)
    qrmax = global_maxval_2d(rain_r(:,:,ke))
    qimax = global_maxval_2d(rain_i(:,:,ke))
    qsmax = global_maxval_2d(rain_s(:,:,ke))
    qgmax = global_maxval_2d(rain_g(:,:,ke))
    qhmax = global_maxval_2d(rain_h(:,:,ke))
    IF (isIO()) THEN
      WRITE (*,*) TRIM(yzroutine)//": rainrate = ", qvmax*3600
      WRITE (*,*) "            rain_r   = ", qrmax*3600
      WRITE (*,*) "            rain_g   = ", qgmax*3600
      WRITE (*,*) "            rain_h   = ", qhmax*3600
      WRITE (*,*) "            rain_s   = ", qsmax*3600
      WRITE (*,*) "            rain_i   = ", qimax*3600
    ENDIF
  ENDIF
  ...
  IF (debug_maxvalloc) THEN
    ...
    qhmax = global_maxval(qh(its:ite,jts:jte,kts:kte))  
    ncmax = global_maxval(qnc(its:ite,jts:jte,kts:kte))  
    nimax = global_maxval(qni(its:ite,jts:jte,kts:kte))  
    tmax = global_maxval(t(its:ite,jts:jte,kts:kte,nx))  
    IF (isIO()) THEN
...
CONTAINS
  ...
  SUBROUTINE saturation_adjust_h2o_qv(i, j, k, dq37, dq38, dq39, dq40, tempaenderung)
    ...
    REAL(kind=ireals) :: a1, b1, dq_d, q_sat, T_a, p_a, hlp, rlrd, rlrdm1, e_sat
    REAL(kind=ireals), INTENT(OUT)   :: dq37, dq38, dq39, dq40, tempaenderung
    REAL(kind=ireals) :: t_, e_ws_
...
!      qnc(i,j,k) = MIN(qnc(i,j,k), qc(i,j,k)/cloud%x_min)
  ...
  SUBROUTINE saturation_adjust_h2o_q(T_a, p_a, q_cloud_a, n_cloud_a, q_a, &
    ...
    REAL(kind=ireals), INTENT(in)    :: rho_a, lewfakt, p_a
    REAL(kind=ireals), INTENT(inout) :: T_a, q_cloud_a, n_cloud_a, q_a
    INTEGER(kind=iintegers)          :: ice_typ
    REAL(kind=ireals)                :: a1, b1, dq_d, q_sat, hlp, rlrd, rlrdm1, e_sat
    REAL(kind=ireals), INTENT(OUT)   :: dq37, dq38, dq39, dq40, tempaenderung
    REAL(kind=ireals) :: t_, e_ws_
...
REAL(KIND=ireals), DIMENSION(3,imax), INTENT(out) :: frac
REAL(KIND=ireals), DIMENSION(imax),   INTENT(in)  :: temp,ssi

REAL(KIND=ireals) :: xt,xs,tc2, s_i, s_w, e_v, e_vw, e_vi, q_v
...
REAL(KIND=ireals), PARAMETER :: pref=500e2_ireals
...
! UB_20100421<<
      ...
      afrac_orga(tt,ss) = fractions(3)
    ELSE
      afrac_dust(tt,ss) = 0.0
      afrac_soot(tt,ss) = 0.0
      afrac_orga(tt,ss) = 0.0
    END IF
