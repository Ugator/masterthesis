...
#else
! COSMO
USE data_tracer,        ONLY: idt_qv, idt_qc, idt_qi, idt_qr, idt_qs, idt_qg
! MESSy/BMIL
USE messy_main_tracer_mem_bi, ONLY: GPTRSTR, xtte
USE messy_main_tracer_bi,     ONLY: tracer_halt
! MESSy/SMCL
USE messy_main_tracer,  ONLY: new_tracer, set_tracer                 &
                            , AMOUNTFRACTION, AIR, ON, OFF           &
                            , I_GRIBTAB, I_GRIBPARAM                 &
                            , I_ADVECT,  I_CONVECT, I_VDIFF, I_HDIFF &
                            , I_INITIAL, I_RELAX,   I_DAMP,  I_LBC   &
                            , T_ADV_2_LF,     T_ADV_3_LF                &
                            , T_INI_ZERO,     T_INI_FILE                &
                            , T_LBC_ZEROGRAD, T_LBC_ZERO,   T_LBC_FILE  &
                            , T_RELAX_INFLOW, T_RELAX_FULL, T_RELAX_OFF &
                            , T_DAMP_ON => ON, T_DAMP_OFF => OFF
...
ELSEIF (yaction == 'tracer') THEN
  ...
  ! Mein eigener Entropy-Tracer Flo
    ...
    yunits         = 'kg kg-1',                                               &
    ystandard_name = 'entropy_temperature',                                   &
    ylong_name     = 'entropy temperature after hauf, hoeller 87')!Dont do anything
  ...
  ENDIF
#else
  CALL new_tracer(izerror, GPTRSTR, 'QV', 'COSMO', &
  ...
  CALL set_tracer(izerror, GPTRSTR, idt_qv, I_GRIBTAB, 1)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qv, I_GRIBPARAM, 51)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qv, I_ADVECT,  ON)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qv, I_VDIFF,   ON)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qv, I_HDIFF,   ON)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qv, I_CONVECT, OFF)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qv, I_INITIAL, T_INI_FILE)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qv, I_LBC,     T_LBC_FILE)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qv, I_RELAX,   T_RELAX_FULL)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qv, I_DAMP,    izdamp_qv)
  CALL tracer_halt(yzroutine, izerror)
#endif
  ...
  ENDIF
#else
  CALL new_tracer(izerror, GPTRSTR, 'QC', 'COSMO', &
       longname = 'mass_fraction_of_cloud_liquid_water_in_air', &
       quantity = AMOUNTFRACTION, &
       unit     = 'kg/kg', &
       medium   = AIR, &
       idx      = idt_qc)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qc, I_GRIBTAB, 2)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qc, I_GRIBPARAM, 31)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qc, I_ADVECT,  ON)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qc, I_VDIFF,   ON) 
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qc, I_HDIFF,   ON) 
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qc, I_CONVECT, OFF)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qc, I_INITIAL, T_INI_FILE)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qc, I_LBC,     T_LBC_FILE)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qc, I_RELAX,   T_RELAX_FULL)
  CALL tracer_halt(yzroutine, izerror)
  CALL set_tracer(izerror, GPTRSTR, idt_qc, I_DAMP,    izdamp_qc)
  CALL tracer_halt(yzroutine, izerror)
#endif
  ...
  IF (lprog_qi) THEN
    ...
    ENDIF
#else
     CALL new_tracer(izerror, GPTRSTR, 'QI', 'COSMO', &
          longname = 'mass_fraction_of_cloud_ice_in_air', &
          quantity = AMOUNTFRACTION, &
          unit     = 'kg/kg', &
          medium   = AIR, &
          idx      = idt_qi)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qi, I_GRIBTAB, 2)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qi, I_GRIBPARAM, 33)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qi, I_ADVECT,  T_ADV_2_LF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qi, I_VDIFF,   ON)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qi, I_HDIFF,   OFF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qi, I_CONVECT, OFF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qi, I_INITIAL, izic_qi)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qi, I_LBC,     izlbc_qi)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qi, I_RELAX,   T_RELAX_FULL)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qi, I_DAMP,    izdamp_qi)
     CALL tracer_halt(yzroutine, izerror)
#endif
  ENDIF
  ...
  IF (itype_gscp >= 4) THEN
    ...
    ENDIF
#else
     CALL new_tracer(izerror, GPTRSTR, 'QG', 'COSMO', &
          longname = 'mass_fraction_of_graupel_in_air', &
          quantity = AMOUNTFRACTION, &
          unit     = 'kg/kg', &
          medium   = AIR, &
          idx      = idt_qg)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qg, I_GRIBTAB, 2)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qg, I_GRIBPARAM, 39)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qg, I_ADVECT,  T_ADV_3_LF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qg, I_VDIFF,   OFF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qg, I_HDIFF,   OFF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qg, I_CONVECT, OFF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qg, I_INITIAL, izic_qg)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qg, I_LBC,     izlbc_qg)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qg, I_RELAX,   izrelax_qg)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qg, I_DAMP,    izdamp_qg)
     CALL tracer_halt(yzroutine, izerror)
#endif
  ENDIF
  ...
  ENDIF
#else
     CALL new_tracer(izerror, GPTRSTR, 'QR', 'COSMO', &
          longname = 'mass_fraction_of_rain_in_air', &
          quantity = AMOUNTFRACTION, &
          unit     = 'kg/kg', &
          medium   = AIR, &
          idx      = idt_qr)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qr, I_GRIBTAB, 2)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qr, I_GRIBPARAM, 35)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qr, I_ADVECT,  T_ADV_3_LF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qr, I_VDIFF,   OFF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qr, I_HDIFF,   OFF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qr, I_CONVECT, OFF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qr, I_INITIAL, izic_qr)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qr, I_LBC,     izlbc_qr)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qr, I_RELAX,   izrelax_qr)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qr, I_DAMP,    izdamp_qr)
     CALL tracer_halt(yzroutine, izerror)
#endif
  ...
  IF (itype_gscp > 1_iintegers) THEN
    ...
    ENDIF
#else
     CALL new_tracer(izerror, GPTRSTR, 'QS', 'COSMO', &
          longname = 'mass_fraction_of_snow_in_air', &
          quantity = AMOUNTFRACTION, &
          unit     = 'kg/kg', &
          medium   = AIR, &
          idx      = idt_qs)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qs, I_GRIBTAB, 2)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qs, I_GRIBPARAM, 36)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qs, I_ADVECT,  T_ADV_3_LF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qs, I_VDIFF,   OFF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qs, I_HDIFF,   OFF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qs, I_CONVECT, OFF)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qs, I_INITIAL, izic_qs)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qs, I_LBC,     izlbc_qs)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qs, I_RELAX,   izrelax_qs)
     CALL tracer_halt(yzroutine, izerror)
     CALL set_tracer(izerror, GPTRSTR, idt_qs, I_DAMP,    izdamp_qs)
     CALL tracer_halt(yzroutine, izerror)
#endif
  ENDIF
  ...
  IF ( .NOT. l2tls ) THEN
    ...
    IF (izerr /= 0_iintegers) THEN
      ...
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QI', 'CLP_10E-12', .TRUE.)
  ...
  IF ( l2tls ) THEN
    ...
    IF (izerr /= 0_iintegers) THEN
      ...
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QR', 'ADD_CLP_ADV', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QS', 'ADD_CLP_ADV', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QG', 'ADD_CLP_ADV', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
#ifdef TWOMOM_SB
    CALL trcr_meta_set(izerr, 'QH', 'ADD_CLP_ADV', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNCLOUD', 'ADD_CLP_ADV', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNRAIN', 'ADD_CLP_ADV', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNICE', 'ADD_CLP_ADV', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNSNOW', 'ADD_CLP_ADV', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNGRAUPEL', 'ADD_CLP_ADV', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNHAIL', 'ADD_CLP_ADV', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
  ...
  IF ( l2tls ) THEN
    ...
    IF (izerr /= 0_iintegers) THEN
      ...
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QI', 'BD_0GRAD_FORCED', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QR', 'BD_0GRAD_FORCED', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QS', 'BD_0GRAD_FORCED', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QG', 'BD_0GRAD_FORCED', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
#ifdef TWOMOM_SB
    CALL trcr_meta_set(izerr, 'QH', 'BD_0GRAD_FORCED', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNCLOUD', 'BD_0GRAD_FORCED', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNRAIN', 'BD_0GRAD_FORCED', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNICE', 'BD_0GRAD_FORCED', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNSNOW', 'BD_0GRAD_FORCED', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNGRAUPEL', 'BD_0GRAD_FORCED', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNHAIL', 'BD_0GRAD_FORCED', .TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
  ...
  IF ( .NOT. l2tls ) THEN
    ...
    IF (izerr /= 0_iintegers) THEN
      ...
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QI', 'SP_ADV_LF', 2_iintegers)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QR', 'SP_ADV_LF', 3_iintegers)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QS', 'SP_ADV_LF', 3_iintegers)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QG', 'SP_ADV_LF', 3_iintegers)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
#ifdef TWOMOM_SB
    CALL trcr_meta_set(izerr, 'QH', 'SP_ADV_LF', 3_iintegers)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNCLOUD', 'SP_ADV_LF', 2_iintegers)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNRAIN', 'SP_ADV_LF', 3_iintegers)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNICE', 'SP_ADV_LF', 3_iintegers)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNSNOW', 'SP_ADV_LF', 3_iintegers)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNGRAUPEL', 'SP_ADV_LF', 3_iintegers)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNHAIL', 'SP_ADV_LF', 3_iintegers)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
  ...
  IF (itype_lbc_qrsg == 1 .OR. itype_lbc_qrsg == 2) THEN
    CALL trcr_meta_set(izerr, 'QR', 'BD_SET_FORCED', itype_lbc_qrsg)   
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QS', 'BD_SET_FORCED', itype_lbc_qrsg)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QG', 'BD_SET_FORCED', itype_lbc_qrsg)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
#ifdef TWOMOM_SB
    CALL trcr_meta_set(izerr, 'QH', 'BD_SET_FORCED', itype_lbc_qrsg)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNRAIN', 'BD_SET_FORCED', itype_lbc_qrsg)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNSNOW', 'BD_SET_FORCED', itype_lbc_qrsg)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNGRAUPEL', 'BD_SET_FORCED', itype_lbc_qrsg)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ENDIF
    CALL trcr_meta_set(izerr, 'QNHAIL', 'BD_SET_FORCED', itype_lbc_qrsg)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
  ...
  IF (izerr /= 0_iintegers) THEN
    ...
    RETURN
  ENDIF
  CALL trcr_meta_set(izerr, 'QV', 'MASSFLX_CLP', .TRUE.)
  IF (izerr /= 0_iintegers) THEN
    ierror  = izerr
    yerrmsg = trcr_errorstr(izerr)
    RETURN
  ENDIF
  CALL trcr_meta_set(izerr, 'QC', 'MASSFLX_CLP', .TRUE.)
  IF (izerr /= 0_iintegers) THEN
    ierror  = izerr
    yerrmsg = trcr_errorstr(izerr)
    RETURN
  ...
  IF ( lspubc .AND. itype_spubc==2 .AND. .NOT. llb_qg                         &
    ...
    CALL trcr_meta_set(izerr, 'QG', 'DAMP_FORCED',.TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
...
#ifdef TWOMOM_SB
    IF ( itype_gscp >= 100 ) THEN
      CALL trcr_meta_set(izerr, 'QH', 'DAMP_FORCED',.TRUE.)
      IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
        ierror  = izerr
        yerrmsg = trcr_errorstr(izerr)
        RETURN
      ...
      IF (izdebug > 1_iintegers) THEN
        ...
        PRINT *,  ''
      ENDIF
      CALL trcr_meta_set(izerr, 'QNGRAUPEL', 'DAMP_FORCED',.TRUE.)
      IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
        ierror  = izerr
        yerrmsg = trcr_errorstr(izerr)
        RETURN
  ...
  IF ( lspubc .AND. itype_spubc==2 .AND. .NOT. llb_qr_qs                      &
      .AND. itype_lbc_qrsg == 1 ) THEN
    CALL trcr_meta_set(izerr, 'QR', 'DAMP_FORCED',.TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
    ...
    IF (izdebug > 1_iintegers) THEN
      ...
      PRINT *,  ''
    ENDIF
    CALL trcr_meta_set(izerr, 'QS', 'DAMP_FORCED',.TRUE.)
    IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
      ierror  = izerr
      yerrmsg = trcr_errorstr(izerr)
      RETURN
...
#ifdef TWOMOM_SB
    IF ( itype_gscp >= 100 ) THEN
      CALL trcr_meta_set(izerr, 'QNRAIN', 'DAMP_FORCED',.TRUE.)
      IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
        ierror  = izerr
        yerrmsg = trcr_errorstr(izerr)
        RETURN
      ...
      IF (izdebug > 1_iintegers) THEN
        ...
        PRINT *,  ''
      ENDIF
      CALL trcr_meta_set(izerr, 'QNSNOW', 'DAMP_FORCED',.TRUE.)
      IF (izerr /= 0_iintegers .AND. izerr /= T_ERR_NOTFOUND) THEN
        ierror  = izerr
        yerrmsg = trcr_errorstr(izerr)
        RETURN
...
!no more:        CALL organize_conv_kainfri
  ...
  CALL trcr_get(ierror, 'QV', ptr_tens=qv_tens)
  IF (izerror /= 0) THEN
    yerrmsg = trcr_errorstr(ierror)
    CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(ierror, 'QC', ptr_tens=qc_tens)
  IF (ierror /= 0) THEN
    yerrmsg = trcr_errorstr(ierror)
    CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(ierror, 'QI', ptr_tens=qi_tens)
  ...
  qv_tens => xtte(:,:,idt_qv,:)
  qc_tens => xtte(:,:,idt_qc,:)
  IF (idt_qi > 0) qi_tens => xtte(:,:,idt_qi,:)
#endif
