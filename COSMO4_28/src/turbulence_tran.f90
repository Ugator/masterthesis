...
USE data_runcontrol, ONLY:   &
    lseaice, llake, icldm_tran, lprfcor, itype_synd, imode_tran, itype_wcld, &
    itype_diag_t2m

USE data_turbulence, ONLY:   &
    zt_ice, z0_ice, len_min, tet_g, rim, akt, l_scal,         &
    alpha0, rat_sea, rlam_mom, rlam_heat, rat_can, rat_lam,   &
    z0m_dia, epsi, it_end, vel_min, tkesmot,                  &
    a_3, a_5, a_6,           b_1, b_2,        d_4,            &
    c_g, d_m=>d_mom,                                          &
    clc_diag, q_crit

USE meteo_utilities,       ONLY:   cloud_diag
USE turbulence_utilities,  ONLY:   stab_funct, turb_param, turb_cloud, diag_level
...
!------------------------------------------------------------------------------
  ...
  REAL (KIND=ireals),       INTENT(INOUT) :: &
    ...
    tkvm(ie,je,2:ke1),   & ! turbulent diffusion coefficients for momentum (m/s2 )
    tkvh(ie,je,2:ke1),   & ! turbulent diffusion coefficients for heat     (m/s2 )
                           ! and moisture
...
!------------------------------------------------------------------------------
  ...
  REAL (KIND=ireals),       INTENT(INOUT), OPTIONAL :: &
...
!     sonstiges:
      ...
      REAL (KIND=ireals) :: &
        alfa,beta,gama,delt,& ! Platzhalter fuer beliebige
        ...
        DO i=i_st,i_en
          ...
          IF (fr_land(i,j).LT.z1d2) THEN
            ...
            ELSE
              ...
              lo_ice(i,j) = h_ice(i,j) > 0.0_ireals
            END IF
            IF (llake) THEN
              ! Lake model is used.
              ! Ice surface if this is a lake point AND ice is present.
              IF ((depth_lk(i,j) > 0.0_ireals) .AND. (h_ice(i,j) >= h_Ice_min_flk)) &
              lo_ice(i,j) = .TRUE.
            END IF
          END IF
...
!           Berechnung der zugehoerigen Taupunktstemperatur:
            ...
            rh_2m(i,j)=100.0_ireals*MIN(wert/zpsat_w(t_2m(i,j)),z1)
