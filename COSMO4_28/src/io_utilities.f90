...
USE vgrid_refatm_utils, ONLY :   vcoord_type, vcoord_in, uuid_match,         &
...
!------------------------------------------------------------------------------
    ...
    IF ((ylevtyp == 'hybrid') .OR. (ylevtyp == 'hybridLayer')) THEN
      ...
      ENDIF
    ELSEIF ( ((ylevtyp == 'generalVertical') .OR. (ylevtyp == 'generalVerticalLayer')) ) THEN
      IF (iznlev /= vcoord_in%nlevels) ierror = 3
      IF (iznrvg /= vcoord_in%ivctype) ierror = 3
      IF (.NOT. uuid_match(vcoord_in%vc_uuid, yzuuid)) ierror = 3
      CALL uuid_2char(yzuuid,              yzuuid_string)
      CALL uuid_2char(vcoord_in%vc_uuid,   yzuuidin_string)
    ENDIF
    ...
    IF (irecverror(i)/= 0) THEN
      ...
      IF (myid == i) THEN
        ...
        IF ((ylevtyp == 'hybrid') .OR. (ylevtyp == 'hybridLayer')) THEN
          ...
          ENDIF
        ELSEIF ( ((ylevtyp == 'generalVertical') .OR. (ylevtyp == 'generalVerticalLayer')) ) THEN
          PRINT *, 'nlevels               ',vcoord_in%nlevels  ,'       ',iznlev
          PRINT *, 'Number of Grid Used   ',vcoord_in%ivctype  ,'       ',iznrvg
...
!------------------------------------------------------------------------------
  ...
  IF (izdebug > 10) THEN
    WRITE (*,*) "proc ", my_comm_id, ": Enter write_ready_final."
    WRITE (*,*) "proc ", my_comm_id, ": file name head = ", yzhead
  END IF
