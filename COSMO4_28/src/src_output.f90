...
USE messy_main_tracer_mem_bi, ONLY: GPTRSTR, xt_array
...
gribapi_loop1: DO
  ...
  IF (izdebug > 5) THEN
    WRITE (*,'(A)') '  src_output: setting constant GRIB meta data for grib_api'
  ENDIF
...
!------------------------------------------------------------------------------
  ...
  IF ( (outblock%yform_write == 'bina') .AND. (my_cart_id == 0) .AND. (yextension == 'o')) THEN
    ...
    IF     (refatm_in%irefatm == 1) THEN
      izvctype_write = vcoord_in%ivctype
      IF     (vcoord_in%ivctype == 1) THEN
        WRITE (nuedat,IOSTAT=niostat) izvctype_write, refatm_in%p0sl, refatm_in%t0sl, &
              refatm_in%dt0lp, vcoord_in%vcflat, vcoord_in%sigm_coord
      ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
        WRITE (nuedat,IOSTAT=niostat) izvctype_write, refatm_in%p0sl, refatm_in%t0sl, &
              refatm_in%dt0lp, vcoord_in%vcflat, vcoord_in%vert_coord
      ENDIF
    ELSEIF (refatm_in%irefatm == 2) THEN
      izvctype_write = vcoord_in%ivctype+100
      IF     (vcoord_in%ivctype == 1) THEN
        WRITE (nuedat,IOSTAT=niostat) izvctype_write, refatm_in%p0sl, refatm_in%t0sl, &
              refatm_in%dt0lp, vcoord_in%vcflat, vcoord_in%sigm_coord
      ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
        WRITE (nuedat,IOSTAT=niostat) izvctype_write, refatm_in%p0sl, refatm_in%t0sl, &
              refatm_in%dt0lp, vcoord_in%vcflat, vcoord_in%vert_coord
      ENDIF
      WRITE (nuedat,IOSTAT=niostat) refatm_in%delta_t, refatm_in%h_scal
    ELSEIF (refatm_in%irefatm == 3) THEN
      izvctype_write = vcoord_in%ivctype+200
      IF     (vcoord_in%ivctype == 1) THEN
        WRITE (nuedat,IOSTAT=niostat) izvctype_write, refatm_in%p0sl, refatm_in%t0sl, &
              refatm_in%dt0lp, vcoord_in%vcflat, vcoord_in%sigm_coord
      ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
        WRITE (nuedat,IOSTAT=niostat) izvctype_write, refatm_in%p0sl, refatm_in%t0sl, &
              refatm_in%dt0lp, vcoord_in%vcflat, vcoord_in%vert_coord
      ENDIF
      WRITE (nuedat,IOSTAT=niostat) refatm_in%bvref
    ENDIF
  ENDIF
...
!------------------------------------------------------------------------------
  ...
  CALL trcr_get(izerror, 'QV', ptr_tlev = itimelevel, ptr = qv)
  IF (izerror /= 0) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QC', ptr_tlev = itimelevel, ptr = qc)
  IF (izerror /= 0) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  ...
  CALL trcr_get(izerror, 'QI', ptr_tlev = itimelevel, ptr = qi)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QR', ptr_tlev = itimelevel, ptr = qr)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg,                       &
                       yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QS', ptr_tlev = itimelevel, ptr = qs)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QG', ptr_tlev = itimelevel, ptr = qg)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
    ...
    IF (idx_qv < 0) THEN
     ...
     IF (izerror /= TR_NEXIST) CALL tracer_halt(yroutine,izerror)
    ENDIF
    qv => xt_array(itimelevel)%ptr(:,:,idx_qv,:)
    qc => xt_array(itimelevel)%ptr(:,:,idx_qc,:)
    IF (idx_qi >0 ) THEN
       qi     => xt_array(itimelevel)%ptr(:,:,idx_qi,:)
!US not needed any more       qi_new => xt_array(nnew)%ptr(:,:,idx_qi,:)
    ENDIF
    IF (idx_qr >0 ) qr => xt_array(itimelevel)%ptr(:,:,idx_qr,:)
    IF (idx_qs >0 ) qs => xt_array(itimelevel)%ptr(:,:,idx_qs,:)
    IF (idx_qg >0 ) qg => xt_array(itimelevel)%ptr(:,:,idx_qg,:)
#endif

#ifdef TWOMOM_SB
  ! Tracers for the 2-moment scheme
  CALL trcr_get(izerror, 'QH', ptr_tlev = itimelevel, ptr = qh)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNCLOUD', ptr_tlev = itimelevel, ptr = qnc)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNRAIN', ptr_tlev = itimelevel, ptr = qnr)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNICE', ptr_tlev = itimelevel, ptr = qni)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNSNOW', ptr_tlev = itimelevel, ptr = qns)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNGRAUPEL', ptr_tlev = itimelevel, ptr = qng)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNHAIL', ptr_tlev = itimelevel, ptr = qnh)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
...
#else
          IF (luse_rttov .AND. ASSOCIATED(synme7) .AND. (nentry > 0)) THEN
#endif
...
#else
          IF (luse_rttov .AND. ASSOCIATED(synmsg) .AND. (nentry > 0)) THEN
#endif
    ...
    CASE(2)
      ...
      ELSEIF ( ylist(n)(1:izlen) == 'HTOP_DC' ) THEN
        CALL caltopdc( zvarlev(:,:,1), t(:,:,:,itl), p0, pp(:,:,:,itl), &
                       qv(:,:,:), hhl, vcoord_in%vert_coord, ie, je, ke,&
      ...
      ELSEIF ( ylist(n)(1:izlen) == 'HZEROCL' ) THEN
        CALL calhzero( zvarlev(:,:,1), t(:,:,:,itl), hhl, vcoord_in%vert_coord,&
                       ie, je, ke, t0_melt)
      ELSEIF ( ylist(n)(1:izlen) == 'SNOWLMT' ) THEN
        CALL calsnowlmt( zvarlev(:,:,1), t(:,:,:,itl), pp(:,:,:,itl),      &
              p0(:,:,:), qv(:,:,:), hhl, vcoord_in%vert_coord, ie, je, ke, &
              t0_melt, 1.3_ireals)
  ...
  IF (.NOT. lflush) THEN
    ...
    ELSEIF (outblock%yform_write == 'ncdf') THEN
      ...
      IF (var(i1,i2,i3)%lsm == 'i') THEN
        WHERE (fr_lake(1:ie,1:je) <= 0.5) array2d_grib(1:ie,1:je) = undefncdf
      ENDIF
...
#ifdef GRIBDWD
          ! degrib the level
          CALL grbex1(idwdednr, iz_ps, undefgrib, ndims, idims_out, ipds,     &
