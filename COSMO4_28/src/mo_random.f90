...
!
! $Id: mo_random.f90,v 4.28 2013-07-12 11:21:32 for0adm Exp $
!
MODULE mo_random
!
! Description:
...
#define PURE pure
  ...
  ! Modules used
  !-------------
  use mo_kind,        only: dp, i8         ! Double precision only (really!)
...
!!!FTRACE_END  ("knuth_gauss_vector:outer")
  ...
  subroutine test_random_knuth ()
    ...
    real(dp), parameter :: EPS = epsilon (1.d0) / 2
...
!      call construct (state(i), seed=i)
  ...
  subroutine test_gaussian ()
    ...
    real(dp), parameter  :: ref(3,4) = reshape ( (/ &
         ...
         /), (/3, 4/), order=(/1,2/) )
    real(dp), parameter  :: TOL = epsilon (1.d0)        ! Tolerance
