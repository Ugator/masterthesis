...
! conversion factors for horiz. distances from 'km' to rotated grid point units
    ...
    gppkmj = 360.0_ireals / (40042.5_ireals * dlat)
    joffset = NINT( (startlat - startlat_tot) /dlat )
    DO j = jrs_tot , jre_tot
      zcrlats     = COS( (startlat_tot + (joffset+j-1) *dlat) * degrad )
      IF (zcrlats > epsy) THEN
        gppkmi (j) = 360.0_ireals / (40042.5_ireals * zcrlats * dlon)
      ELSE
...
!       IF ((top_con(i,j) > c05) .AND. (mflx_con(i,j) > 0.1_ireals)) THEN
...
! -----------------------------------------------------
    ...
    DO   j = 1 , jeadd
      DO i = 1 , ieadd
        zconbgp (i,j) = 100000.0_ireals
        zcontgp (i,j) =   -400.0_ireals
      ENDDO
    ENDDO
    DO ic = 1 , ncon
      ...
      DO i = istart, iend
        ...
        ELSE
          IF (zconbas(i,j) < hhl(i,j,ke)) THEN
            mconv(noffset+2) = mconv(noffset+2) + 1
          ELSEIF (zconbas(i,j) < 1500.0_ireals) THEN
            mconv(noffset+3) = mconv(noffset+3) + 1
          ELSE
            mconv(noffset+4) = mconv(noffset+4) + 1
          ENDIF
          IF (zcontop(i,j) < 1500.0_ireals) THEN
            mconv(noffset+5) = mconv(noffset+5) + 1
          ELSEIF (zcontop(i,j) < 5000.0_ireals) THEN
            mconv(noffset+6) = mconv(noffset+6) + 1
          ELSE
            mconv(noffset+7) = mconv(noffset+7) + 1
          ENDIF
          noffset = 28
          IF (zcontop(i,j)-zconbas(i,j) < 1500.0_ireals) THEN
            mconv(noffset+1) = mconv(noffset+1) + 1
          ELSEIF (zcontop(i,j)-zconbas(i,j) < 3000.0_ireals) THEN
            mconv(noffset+2) = mconv(noffset+2) + 1
          ELSEIF (zcontop(i,j)-zconbas(i,j) < 5000.0_ireals) THEN
            mconv(noffset+3) = mconv(noffset+3) + 1
          ELSE
            mconv(noffset+4) = mconv(noffset+4) + 1
          ENDIF
...
! convert hPa to Pa
    DO ivar = 1 , 4
      topobs (ivar) = topobs(ivar) * 100.0_ireals
      botmod (ivar) = botmod(ivar) * 100.0_ireals
    ENDDO
    ptpstop = ptpstop * 100.0_ireals
...
!     kml250 = 0
      ...
      DO kk = 1 , ke
        wvgeo (kk) = c1
        IF (c05*(vcoord_in%sigm_coord(kk)+vcoord_in%sigm_coord(kk+1)) > 0.95_ireals)                         &
          wvgeo (kk) = MIN( (c1 - c05 *(vcoord_in%sigm_coord(kk) + vcoord_in%sigm_coord(kk+1))) * 20._ireals &
                          , c1 )
!       IF (      (vcoord_in%sigm_coord(kk)  <= 0.25_ireals)                                  &
!           .AND. (vcoord_in%sigm_coord(kk+1) > 0.25_ireals)) kml250 = kk
      ENDDO
    ENDIF
...
!   IF ( (num_compute > 1) .AND. (lcroot) .AND. (lnudge)) THEN
    IF ((lcroot) .AND. (lnudge)) THEN
      nhend = INT( MIN( nstop , nudgend ) * dtdeh + epsy ) + 2
...
! adjust namelist variables
    ptpstop = MIN( ptpstop , 70000.0_ireals , -zgather(4) -10000.0_ireals )
    IF (zgather(5) > epsy) THEN
      ionl2     = ionl
      jonl2     = jonl
      lwonl2    = lwonl
    ENDIF
    onl_cart_id = zgather(6)
      ...
      IF (lwonl) THEN
        ...
        DO itmp = 1 , ntotys
          ...
          ELSE
            yscaldp        = ' '
          ENDIF
          ntimysu = NINT( MAX( oysu(iysu,17) , -9.0_ireals ) )
...
!        .AND. (lnudge) .AND. (ntstep == nulast)) THEN
    IF ((lcroot) .AND. (lnudge) .AND. (ntstep == nulast)) THEN
      ...
      nhend = INT( MIN( nstop , nudgend ) * dtdeh + epsy ) + 1
      DO nhour = 1 , nhend
