...
USE vgrid_refatm_utils,       ONLY :  &
    refatm_in, refatm_out, vcoord_in, vcoord_out, nfltvc_in, svc1_in, svc2_in
...
IF (ALLOCATED(pv_in)) THEN
  ...
  ELSEIF ((l_ke_in_gds) .AND. (.NOT. l_ke_in_input)) THEN
    ...
    ! present in pv_in  
    zpvloc( 1) = REAL (vcoord_in%ivctype, ireals)
  ...
  ELSEIF ((.NOT. l_ke_in_gds) .AND. (l_ke_in_input)) THEN
    ...
    IF (vcoord_in%ivctype == 3) THEN
      ! the first value after the vertical coordinate parameters in the old
      ! coding style is ivctype: in the new style this is already in pv(1)
      zpvloc (4+ke+1+1) = REAL (vcoord_in%ivctype, ireals)
...
ELSE   
  ...
  IF (.NOT. l_ke_in_gds) THEN
    ...
    ELSE                     ! NON SLEVE coordinates
      inrvert_out = 5 + ke+1
    ENDIF
    IF (refatm_in%irefatm == 2) inrvert_out = 10 + ke+1
  ELSE
    ...
    ELSE                     ! NON SLEVE coordinates
      inrvert_out = 6 + ke+1
    ENDIF
    IF (refatm_in%irefatm == 2) inrvert_out = 11 + ke+1
  ENDIF
  ...
  IF (.NOT. l_ke_in_gds) THEN
    ...
    pv_out( 1) = refatm_in%p0sl
    pv_out( 2) = refatm_in%t0sl
    pv_out( 3) = refatm_in%dt0lp
    pv_out( 4) = vcoord_in%vcflat

    IF     (vcoord_in%ivctype == 1) THEN
      DO k = 1, ke+1
        pv_out (4 + k) = vcoord_in%sigm_coord(k)
      ENDDO
    ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
      DO k = 1, ke+1
        pv_out (4 + k) = vcoord_in%vert_coord(k)
      ENDDO
    ENDIF

    ! But this must be coded as integer later on because of 
    ! backwards compatibility
    pv_out (4+ke+1+1) = REAL (vcoord_in%ivctype, ireals)

    IF (vcoord_in%ivctype == 3) THEN
      pv_out (4+ke+1+2) = svc1_in
      pv_out (4+ke+1+3) = svc2_in
      pv_out (4+ke+1+4) = REAL(nfltvc_in,ireals)
    ENDIF

    IF (refatm_in%irefatm == 2) THEN ! Write parameters for new reference atmosphere
      pv_out (4+ke+1+1) = REAL(vcoord_in%ivctype+100, ireals)
      IF (vcoord_in%ivctype /= 3) THEN
        pv_out (4+ke+1+2) = 0.0_ireals
        pv_out (4+ke+1+3) = 0.0_ireals
        pv_out (4+ke+1+4) = 0.0_ireals
      ENDIF
      pv_out (4+ke+1+5) = refatm_in%delta_t
      pv_out (4+ke+1+6) = refatm_in%h_scal
    ENDIF
  ELSE
    ...
    IF (refatm_in%irefatm == 1) THEN
      pv_out ( 1) = REAL (vcoord_in%ivctype, ireals)
    ELSE IF (refatm_in%irefatm == 2) THEN
      pv_out ( 1) = REAL (vcoord_in%ivctype+100, ireals)
    ENDIF

    pv_out ( 2) = REAL (ke,  ireals)
    pv_out ( 3) = refatm_in%p0sl
    pv_out ( 4) = refatm_in%t0sl
    pv_out ( 5) = refatm_in%dt0lp
    pv_out ( 6) = vcoord_in%vcflat

    IF     (vcoord_in%ivctype == 1) THEN
      DO k = 1, ke+1
        pv_out (6 + k) = vcoord_in%sigm_coord(k)
      ENDDO
    ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
      DO k = 1, ke+1
        pv_out (6 + k) = vcoord_in%vert_coord(k)
      ENDDO
    ENDIF

    IF (vcoord_in%ivctype == 3) THEN
      ! SLEVE coordinate: 3 new parameters are written to GDS
      pv_out (6+ke+1+1) =      svc1_in
      pv_out (6+ke+1+2) =      svc2_in
      pv_out (6+ke+1+3) = REAL(nfltvc_in, ireals)
    ENDIF
    ...
    IF (refatm_in%irefatm == 2) THEN ! Write parameters for new reference atmosphere
      ...
      ENDIF
      pv_out (6+ke+1+4) = refatm_in%delta_t
      pv_out (6+ke+1+5) = refatm_in%h_scal
    ENDIF
  ENDIF
...
!------------------------------------------------------------------------------
! Section 2: set vcoord_out
!------------------------------------------------------------------------------

vcoord_out = vcoord_in

!------------------------------------------------------------------------------
! Section 3: set refatm_out
!------------------------------------------------------------------------------

refatm_out = refatm_in
...
!------------------------------------------------------------------------------
    ...
    ipds(3)  = ncenter            ! centre identification
    ipds(5)  = 255_intgribf       ! number of used grid
    ipds(6)  = 128_intgribf       ! additional flags
...
CASE ('api2')
  ...
  CASE (78)
    ...
    CALL grib_set (izgrbid, 'localNumberOfExperiment',      izmodnvers)  ! GRIB1: ipds(47)
  ...
  CASE DEFAULT
    ...
    ! local number of experiment: set "pure" experiment number from nvers
    CALL grib_set (izgrbid, 'localNumberOfExperiment',      nvers)  ! GRIB1: ipds(47)

  END SELECT

  ! local information number
  CALL grib_set (izgrbid, 'localInformationNumber',                0)  !  ipds(41)
...
!------------------------------------------------------------------------------
  ...
  CASE ('grb1','bina')
    ...
    ipds(2)  = lst_gribtabs(n3)   ! GRIB1 table number
    IF (ptr_to_out%lanalysis .OR. ptr_to_out%lsfc_ana) THEN
      ipds(4)  = ptr_to_out%nprocess_ini_out ! generating process identification
    ELSE
      ipds(4)  = ptr_to_out%nprocess_bd_out  ! generating process identification
    ENDIF
    ipds(7)  = n2                 ! element number / identification of parameter
...
!US     IF (leps) THEN
!US       CALL grib_set (igrbid,'productDefinitionTemplateNumber', 33) ! individual ensemble forecast
!US     ELSE
...
!     CALL grib_set (igrbid, 'level',                          nlevel)
      ...
      CALL grib_set (igrbid, 'nlev',               vcoord_out%nlevels)
      CALL grib_set (igrbid, 'numberOfVGridUsed',  vcoord_out%ivctype)
      CALL grib_set (igrbid, 'uuidOfVGrid',        vcoord_out%vc_uuid)
...
!     CALL grib_set (igrbid,'numberOfVerticalGridDescriptors',      6)
    ...
    END SELECT
     ...
     IF (TRIM(yzname) /= 'HHL') THEN
      !!    IF (yzlevtyp(1:15) /= 'generalVertical') THEN
      ! setting it afterwards gives problems in setting the levels for hhl?!?!?!
      CALL grib_set (igrbid,'shortName',        TRIM(yzname))
    ENDIF
#endif
  ...
  !     GRIB1: ipds(11,12,13,14,15,16,17,18,19,22)
  ...
  ipds(15) = INT (nmingb, intgribf)
   
  ! Indicator of unit of time range
  ipds(16) = INT (ptr_to_out%nunit_of_time, intgribf)
...
!print *, 'setting steps:     ', yzname, '  ', yzsteptyp, '  ', yzlevtyp, '  ', istartstep, iendstep
...
!print *, 'setting statproc:  ', yzname, '  ', yzsteptyp, '  ', yzlevtyp, '  ', istartstep, iendstep
      CALL grib_set (igrbid,'startStep',             istartstep)     !
      CALL grib_set (igrbid,'endStep',                 iendstep)     !
  ...
  !     GRIB1: ipds(37,41,42-46,47,50-52)
...
! ipds(23) = 0_intgribf
...
#if defined RTTOV7 || defined RTTOV9 || defined RTTOV10
    ipds(41) = sat_compute(NINT(slev))%ngrib_aees(nlevel)
#endif
  ELSE
  ...
  CASE ('api1')
    ...
    SELECT CASE (ncenter)
    CASE (78)  ! DWD
      CALL grib_set (igrbid,'localDefinitionNumber',      ipds(37))
      CALL grib_set (igrbid,'localInformationNumber',     ipds(41))
 
      CALL grib_set (igrbid,'localDecodeDateYear',        ipds(42))
      CALL grib_set (igrbid,'localDecodeDateMonth',       ipds(43))
      CALL grib_set (igrbid,'localDecodeDateDay',         ipds(44))
      CALL grib_set (igrbid,'localDecodeDateHour',        ipds(45))
      CALL grib_set (igrbid,'localDecodeDateMinute',      ipds(46))
      CALL grib_set (igrbid,'localVersionNumber',         ipds(47))

      IF (leps) THEN
        CALL grib_set (igrbid,'etyp',                     ipds(50))
        CALL grib_set (igrbid,'etot',                     ipds(51))
        CALL grib_set (igrbid,'enum',                     ipds(52))
      ENDIF
  ...
  CASE ('api2')
    ...
    CASE (78)  ! DWD
      ...
      IF     ( TRIM(yzname) == 'HHL') THEN
        CALL grib_set (igrbid,'localInformationNumber',     vcoord_out%ivcoord_id)
      ELSEIF ((TRIM(yzname) == 'PP') .OR. (TRIM(yzname) == 'P')) THEN
        CALL grib_set (igrbid,'localInformationNumber',     refatm_out%irefatm_id)
      ELSE
        CALL grib_set (igrbid,'localInformationNumber',     ipds(41))
      ENDIF
