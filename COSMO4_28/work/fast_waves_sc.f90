...
! Use vcoord_in%kflat from new module vgrid_refatm_utils (US)
!
! Code Description:
! Language: Fortran 90.
...
USE vgrid_refatm_utils, ONLY: &
  vcoord_in       ! definitions for vertical grid (here: kflat)
...
SUBROUTINE init_fast_waves_sc_1
  ...
  ! to be sure:
  vcoord_in%kflat = MAX( 1, vcoord_in%kflat )    ! upper half level (k=1) is flat in any case
  ...
  IF ( l_hor_p_grad_Mahrer ) THEN
    ...
    k_index_same_level_Mahrer = vcoord_in%kflat-1
    is_search_finished = .FALSE.

    DO k=vcoord_in%kflat, ke
...
SUBROUTINE fast_waves_strong_conserv ( n_small_steps, dts, xkd,             &
  ...
  ALLOCATE( dpp_dz_s(ie, je, vcoord_in%kflat:ke), STAT=izstata)
  ...
  div_tilde_vertic_uv(:,:,1:MAX(1,vcoord_in%kflat-1)) = 0.0_ireals
  ...
  DO n_step = 1, n_small_steps 
    ...
    IF ( .NOT. l_hor_p_grad_Mahrer ) THEN
      ...
      ! (is only needed for k=vcoord_in%kflat...ke)

      DO j=j_start_v, jendv + 1

        DO k=MAX(2,vcoord_in%kflat), ke
        ...
        DO k=MAX(2,vcoord_in%kflat), ke-1
          DO i=i_start_u, iendu + 1
      ...
      IF ( vcoord_in%kflat==1 ) THEN
        ...
        END DO
      ELSE
        ! dpp_dz_s(k=1) not needed, if vcoord_in%kflat > 1
      END IF
    ...
    k1 = MIN( vcoord_in%kflat-1, k_bottom )
    ...
    IF ( l_hor_p_grad_Mahrer ) THEN
      ...
      ELSE
        ...
        k2 = k1    ! i.e. the Mahrer-discretization immediately starts below vcoord_in%kflat
      END IF
    ELSE
      k2 = k_bottom
    END IF
...
SUBROUTINE calc_dz_dx( dz_dlam, dz_dphi,               &
  ...
  PRINT*, "Subr. [calc_dz_dx] ..."

  dz_dlam(:,:, 1:vcoord_in%kflat) = 0.0_ireals
  DO k=vcoord_in%kflat+1, ke1
    DO j=1, je
      DO i=1, ie-1
  ...
  dz_dphi(:,:, 1:vcoord_in%kflat) = 0.0_ireals
  DO k=vcoord_in%kflat+1, ke1
    DO j=1, je-1
      DO i=1, ie
  ...
  dz_lam_u_half(:,:,1:vcoord_in%kflat-1) = 0.0_ireals
  dz_phi_v_half(:,:,1:vcoord_in%kflat-1) = 0.0_ireals
  DO k=vcoord_in%kflat, ke
    DO j=1, je
      DO i=1, ie
...
SUBROUTINE init_div_damping_coeff( xkd, dts, alpha_div_h, alpha_div_v_to_h )
  ...
  ELSE   ! limit_alpha_div_by_slope_stability = .TRUE.
    ...
    alpha_div_h(:,:, 1:vcoord_in%kflat-1) = alpha_div_h_limit
...
SUBROUTINE calc_Z_horiz ( u, v, u_sfc, v_sfc, Z_horiz )
  ...
  Z_horiz(:,:,1:MAX(1,vcoord_in%kflat)) = 0.0_ireals

  DO k=MAX(2,vcoord_in%kflat+1), ke
