...
!CDIR NOMOVE
DO i = istarts, iends
  fetch_lk(i,j) = 1.E4_ireals           ! Use a constant long fetch
...
DO i = istarts, iends
  IF(depth_lk(i,j) > 0.0_ireals) THEN
    ...
    h_ml_lk(i,j,nx) = MIN(depth_lk(i,j), MAX(h_ml_lk(i,j,nx), 0._ireals))
...
!CDIR NOMOVE
DO i = istarts, iends
  Lake_points: IF(depth_lk(i,j) > 0.0_ireals) THEN
...
IF (lmulti_snow) THEN
  ...
  DO ks = 0, ke_snow  ! DO loop over snow layers
    WHERE (depth_lk(:,:) > 0.0_ireals) t_snow_mult(:,:,ks,nt) = t_snow(:,:,nx)
  END DO
  END DO
ENDIF
...
IF (lmulti_layer) THEN
  ...
  DO ks = 1, ke_soil  ! DO loop over soil layers
    WHERE (depth_lk(:,:) > 0.0_ireals) t_so(:,:,ks,nt) = tpl_T_r
  END DO
  END DO
ENDIF
...
REAL (KIND = ireals), PARAMETER :: &
  Vel_min = 1.0E-02          ! Minimum wind speed [m s^{-1}] 
...
#ifdef MESSY
LOGICAL, SAVE :: l_init_pointers = .TRUE.
#endif
...
ENDIF
  ...
  IF (.NOT.ASSOCIATED(qv)) THEN
     CALL get_tracer(izerror, GPTRSTR, 'QV',  pxtm1=qv)
...
!_nu albedo_ice   = albedo_whiteice_ref*(1._ireals-albedo_ice) + 
...
!------------------------------------------------------------------------------
    ...
    dMsnowdt_flk = 0._ireals      ! Snow is not considered
...
IF (lmulti_snow) THEN
  ...
  DO ksnow = 0, ke_snow  ! DO loop over snow layers
    WHERE (depth_lk(:,:) > 0.0_ireals) t_snow_mult(:,:,ksnow,nnew) = t_snow(:,:,nnew)
  END DO
ENDIF
