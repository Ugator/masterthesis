...
!
! Current Code Owner: DWD, Reinhold Hess + Christina Koepken
!  phone:  +49  69  8062 2723
!  fax:    +49  69  8062 3721
...
!------------------------------------------------------------------------------
  ...
  ENDIF
#else
  IF (.NOT. ASSOCIATED(qi)) THEN
     CALL get_tracer(izerror, GPTRSTR, 'QV',  pxtm1=qv)
     CALL tracer_halt(yzroutine,izerror)
     CALL get_tracer(izerror, GPTRSTR, 'QC',  pxtm1=qc)
     CALL tracer_halt(yzroutine,izerror)
     CALL get_tracer(izerror, GPTRSTR, 'QI',  pxtm1=qi)
...
! -------------
  INTEGER (KIND=iintegers) ::  &
    i,  j,  k, izerror     !  Loop indices in longitudinal, latitudinal and
...
!------------------------------------------------------------------------------
  ...
  DO      k = 1,         ke
    DO    j = jstart-1 , jend
      DO  i = istart-1 , iend
        psi_u(i,j,k) = 0.5 * (psih(i,j,k)+psih(i+1,j,k)) * u(i,j,k,nnow)
        psi_v(i,j,k) = 0.5 * (psih(i,j,k)+psih(i,j+1,k)) * v(i,j,k,nnow)    &
                                                         * crlat(j,2)
      ENDDO
    ENDDO
  ENDDO
