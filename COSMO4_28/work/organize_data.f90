...
USE messy_main_tracer_mem_bi, ONLY: ti_gp, ntrac_gp
USE messy_main_tracer_bi,     ONLY: tracer_halt
! MESSy/SMCL
USE messy_main_tracer,        ONLY: I_INITIAL, I_LBC, T_INI_FILE, T_LBC_FILE
#endif
...
ELSEIF (yaction == 'start') THEN
  ...
  DO iztrcr = 1, ntrac_gp
     IF (ti_gp(iztrcr)%tp%meta%cask_i(I_INITIAL) == T_INI_FILE) THEN
        yvarini(nyvar_i+1) = TRIM(ti_gp(iztrcr)%tp%ident%fullname)
        nyvar_i = nyvar_i + 1
     ENDIF
  END DO
#endif
...
! Variables for boundary data
  ...
  DO iztrcr = 1, ntrac_gp
    ! check for each tracer if lateral BC should be read from file
    IF (ti_gp(iztrcr)%tp%meta%cask_i(I_LBC) == T_LBC_FILE) THEN

      yvarbd (nyvar_b+1) = TRIM(ti_gp(iztrcr)%tp%ident%fullname)
      nyvar_b = nyvar_b + 1
    ENDIF
...
!   ENDIF
  ...
  ENDIF
#else
  CALL tracer_halt('gen artifdata',ierror)
#endif
...
#else
    iout = iout +1
    IF ( (L_BM_ORIG_OUTPUT).OR. L_FORCE_calcout) THEN
  ...
  IF(my_cart_id == 0) THEN
    ...
    DO k = 1, ke+1
      WRITE(nuspecif,'(1X,I5,F12.4,F12.4)' ) &
         k, vcoord_in%vert_coord(k), vcoord_in%sigm_coord(k)*refatm_in%p0sl*0.01
    ENDDO

    WRITE(nuspecif,'(A)')         ' '
    WRITE(nuspecif,'(A,F10.4)')                                            &
      '   Boundary for change from z to terrain-following: vcflat = ', vcoord_in%vcflat
    WRITE(nuspecif,'(A,I3)')                                            &
      '   Half-level index where levels become flat: k = ' , vcoord_in%kflat
    WRITE(nuspecif,'(A)')         ' '
    IF (refatm_in%irefatm == 1) THEN
      ...
      WRITE(nuspecif,'(A37,F15.4)')                                            &
       '   Pressure at Sea-Level:            ', refatm_in%p0sl * 0.01_ireals
      WRITE(nuspecif,'(A37,F15.4)')                                            &
       '   Temperature at Sea-Level:         ', refatm_in%t0sl-273.15_ireals
      WRITE(nuspecif,'(A37,F15.4)')                                            &
       '   Temperature-Rate of decrease:     ', refatm_in%dt0lp
    ELSE IF (refatm_in%irefatm == 2) THEN
      ...
      WRITE(nuspecif,'(A37,F15.4)')                                            &
       '   Temperature at Sea-Level:         ', refatm_in%t0sl-273.15_ireals
      WRITE(nuspecif,'(A37,F15.4)')                                            &
       '   Temp. diff. sea-level-stratosph.: ', refatm_in%delta_t
      WRITE(nuspecif,'(A37,F15.4)')                                            &
       '  Scale height for temp. decrease:   ', refatm_in%h_scal
    ELSE IF (refatm_in%irefatm == 3) THEN
      ...
      WRITE(nuspecif,'(A37,F15.4)')                                            &
       '   Temperature at Sea-Level:         ', refatm_in%t0sl-273.15_ireals
      WRITE(nuspecif,'(A37,F15.4)')                                            &
       '   Brunt-Vaisala-frequency           ', refatm_in%bvref
    ENDIF
    WRITE(nuspecif,'(A)')         ' '
    IF (vcoord_in%ivctype == 3) THEN
       WRITE(nuspecif,'(A,F10.4)')                                            &
         '   Decay Rate for Large-Scale Topography: svc1 = ', svc1_in
       WRITE(nuspecif,'(A,F10.4)')                                            &
         '   Decay Rate for Small-Scale Topography: svc2 = ', svc2_in
    ENDIF
