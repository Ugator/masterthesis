...
USE vgrid_refatm_utils, ONLY :  &
  refatm_in, vcoord_in, nfltvc_in, svc1_in, svc2_in
...
!_br 25.11.08
  ELSE ! model coordinate
      ...
      IF     (vcoord_in%ivctype == 1) THEN
        zvcoord(1:ke1) = vcoord_in%sigm_coord(1:ke1)
      ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
        zvcoord(1:ke1) = vcoord_in%vert_coord(1:ke1)
      ENDIF
      ...
      IF (vcoord_in%ivctype == 1) THEN
        ...
        IF (ierror /= NF90_NOERR) THEN
          ...
          RETURN
        ENDIF
      ELSE IF (vcoord_in%ivctype == 2) THEN
        ierror=nf90_put_att(ncid, jvcVarID, "long_name",                     &
        ...
        IF (ierror /= NF90_NOERR) THEN
          ...
          RETURN
        ENDIF
      ELSE IF (vcoord_in%ivctype == 3) THEN
        ierror=nf90_put_att(ncid, jvcVarID, "long_name",                     &
                                                           "SLEVE coordinate")
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
      ...
      IF (ierror /= NF90_NOERR) THEN
        ...
        RETURN
      ENDIF
      ierror=nf90_put_att(ncid, jvcVarID, "ivctype", vcoord_in%ivctype)
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
      ENDIF
      ierror=nf90_put_att(ncid, jvcVarID, "irefatm", refatm_in%irefatm)
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
      ENDIF
      ierror=nf90_put_att(ncid, jvcVarID, "p0sl", refatm_in%p0sl)
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
      ENDIF
      ierror=nf90_put_att(ncid, jvcVarID, "t0sl", refatm_in%t0sl)
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
      ENDIF
      ierror=nf90_put_att(ncid, jvcVarID, "dt0lp", refatm_in%dt0lp)
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
      ENDIF
      ierror=nf90_put_att(ncid, jvcVarID, "vcflat", vcoord_in%vcflat)
      IF (ierror /= NF90_NOERR) THEN
        yerrmsg = TRIM(NF90_strerror(ierror))
        RETURN
      ENDIF
      ...
      IF (vcoord_in%ivctype == 3) THEN
        ierror=nf90_put_att(ncid, jvcVarID, "svc1", svc1_in)
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
        ierror=nf90_put_att(ncid, jvcVarID, "svc2", svc2_in)
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
        ierror=nf90_put_att(ncid, jvcVarID, "nfltvc", nfltvc_in)
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
      ...
      IF (refatm_in%irefatm == 2) THEN
        ierror=nf90_put_att(ncid, jvcVarID, "delta_t", refatm_in%delta_t)
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
        ierror=nf90_put_att(ncid, jvcVarID, "h_scal", refatm_in%h_scal)
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
      ELSE IF (refatm_in%irefatm == 3) THEN
        ierror=nf90_put_att(ncid, jvcVarID, "bvref", refatm_in%bvref)
        IF (ierror /= NF90_NOERR) THEN
          yerrmsg = TRIM(NF90_strerror(ierror))
          RETURN
        ENDIF
  ...
  IF( nc_asyn_io > 0 .AND. &
    ...
    IF(my_world_id == 0) THEN
      ...
      IF     (vcoord_in%ivctype == 1) THEN
        DO i=1,ke1
          realcnt = realcnt+1
          realbuf(realcnt) = vcoord_in%sigm_coord(i)
        ENDDO
      ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
        DO i=1,ke1
          realcnt = realcnt+1
          realbuf(realcnt) = vcoord_in%vert_coord(i)
        ENDDO
      ENDIF
      ...
      realcnt = realcnt+1
      realbuf(realcnt) = refatm_in%p0sl

      realcnt = realcnt+1
      realbuf(realcnt) = refatm_in%t0sl
 
      realcnt = realcnt+1
      realbuf(realcnt) = refatm_in%dt0lp

      realcnt = realcnt+1
      realbuf(realcnt) = vcoord_in%vcflat 
...
!US      ALLOCATE ( vcoord (ke+1)      , STAT=istat )
      ...
      IF     (vcoord_in%ivctype == 1) THEN
        DO i=1,ke1
          realcnt = realcnt+1
          vcoord_in%sigm_coord(i) = realbuf(realcnt)
        ENDDO
      ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
        DO i=1,ke1
          realcnt = realcnt+1
          vcoord_in%vert_coord(i) = realbuf(realcnt)
        ENDDO
      ENDIF
      ...
      realcnt = realcnt+1
      refatm_in%p0sl =  realbuf(realcnt)

      realcnt = realcnt+1
      refatm_in%t0sl =  realbuf(realcnt)

      realcnt = realcnt+1
      refatm_in%dt0lp =  realbuf(realcnt)

      realcnt = realcnt+1
      vcoord_in%vcflat =  realbuf(realcnt)
