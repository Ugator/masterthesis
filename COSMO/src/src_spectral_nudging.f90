...
USE vgrid_refatm_utils, ONLY : &
    vcoord_in           ! parameters for the vertical grid
...
! for reference pressure   p0sl = 10E5_ireals
    IF (      (vcoord_in%sigm_coord(k)  <= pp_sn/1000._ireals)   &
        .AND. (vcoord_in%sigm_coord(k+1) > pp_sn/1000._ireals)) THEN
      k_sn = k
      EXIT
    ENDIF
  ENDDO
  ...
  DO k = 1, k_sn
    wgtvd_sn(k) = ( 1.0_ireals -                                               &
     MIN ( ((vcoord_in%sigm_coord(k)+vcoord_in%sigm_coord(k+1))*0.5E5_ireals), &
                     pp_sn*100.0_ireals )/                                     &
