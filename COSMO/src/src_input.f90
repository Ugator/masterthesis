...
USE vgrid_refatm_utils, ONLY:                                              &
    ...
    vcoord_in, refatm_in, rundefined, nfltvc_in, svc1_in, svc2_in,         &
...
PRIVATE bd_from_id, id_mix_bd, get_vertcoord, fill_procarray,              &
        scatter_data, read_nc_gdefs, read_nc_vdefs
...
INTEGER  (KIND=iintegers)  ::  &
  ...
  igenproc, izdimpvin, izrefatm_id
...
REAL (KIND=ireals)         ::  &
  ...
  zvcoord_in(ke1)    ! Additional variables
...
!------------------------------------------------------------------------------
  ...
  IF ( (yzformat_tmp == 'bina') .AND. (my_cart_id == 0) .AND. (yextension == 'o')) THEN
    ...
    READ (nufile,IOSTAT=niostat) izvctype_read, refatm_in%p0sl,   refatm_in%t0sl,   &
                               refatm_in%dt0lp, vcoord_in%vcflat, zvcoord_in
    IF     ( (izvctype_read >   0) .AND. (izvctype_read <= 100) ) THEN
      refatm_in%irefatm = 1
      vcoord_in%ivctype = izvctype_read
    ELSEIF ( (izvctype_read > 100) .AND. (izvctype_read <= 200) ) THEN
      refatm_in%irefatm = 2
      vcoord_in%ivctype = izvctype_read - 100
      READ (nufile,IOSTAT=niostat) refatm_in%delta_t, refatm_in%h_scal
    ELSEIF ( (izvctype_read > 200) .AND. (izvctype_read <= 300) ) THEN
      refatm_in%irefatm = 3
      vcoord_in%ivctype = izvctype_read - 200
      READ (nufile,IOSTAT=niostat) refatm_in%bvref
    ENDIF
    vcoord_in%nlevels           = ke1
    IF     (vcoord_in%ivctype == 1) THEN
      vcoord_in%sigm_coord(1:ke1) = zvcoord_in(1:ke1)
    ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
      vcoord_in%vert_coord(1:ke1) = zvcoord_in(1:ke1)
    ENDIF
  ENDIF
...
!------------------------------------------------------------------------------
  ...
  !----------------------------------------------------------------------------
    ...
    CASE ('bina')
      ...
      CALL read_restart (nufile, ie_tot, je_tot, ds_real, ipds, npds,       &
      ...
      IF (izdebug >= 20) THEN
        IF (iz_rsize == 0) THEN
          PRINT *, '       EOF reached'
        ELSE
          PRINT *, '       Got record ', ipds(2), ipds(7), ipds(8)
        ENDIF
      ENDIF
    ...
    IF (.NOT. leof) THEN 
      IF     (yzformat_tmp == 'grb1') THEN
        ...
        CALL grbin1(idwdednr, undefgrib, ndims, idims_in, iblock, ibmap,       &
                    ipds, igds_in, ibms, ibds, dsup, ds_grib, ierrf)
        IF (ierrf /= 0) THEN
  ...
  !----------------------------------------------------------------------------
    ...
    IF (.NOT. lgot_vcoord) THEN
      ...
      IF ( (yzformat_tmp == 'grb1') .OR. (yzformat_tmp == 'apix') ) THEN
        IF (igrbednr == 1) THEN
          ...
          IF (izexch < nproc) THEN
            ...
            CALL get_vertcoord (inrvert_in, pv_in, igrbednr)
            lgot_vcoord = .TRUE.

            ! check, whether a default set of reference atmosphere parameters corresponds 
            ! to the parameters read:
            refatm_in%irefatm_id = -1    ! means: not available

            search_refatm1: DO n = 1, imax_refatmtype
              IF     (refatm_in%irefatm == 1) THEN
                IF ( (refatm_in%irefatm == refatm_defaults(n)%irefatm)            .AND.   &
                     (ABS(refatm_in%p0sl    - refatm_defaults(n)%p0sl   ) < 1E-3_ireals)    .AND.   &
                     (ABS(refatm_in%t0sl    - refatm_defaults(n)%t0sl   ) < 1E-3_ireals)    .AND.   &
                     (ABS(refatm_in%dt0lp   - refatm_defaults(n)%dt0lp  ) < 1E-3_ireals) ) THEN
                  refatm_in%irefatm_id = n
                  refatm_in%delta_t    = rundefined
                  refatm_in%h_scal     = rundefined
                  refatm_in%bvref      = rundefined
                  IF (my_cart_id == 0) THEN
                    PRINT *, ' *** A default set for refatm parameters is used: ', refatm_in%irefatm_id
                  ENDIF
                  EXIT search_refatm1
                ENDIF
              ELSEIF (refatm_in%irefatm == 2) THEN
                IF ( (refatm_in%irefatm == refatm_defaults(n)%irefatm)            .AND.   &
                     (ABS(refatm_in%p0sl    - refatm_defaults(n)%p0sl   ) < 1E-3_ireals)    .AND.   &
                     (ABS(refatm_in%t0sl    - refatm_defaults(n)%t0sl   ) < 1E-3_ireals)    .AND.   &
                     (ABS(refatm_in%delta_t - refatm_defaults(n)%delta_t) < 1E-3_ireals)    .AND.   &
                     (ABS(refatm_in%h_scal  - refatm_defaults(n)%h_scal ) < 1E-3_ireals) ) THEN
                  refatm_in%irefatm_id = n
                  ...
                  EXIT search_refatm1
                ENDIF
              ELSEIF (refatm_in%irefatm == 3) THEN
                IF ( (refatm_in%irefatm == refatm_defaults(n)%irefatm)            .AND.   &
                     (ABS(refatm_in%p0sl    - refatm_defaults(n)%p0sl   ) < 1E-3_ireals)    .AND.   &
                     (ABS(refatm_in%t0sl    - refatm_defaults(n)%t0sl   ) < 1E-3_ireals)    .AND.   &
                     (ABS(refatm_in%bvref   - refatm_defaults(n)%bvref  ) < 1E-3_ireals) ) THEN
                  refatm_in%irefatm_id = n
                  refatm_in%dt0lp      = rundefined
                  refatm_in%delta_t    = rundefined
                  refatm_in%h_scal     = rundefined
      ...
      ELSEIF (yzformat_tmp == 'bina') THEN
        ...
        IF (my_cart_id == 0) THEN
          intbuf(1) = vcoord_in%ivctype
          intbuf(2) = refatm_in%irefatm
          intbuf(3) = izvctype_read     ! for convenience: then we do not need to recompute
          zrbuf (1) = refatm_in%p0sl
          zrbuf (2) = refatm_in%t0sl
          zrbuf (3) = refatm_in%dt0lp
          zrbuf (4) = vcoord_in%vcflat
          DO k = 1, ke1
            zrbuf (4+k) = zvcoord_in(k)
          ENDDO
          inrvert_in = 4+ke1
          IF (refatm_in%irefatm == 2) THEN
            zrbuf (4+ke1+1) = refatm_in%delta_t
            zrbuf (4+ke1+2) = refatm_in%h_scal
            inrvert_in = 4+ke1+2
          ENDIF
          IF (refatm_in%irefatm == 3) THEN
            zrbuf (4+ke1+1) = refatm_in%bvref
            inrvert_in = 4+ke1+1
          ENDIF
        ENDIF
        IF (num_compute > 1) THEN
        ...
        vcoord_in%ivctype   = intbuf(1)
        refatm_in%irefatm   = intbuf(2)
        izvctype_read       = intbuf(3)
        refatm_in%p0sl      = zrbuf (1)
        refatm_in%t0sl      = zrbuf (2)
        refatm_in%dt0lp     = zrbuf (3)
        vcoord_in%vcflat    = zrbuf (4)

        pv_in( 1) = REAL (izvctype_read, ireals)    ! has been read above
        pv_in( 2) = REAL (ke,            ireals)
        pv_in( 3) = refatm_in%p0sl
        pv_in( 4) = refatm_in%t0sl
        pv_in( 5) = refatm_in%dt0lp
        pv_in( 6) = vcoord_in%vcflat

        vcoord_in%nlevels           = ke1
        IF     (vcoord_in%ivctype == 1) THEN
          DO k = 1, ke1
            vcoord_in%sigm_coord(k) = zrbuf (4+k)
            pv_in(6+k)              = zrbuf (4+k)
          ENDDO
        ELSEIF (vcoord_in%ivctype == 2) THEN
          DO k = 1, ke1
            vcoord_in%vert_coord(k) = zrbuf (4+k)
            pv_in(6+k)              = zrbuf (4+k)
          ENDDO
        ENDIF
        inrvert_in = 6+ke1
        IF (refatm_in%irefatm == 2) THEN
          refatm_in%delta_t = zrbuf (4+ke1+1)
          refatm_in%h_scal  = zrbuf (4+ke1+2)
          pv_in(6+ke1+1)    = 0.0_ireals
          pv_in(6+ke1+2)    = 0.0_ireals
          pv_in(6+ke1+3)    = 0.0_ireals
          pv_in(6+ke1+4)    = refatm_in%delta_t
          pv_in(6+ke1+5)    = refatm_in%h_scal
          inrvert_in = 6+ke1+5
        ENDIF
        IF (refatm_in%irefatm == 3) THEN
          refatm_in%bvref = zrbuf (4+ke1+1)
          pv_in(6+ke1+1)    = 0.0_ireals
          pv_in(6+ke1+2)    = 0.0_ireals
          pv_in(6+ke1+3)    = 0.0_ireals
          pv_in(6+ke1+4)    = refatm_in%bvref
          inrvert_in = 6+ke1+4
        ENDIF
    ...
    IF ((.NOT. lgot_refatm) .AND. (igrbednr == 2)) THEN
#ifdef GRIBAPI
      ...
      IF (izexch < nproc) THEN
        ...
        IF (izexch == my_cart_id) THEN
          CALL grib_get (igribid, 'localInformationNumber',    refatm_in%irefatm_id)
        ENDIF

        ! Distribute this number to all PEs
        IF (num_compute > 1) THEN
          CALL distribute_values (refatm_in%irefatm_id, 1, izexch, imp_integers, icomm_cart, ierr)
        ENDIF

        ! Check whether a default refatm set is available
        refatm_in%irefatm = 0
        search_refatm: DO n = 1, imax_refatmtype
          IF (refatm_in%irefatm_id == refatm_defaults(n)%irefatm_id) THEN
            refatm_in = refatm_defaults(n)
            EXIT search_refatm
          ENDIF
        ENDDO search_refatm

        IF (refatm_in%irefatm == 0) THEN
    ...
    IF (lzread_hhl) THEN
      ...
      IF (.NOT. lgot_vparam) THEN
        ...
        IF (my_cart_id == 0) THEN
          ! get localInformationNumber => vcoord_in%ivcoord_id
          CALL grib_get (igribid, 'localInformationNumber',    vcoord_in%ivcoord_id)
        ENDIF

        IF (num_compute > 1) THEN
          CALL distribute_values (vcoord_in%ivcoord_id, 1, 0, imp_integers, icomm_cart, ierr)
        ENDIF

        ! check, whether a default vertical coordinate set is available
        search_vcoord: DO n = 1, imax_vcoordtype
          IF ( (vcoord_in%ivcoord_id == vcoord_defaults(n)%ivcoord_id) .AND.   &
               (vcoord_in%nlevels    == vcoord_defaults(n)%nlevels)    .AND.   &
               (vcoord_in%ivctype    == vcoord_defaults(n)%ivctype) ) THEN
            IF     (vcoord_in%ivctype == 1) THEN
              vcoord_in%sigm_coord(1:vcoord_in%nlevels) = vcoord_defaults(n)%sigm_coord(1:vcoord_in%nlevels)
              vcoord_in%vcflat                          = vcoord_defaults(n)%vcflat
            ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
              vcoord_in%vert_coord(1:vcoord_in%nlevels) = vcoord_defaults(n)%vert_coord(1:vcoord_in%nlevels)
              vcoord_in%vcflat                          = vcoord_defaults(n)%vcflat
            ENDIF
            IF (my_cart_id == 0) THEN
              PRINT *, ' *** A default set for vertical coordinate parameters is used: ', &
                       vcoord_in%ivcoord_id
            ENDIF
            lgot_vparam = .TRUE.
            EXIT search_vcoord
          ENDIF
    ...
    IF ( (yzformat_tmp == 'grb1') .OR. (yzformat_tmp == 'apix') .OR. (yzformat_tmp == 'bina') ) THEN
      ...
      IF (igrbednr == 2) THEN
        IF ( (TRIM(var(izloc1,izloc2,izloc3)%name) == 'PP')   .OR.   &
          ...
          IF (refatm_in%irefatm_id /= izrefatm_id) THEN
...
!------------------------------------------------------------------------------
  ...
  IF (ydata == 'initial' .OR.                                &
    ...
    IF (vcoord_in%ivctype == 3) THEN
      ...
      ! split topo on PE 0
      IF (my_cart_id == 0) THEN
        CALL sleve_split_oro(zhsurf_tot, zhsurfs_tot, ie_tot, je_tot, nfltvc_in, &
                             0_iintegers, svc1_in, svc2_in, vcoord_in%vcflat,    &
    ...
    CASE (1)
      ...
      CALL reference_atmosphere                                                &
       ( hhl, p0, p0hl, rho0, t0, t0hl, dp0, hsurf, zhsurfs, ie, je, ke,       &
         refatm_in, vcoord_in, svc1_in, svc2_in, r_d, g, lanalyt_calc_t0p0,    &
         .NOT. lhhl_in_read)

    CASE (2)
      CALL reference_atmosphere_2                                              &
       ( hhl, p0, p0hl, rho0, t0, t0hl, dp0, hsurf, zhsurfs, ie, je, ke,       &
         refatm_in, vcoord_in, svc1_in, svc2_in, r_d, g, .NOT. lhhl_in_read)
    CASE (3)
...
!------------------------------------------------------------------------------
 ...
 ! Retrieve the required microphysics tracers
  ...
  ENDIF
#else
  CALL trcr_get(izerror, 'QC', ptr_tlev = nnew, ptr = qc, ptr_bd = qc_bd)
  IF (izerror /= 0) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QR', ptr_tlev = nnew, ptr = qr, ptr_bd = qr_bd)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QI', ptr_tlev = nnew, ptr = qi, ptr_bd = qi_bd)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QS', ptr_tlev = nnew, ptr = qs, ptr_bd = qs_bd)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QG', ptr_tlev = nnew, ptr = qg, ptr_bd = qg_bd)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QH', ptr_tlev = nnew, ptr = qh, ptr_bd = qh_bd)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNCLOUD', ptr_tlev = nnew, ptr = qnc, ptr_bd = qnc_bd)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNRAIN', ptr_tlev = nnew, ptr = qnr, ptr_bd = qnr_bd)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNICE', ptr_tlev = nnew, ptr = qni, ptr_bd = qni_bd)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNSNOW', ptr_tlev = nnew, ptr = qns, ptr_bd = qns_bd)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNGRAUPEL', ptr_tlev = nnew, ptr = qng, ptr_bd = qng_bd)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNHAIL', ptr_tlev = nnew, ptr = qnh, ptr_bd = qnh_bd)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNCLOUD', ptr_tlev = nnow, ptr = qnc_now)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNRAIN', ptr_tlev = nnow, ptr = qnr_now)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNICE', ptr_tlev = nnow, ptr = qni_now)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNSNOW', ptr_tlev = nnow, ptr = qns_now)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNGRAUPEL', ptr_tlev = nnow, ptr = qng_now)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNHAIL', ptr_tlev = nnow, ptr = qnh_now)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yerrmsg, yroutine)
  ENDIF
  ...
  IF (.NOT. ASSOCIATED(qc)) THEN
     CALL get_tracer(izerror, GPTRSTR, 'QV', pxt=qv)
     CALL tracer_halt(yroutine,izerror)

     CALL get_tracer(izerror, GPTRSTR, 'QC', pxt=qc)
     CALL tracer_halt(yroutine,izerror)

   !US habe ich neu eingebaut, passt das so?
     CALL get_tracer(izerror, GPTRSTR, 'QI', pxt=qi)
     IF (izerror /= TR_NEXIST) CALL tracer_halt(yroutine,izerror)

     CALL get_tracer(izerror, GPTRSTR, 'QR', pxt=qr)
     IF (izerror /= TR_NEXIST) CALL tracer_halt(yroutine,izerror)

     CALL get_tracer(izerror, GPTRSTR, 'QS', pxt=qs)
     IF (izerror /= TR_NEXIST) CALL tracer_halt(yroutine,izerror)

     CALL get_tracer(izerror, GPTRSTR, 'QG', pxt=qg)
     IF (izerror /= TR_NEXIST) CALL tracer_halt(yroutine,izerror)
  ENDIF
#endif
...
#ifdef MESSY
!US: the calculations of the reference atmosphere have been moved to the block above
!    in the last version: what does this mean for MESSY?
ENDIF
#endif 
...
!------------------------------------------------------------------------------
  ...
  IF (lrequired) THEN
    IF (ydata=='initial') THEN
      ...
      ! Check additional element number ('iaee') of some special fields for
      ! soil moisture analysis. If it does not meet
      ! the required one the input record is discarded

      IF ( (yshortname(1:LEN_TRIM(yshortname)) == 'W_G1') .OR.                    &
           (yshortname(1:LEN_TRIM(yshortname)) == 'W_G2') .OR.                    &
           (yshortname(1:LEN_TRIM(yshortname)) == 'W_G3') .OR.                    &
           (yshortname(1:LEN_TRIM(yshortname)) == 'W_SO') ) THEN
...
REAL     (KIND=ireals)    :: rfact, rdepth1, rdepth2
...
!US          ! The levels are coded in cm (integer values) in 'bottomlevel' (ipds(10))
...
SUBROUTINE get_vertcoord (inv, pv, igrbednr)
...
INTEGER (KIND=iintegers), INTENT(IN) :: inv, igrbednr
...
!==============================================================================
  ...
  IF ((idummy >= 1) .AND. (idummy <= 200)) THEN
    ...
    IF (vcoord_in%ivctype == 3) THEN
      ...
      svc1_in = pv(6 + ke1 + 1)
      svc2_in = pv(6 + ke1 + 2)
      nfltvc_in   = NINT (pv(6 + ke1 + 3), iintegers)
    ELSE
      ! just to have an initialization
      svc1_in = 0.0_ireals
      svc2_in = 0.0_ireals
      nfltvc_in = 0_iintegers
    ENDIF

    IF (refatm_in%irefatm == 2) THEN
      ! read additional parameters for new reference atmosphere
      refatm_in%delta_t = pv(6 + ke1 + 4)
      refatm_in%h_scal  = pv(6 + ke1 + 5)
    ENDIF
  ...
  ELSE
    ...
    CASE (0, 1)
      ...
      refatm_in%irefatm = 1   ! for irefatm > 1, more coordinate parameters are coded
      IF     (pv(5) < pv(6)) THEN
    ...
    CASE (4)
      refatm_in%irefatm = 1   ! for irefatm > 1, more coordinate parameters are coded
      vcoord_in%ivctype = 3   ! Sleve coordinate

      ! read three more SLEVE parameters
      svc1_in   = pv(4 + ke1 + 2)
      svc2_in   = pv(4 + ke1 + 3)
      nfltvc_in = NINT (pv(4 + ke1 + 4), iintegers)

    CASE (5)
      refatm_in%irefatm = 3   ! for irefatm > 1, more coordinate parameters are coded
      vcoord_in%ivctype = NINT (pv(4 + ke1 + 1), iintegers)

    CASE (6)
      refatm_in%irefatm = 2   ! for irefatm > 1, more coordinate parameters are coded
      vcoord_in%ivctype = NINT (pv(4 + ke1 + 1), iintegers)
    ...
    IF (vcoord_in%ivctype == 3) THEN
      ...
      IF ((svc1_in > vcoord_in%vert_coord(1)) .OR. (svc1_in < 0.0)) THEN
         yerrmsg = ' ERROR *** svc1 not in allowed range for ivctype = 3 ***'
         ierrstat = 2008
         CALL model_abort (my_cart_id, ierrstat, yerrmsg, yroutine)
      ENDIF

      IF ((svc2_in > vcoord_in%vert_coord(1)) .OR. (svc2_in < 0.0)) THEN
         yerrmsg = ' ERROR *** svc2 not in allowed range for ivctype = 3 ***'
         ierrstat = 2008
         CALL model_abort (my_cart_id, ierrstat, yerrmsg, yroutine)
      ENDIF
    ...
    IF (refatm_in%irefatm == 2) THEN
      ! read additional parameters for new reference atmosphere
      refatm_in%delta_t = pv(4 + ke1 + 5)
      refatm_in%h_scal  = pv(4 + ke1 + 6)
    ENDIF
...
!  Local arrays:
  ...
  REAL (KIND=irealgrib), ALLOCATABLE   :: &
    zvcoord_in(:), & ! vertical coordinate
...
! Get the dimension ID's and the length of the dimensions
  ...
  ALLOCATE (longitude(iedim_in), latitude(jedim_in), vcoordgrib(ke1dim_in), &
            zvcoord_in(ke1dim_in), STAT=istatus)
  IF (istatus /= 0) THEN    
...
! Get the vertical co-ordinate
  ...
  vcoord_in%nlevels           = ke1dim_in
  IF     (vcoord_in%ivctype == 1) THEN
    vcoord_in%sigm_coord(1:ke1dim_in) = zvcoord_in(1:ke1dim_in)
  ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
    vcoord_in%vert_coord(1:ke1dim_in) = zvcoord_in(1:ke1dim_in)
  ENDIF
...
!_br 25.11.08
  ...
  ELSEIF (vcoord_in%ivctype == 3) THEN
...
! send information to the other processors
  IF (npes > 1) THEN
    CALL MPI_BCAST (vcoord_in%ivctype,  1, imp_integers, 0, icomm, izmplcode)
    CALL MPI_BCAST (refatm_in%irefatm,  1, imp_integers, 0, icomm, izmplcode) !_br 25.11.08
    CALL MPI_BCAST (refatm_in%p0sl,     1, imp_reals,    0, icomm, izmplcode)
    CALL MPI_BCAST (refatm_in%t0sl,     1, imp_reals,    0, icomm, izmplcode)
    CALL MPI_BCAST (refatm_in%dt0lp,    1, imp_reals,    0, icomm, izmplcode)
    CALL MPI_BCAST (vcoord_in%vcflat,   1, imp_reals,    0, icomm, izmplcode)
!_br 05.12.08
    IF (refatm_in%irefatm == 2) THEN
      CALL MPI_BCAST (refatm_in%delta_t,  1, imp_reals,    0, icomm, izmplcode)
      CALL MPI_BCAST (refatm_in%h_scal,   1, imp_reals,    0, icomm, izmplcode)
    ENDIF
!_br 05.12.08 end

    CALL MPI_BCAST (zvcoord_in, ke1, imp_reals,    0, icomm, izmplcode)
    vcoord_in%nlevels           = ke1
    IF     (vcoord_in%ivctype == 1) THEN
      vcoord_in%sigm_coord(1:ke1) = zvcoord_in(1:ke1)
    ELSEIF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
...
!_br 25.11.08
    IF (vcoord_in%ivctype == 3) THEN  ! SLEVE coordinate
