...
TYPE vcoord_type
  ...
  REAL (KIND=ireals)           :: vcflat             ! coordinate where levels become flat
  REAL (KIND=ireals)           :: vert_coord(khmax)  ! height above sea level
  REAL (KIND=ireals)           :: sigm_coord(khmax)  ! reference pressure normalized to [0,1]
END TYPE vcoord_type

TYPE (vcoord_type), ALLOCATABLE  :: vcoord_defaults(:)
TYPE (vcoord_type)               :: vcoord_in, vcoord_out

! data type for defaults of reference atmosphere settings
TYPE refatm_type
  INTEGER(KIND=iintegers)      :: irefatm
  INTEGER(KIND=iintegers)      :: irefatm_id
  REAL (KIND=ireals)           :: p0sl
  REAL (KIND=ireals)           :: t0sl
  REAL (KIND=ireals)           :: dt0lp
  REAL (KIND=ireals)           :: delta_t
  REAL (KIND=ireals)           :: h_scal
  REAL (KIND=ireals)           :: bvref
END TYPE refatm_type

TYPE (refatm_type), ALLOCATABLE  :: refatm_defaults(:)
TYPE (refatm_type)               :: refatm_in, refatm_out
...
! some initializations for vcoord_in, vcoord_out
vcoord_in%ivcoord_id = -1
vcoord_out%ivcoord_id = -1
