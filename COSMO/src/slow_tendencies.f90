...
USE messy_main_tracer,        ONLY: R_molarmass, I_VDIFF, ON &
                                  , get_tracer
...
!-----------------
  ...
  REAL (KIND=ireals)              :: surffac = 0._ireals
  REAL (KIND=ireals)              :: zd1gtrac
  REAL (KIND=ireals), ALLOCATABLE, DIMENSION(:,:,:,:) :: zd1trac
  REAL (KIND=ireals)              :: zxtcor(ie,je) 
#endif
...
!------------------------------------------------------------------------------
  ...
  qv_old => xt_array(nold)%ptr(:,:,idx_qv,:)
#endif
...
!-------------------------------------------------------------------------------
! Section 2b: Setup of tridiagonal matrix systems resulting from the implicit
!             numerical formulation of advection and diffusion of MESSY tracers.
!-------------------------------------------------------------------------------

#ifdef MESSY
  ALLOCATE(zd1trac(ie,je,ntrac_gp,ke))
  zd1trac(:,:,:,:) = 0._ireals
  ! TOP LAYER
   DO j = jstart, jend
    DO i = istart, iend
      ...
      DO jt = 1, ntrac_gp
         IF (ti_gp(jt)%tp%meta%cask_i(I_VDIFF) == ON) THEN
            zd1trac(i,j,jt,1) =ed2dt *xt_array(nold)%PTR(i,j,jt,1) &
                 + xtte(i,j,jt,1) &
                 - zcs *(xt_array(nold)%PTR(i,j,jt,2) &
                 -xt_array(nold)%PTR(i,j,jt,1)) 
            zd1trac(i,j,jt,1)  = zd1trac(i,j,jt,1)/zbg
         ELSE
            zd1trac(i,j,jt,1) = xt_array(nold)%PTR(i,j,jt,1) &
                 + xtte(i,j,jt,1) / ed2dt
         ENDIF
      ENDDO
    ENDDO
   ...
   DO k = 2, ke-1
      DO j = jstart, jend
         DO i = istart, iend
            ...
            DO jt = 1,ntrac_gp
               IF (ti_gp(jt)%tp%meta%cask_i(I_VDIFF) == ON) THEN
                  zd1gtrac   = ed2dt * xt_array(nold)%PTR(i,j,jt,k) &
                       + xtte(i,j,jt,k)      &
                       - zas * (xt_array(nold)%PTR(i,j,jt,k-1)    &
                                 - xt_array(nold)%PTR(i,j,jt,k))  &
                       - zcs * (xt_array(nold)%PTR(i,j,jt,k+1)    &
                                 - xt_array(nold)%PTR(i,j,jt,k))
                  zd1trac(i,j,jt,k) = ( zd1gtrac -zag*zd1trac(i,j,jt,k-1) )*zz
               ELSE
                  zd1trac(i,j,jt,k) = xt_array(nold)%PTR(i,j,jt,k) &
                       + xtte(i,j,jt,k) / ed2dt
               ENDIF
           ENDDO
         ENDDO
   ...
   DO j = jstart, jend
      DO i = istart, iend
         zgat       = - zkh (i,j,ke)*zeddpq(i,j,ke)
         zgct       = - ztch(i,j   )*zeddpq(i,j,ke) ! ak in ART differently
         zag        = zgavx(i,j,ke)*zbetp + zgat*za1t(ke)
         zas        = zgavx(i,j,ke)*zbetm + zgat*za2t(ke)
         zcg        = zgct*za1t_surf
         zcs        = zgct*za2t_surf
         ...
         DO jt = 1,ntrac_gp
            IF (ti_gp(jt)%tp%meta%cask_i(I_VDIFF) == ON) THEN
               zd1gtrac   = ed2dt * xt_array(nold)%PTR(i,j,jt,ke) &
                    + xtte(i,j,jt,ke)       &
                    - zas * (xt_array(nold)%PTR(i,j,jt,ke-1) &
                    - xt_array(nold)%PTR(i,j,jt,ke))  
               IF (idx_qv == jt) &
                   zd1gtrac = zd1gtrac - zgct * qv_s(i,j,nold)
                ! um_ak_20100810 do we need this term? it may be an artifical
                ! source or sink term as in src_slow_tendencies_rk
                !    + zcs * xt_array(nold)%PTR(i,j,jt,ke) 
               zz         = 1.0_ireals / ( zbg  - zag*zc (i,j,ke-1) )
               xt_array(nnew)%PTR(i,j,jt,ke) = &
                    ( zd1gtrac - zag*zd1trac(i,j,jt,ke-1) ) * zz
            ELSE
               xt_array(nnew)%PTR(i,j,jt,ke) = xt_array(nold)%PTR(i,j,jt,ke) &
                    + xtte(i,j,jt,ke) * dt2
            ENDIF
            ! um_ak_20080815+
            !      xtte(i,j,jt,ke) = (xt(i,j,jt,ke)-xtm1(i,j,jt,ke))*ed2dt
            ! um_ak_20080815-
         ENDDO
      ENDDO
   ENDDO
   ...
   DO k = ke-1, 1, -1
      DO j =jstart, jend
         DO i = istart, iend
            DO jt = 1, ntrac_gp
               IF (ti_gp(jt)%tp%meta%cask_i(I_VDIFF) == ON) THEN
                  xt_array(nnew)%PTR(i,j,jt,k) = zd1trac(i,j,jt,k) &
                                - zc(i,j,k)*xt_array(nnew)%PTR(i,j,jt,k+1)
               ELSE
                  xt_array(nnew)%PTR(i,j,jt,k) = zd1trac(i,j,jt,k) 
               ENDIF
            ENDDO
