...
#ifdef MESSY
USE messy_main_tracer_mem_bi, ONLY: xt_array, xtte, ti_gp, ntrac_gp, GPTRSTR
USE messy_main_tracer_bi,     ONLY: tracer_halt
USE messy_main_tracer,        ONLY: get_tracer, I_HDIFF, ON, TR_NEXIST
#endif
...
!------------------------------------------------------------------------------
  ...
  IF ( ntstep == nstart ) THEN
    ...
    IF (my_cart_id == 0) THEN
      PRINT *,' LEVELINDEX WHERE LEVELS BECOME FLAT, KFLAT = ', vcoord_in%kflat
    ...
    IF ( itheta_adv == 0 ) THEN
      ...
      IF (refatm_in%irefatm == 1) THEN
        DO  k = 1, ke
          dt0dz(:,:,k) = - refatm_in%dt0lp * g * rho0(:,:,k) / p0(:,:,k)
        ENDDO
      ELSE IF ( refatm_in%irefatm == 2 ) THEN
        DO  k = 1, ke
          dt0dz(:,:,k) =  -refatm_in%delta_t/refatm_in%h_scal*EXP(-0.5_ireals*(hhl(:,:,k)+ &
                           hhl(:,:,k+1))/refatm_in%h_scal)
        ENDDO
      ELSE IF ( refatm_in%irefatm == 3 ) THEN
        DO  k = 1, ke
          dt0dz(:,:,k) = t0(:,:,k)*refatm_in%bvref**2/g - govcp
        ENDDO
      END IF
    ...
    ELSE IF ( itheta_adv >= 1 ) THEN
      ...
      IF ( refatm_in%irefatm == 1 ) THEN
        DO  k = 1, ke
          ...
          dt0dz(:,:,k) = (govcp - refatm_in%dt0lp * g * rho0(:,:,k) / p0(:,:,k))* &
            EXP(-rovcp*LOG(p0(:,:,k)*rp00))
        ENDDO
      ELSE IF ( refatm_in%irefatm == 2 ) THEN
        DO  k = 1, ke
          t0(:,:,k)    = p0(:,:,k) / ( r_d * rho0(:,:,k) )* &
            EXP(-rovcp*LOG(p0(:,:,k)*rp00))
          dt0dz(:,:,k) =  (govcp - refatm_in%delta_t/refatm_in%h_scal*EXP(-0.5_ireals*(hhl(:,:,k)+ &
            hhl(:,:,k+1))/refatm_in%h_scal)) * EXP(-rovcp*LOG(p0(:,:,k)*rp00))
        ENDDO
      ELSE IF ( refatm_in%irefatm == 3 ) THEN
        DO  k = 1, ke
          t0(:,:,k)    = p0(:,:,k) / ( r_d * rho0(:,:,k) )* &
            EXP(-rovcp*LOG(p0(:,:,k)*rp00))
          dt0dz(:,:,k) = t0(:,:,k)*refatm_in%bvref**2/g
        ENDDO
      ENDIF
  ...
  CALL trcr_get(izerror, 'QV', ptr_tlev = nnow, ptr = qv_nnow)
  CALL trcr_get(izerror, 'QC', ptr_tlev = nnow, ptr = qc_nnow)
  CALL trcr_get(izerror, 'QI', ptr_tlev = nnow, ptr = qi_nnow)
  CALL trcr_get(izerror, 'QR', ptr_tlev = nnow, ptr = qr_nnow)
  CALL trcr_get(izerror, 'QS', ptr_tlev = nnow, ptr = qs_nnow)
  CALL trcr_get(izerror, 'QG', ptr_tlev = nnow, ptr = qg_nnow)
  CALL trcr_get(izerror, 'QH', ptr_tlev = nnow, ptr = qh_nnow)

#ifdef TWOMOM_SB
  CALL trcr_get(izerror, 'QH', ptr_tlev = nnew, ptr = qh)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  ...
  runge_kutta_loop: DO irk = 1, irk_order
    ...
    ELSE IF ( itype_fast_waves == 1 ) THEN
      ...
      CALL fast_waves_runge_kutta( ismts, dts, xkd,                          &
        ...
        zubdt_west*dts, zubdt_east*dts, zvbdt_north*dts, zvbdt_south*dts,    &
        irk, alpha_rk, beta_rk, vcoord_in%kflat )
...
!CDIR COLLAPSE
  ...
  IF ( lhordiff ) THEN
    ...
    IF ( hd_corr_trcr_bd /= 0.0_ireals .OR. hd_corr_trcr_in /= 0.0_ireals ) THEN
...
#ifdef MESSY 
    DO jt=1, ntrac_gp
       IF (ti_gp(jt)%tp%meta%cask_i(I_HDIFF) == ON) THEN
