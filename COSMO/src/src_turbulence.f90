...
#ifdef MESSY
  INTEGER(KIND=IINTEGERS), SAVE :: idx_qv, idx_qc
#endif
    ...
    DO k = 2, ke
      DO j = jstart, jend
        DO i = istart, iend
          ...
          ztpm(i,j,k) = ( zsgesh(i,j,k) + zsgesv(i,j,k) )**2
          ztpm_v(i,j,k) = zsgesv(i,j,k)**2
      ...
      DO k = 2, ke
        ...
        DO j = jstart, jend
          DO i = istart, iend
            ...
            zmzb = hhl(i,j,k) - hhl(i,j,ke1)
            zlen = akt*zmzb / ( 1.0_ireals + akt*zmzb/zdze )
            zlenq = zlen*zlen
            ...
            ELSE
              ...
              zphim   = zceps1d3 * zsm
              ztkvmom = zphim * zlen * SQRT( tke(i,j,k,ntke) )
          ...
          DO i = istart, iend
            ...
            ELSE
              ...
              IF ( .NOT.lprog_tke .OR. ntstep == 0) THEN
                ...
                tke(i,j,k,ntke) = MAX( 0.0_ireals, tke(i,j,k,ntke) )
              ENDIF
              tkvm(i,j,k) = zphim * zlen * SQRT( tke(i,j,k,ntke) )
    ...
    DO k = 2, ke
      DO j = jstart, jend
        DO i = istart, iend
          ...
          zdiss  = - zceps / zlen * tke(i,j,k,ntke)**1.5_ireals
