...
USE vgrid_refatm_utils,       ONLY :  vcoord_in
...
#ifdef MESSY
USE messy_main_tracer_mem_bi,   ONLY: xt_array, ntrac_gp, xt_bd, ti_gp, GPTRSTR&
                                    , xtm1
USE messy_main_tracer_bi,       ONLY: tracer_halt
USE messy_main_tracer,          ONLY: get_tracer, ON, I_DAMP, I_ADVECT      &
                                    , I_RELAX, T_RELAX_FULL, T_RELAX_INFLOW &
                                    , I_LBC, T_LBC_ZERO, T_LBC_FILE, TR_NEXIST
#endif
...
!------------------------------------------------------------------------------
  ...
  IF (lspubc .EQV. .TRUE.) THEN
    ...
    ! height vcoord_in%vert_coord (refering to z=0) is used, i.e. it is assumed that the
    ...
    ! The damping coefficients are defined on full model levels.
  
    DO k = 1, ke
      zlev = 0.5_ireals*(vcoord_in%vert_coord(k)+vcoord_in%vert_coord(k+1))
      IF( zlev >= rdheight ) THEN
        rdcoef(k) = 1.0_ireals - COS( pi*(zlev-rdheight)/(vcoord_in%vert_coord(1)-rdheight) )
        rdcoef(k) = rdcoef(k) / ( 2.0_ireals*nrdtau*dt )
        ke_rd = k
      ENDIF
    ENDDO
    ...
    ! The multiplication with dt therefore is done in routine sardass
    ! during the time stepping
    IF (.NOT. l2tls) THEN
      rdcoef(:) = 2.0_ireals * rdcoef(:)
    ENDIF
...
SUBROUTINE sardass (dt_alter)

!------------------------------------------------------------------------------
!
! Description:
!   This module procedure of module "Dynamics" computes the impact of 
!   condensation and evaporation of cloud water on the prognostic variables
!   t, qv and qc at time level nnew.
...
!   Finally, the Asselin time filter is applied on all prognostic variables
!   at time level nnow.
!
! Method:
...
!   fpvsw, fpvsi, fqvs,& ! name of statement functions
    ...
    zdtr,  zwbe, zdtrdcoef, zdt, zrelfac_u, zrelfac_p, zrelfac_s
...
! -------------------------
  ...
  LOGICAL                   :: &
    izdamp_forced(trcr_get_ntrcr()) ! forced damping
#endif
...
! ---------------------------
  ...
  INTEGER (KIND=iintegers) :: n_xt(ntrac_gp)
  REAL(KIND=ireals)        :: ztracbd(ie,je,ntrac_gp,ke)
#endif
...
! boundary values:
  ...
  zdtird = REAL(ntstep + 1 - nlastbound, ireals) / REAL(nincbound, ireals)
  zemzdr = 1. - zdtird
...
! frequency of meteorology is not necessarily equal to chemistry update freq.
  IF (l_cosmo_art) THEN
    IF (lgasbd .OR. laerobd) THEN
      zdtird_art = REAL(ntstep + 1 - nlastbound_art, ireals) / REAL(nincbound_art, ireals)
      zemzdr_art = 1. - zdtird_art
...
! Reduction coefficients for lateral boundary relaxation if lateral radiation condition is used
  ...
  IF (lradlbc) THEN
    zrelfac_u = MIN(1._ireals,2._ireals*relax_fac)
    zrelfac_s = MIN(1._ireals,relax_fac)
    zrelfac_p = 0._ireals
  ELSE
    zrelfac_u = 1._ireals
    zrelfac_s = 1._ireals
...
! Coefficients for implicit calculation of lateral boundary relaxation scheme
  ...
  ELSE
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        zmys(i,j) = zrelfac_s*rmy(i,j,1) / (1.0 + rmy(i,j,1))
        zmyp(i,j) = zrelfac_p*rmy(i,j,1) / (1.0 + rmy(i,j,1))
        zmyu(i,j) = zrelfac_u*rmy(i,j,2) / (1.0 + rmy(i,j,2))
        zmyv(i,j) = zrelfac_u*rmy(i,j,3) / (1.0 + rmy(i,j,3))
      ENDDO
    ENDDO
  ENDIF
#ifdef NECSX
  ...
  DO k = 1,ke
    ...
    ztaur(k)  = zdtrdcoef / (1.0 + zdtrdcoef)
  ENDDO
  ...
  IF (.NOT. l2tls) THEN
    ...
    ENDIF
  ENDIF
  CALL trcr_meta_get(izerror, 'DAMP_FORCED', izdamp_forced)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
...
!------------------------------------------------------------------------------
  ...
  IF ( izcount > 0_iintegers ) THEN
    ...
    DO k = 1, ke
      zu_west(1:je)  = 0.0_ireals
      zv_south(1:ie) = 0.0_ireals
      zu_east(1:je)  = 0.0_ireals
      zv_north(1:ie) = 0.0_ireals
      ...
      DO j = jstartpar, jendpar
        DO i = istartpar, iendpar
          ...
          IF ( .NOT.( lwest_lbdz(i,j) .OR. lsouth_lbdz(i,j) .OR.  &
            ...
            zmyq(i,j,k) = 0.0_ireals
          ! southwest corner
          ELSE IF ( lsouth_lbdz(i,j) .AND. lwest_lbdz(i,j) .AND.  &
            zv_south(i) < 0.0_ireals .AND. zu_west(j) < 0.0_ireals ) THEN
            zmyq(i,j,k) = 0.0_ireals
          ! southeast corner
          ELSE IF ( lsouth_lbdz(i,j) .AND. least_lbdz(i,j) .AND.  &
            zv_south(i) < 0.0_ireals .AND. zu_east(j) > 0.0_ireals ) THEN
            zmyq(i,j,k) = 0.0_ireals
          ! northwest corner
          ELSE IF ( lnorth_lbdz(i,j) .AND. lwest_lbdz(i,j) .AND.  &
            zv_north(i) > 0.0_ireals .AND. zu_west(j) < 0.0_ireals ) THEN
            zmyq(i,j,k) = 0.0_ireals
          ! northeast corner
          ELSE IF ( lnorth_lbdz(i,j) .AND. least_lbdz(i,j) .AND.  &
            zv_north(i) > 0.0_ireals .AND. zu_east(j) > 0.0_ireals ) THEN
            zmyq(i,j,k) = 0.0_ireals
          ! western boundary
          ELSE IF ( lwest_lbdz(i,j)  .AND. zu_west(j)  < 0.0_ireals ) THEN
            zmyq(i,j,k) = 0.0_ireals
          ! southern boundary
          ELSE IF ( lsouth_lbdz(i,j) .AND. zv_south(i) < 0.0_ireals ) THEN
            zmyq(i,j,k) = 0.0_ireals
          ! eastern boundary
          ELSE IF ( least_lbdz(i,j)  .AND. zu_east(j)  > 0.0_ireals ) THEN
            zmyq(i,j,k) = 0.0_ireals
          ! northern boundary
          ELSE IF ( lnorth_lbdz(i,j) .AND. zv_north(i) > 0.0_ireals ) THEN
            zmyq(i,j,k) = 0.0_ireals
          ELSE
            ...
            IF ( lexpl_lbc ) THEN
              zmyq(i,j,k) = rmyq(i,j)
            ELSE
              zmyq(i,j,k) = rmyq(i,j) / (1.0 + rmyq(i,j))
            ENDIF
          ENDIF
        ENDDO
      ENDDO
...
!             structure.
  ...
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    ...
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  IF ( izqi /= T_MISSING ) THEN
    CALL trcr_meta_get(izerror, izqi, T_LBC_ID, izlbcqi)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    IF ( izlbcqi /= T_LBC_FILE ) THEN
      CALL trcr_meta_get(izerror, izqi, T_RELAX_ID, izrelaxqi)
      IF (izerror /= 0) THEN
        yzerrmsg = trcr_errorstr(izerror)
        CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
      ENDIF
      IF ( izrelaxqi == T_RELAX_FULL ) THEN
        ...
        DO k = 1, ke
          DO j = jstartpar, jendpar
            DO i = istartpar, iendpar
              IF (t_bd(i,j,k,nbd2) /= undef) THEN
                ztrbd(i,j,k,izqi) = 0.0_ireals
                IF ( ztbd(i,j,k) < 248.15_ireals ) THEN
                  ztrbd(i,j,k,izqi) = ztrbd(i,j,k,izqc)
                  ztrbd(i,j,k,izqc) = 0.0_ireals
                ENDIF
              ENDIF
            ENDDO
          ENDDO
      ...
      ! diameter given in the below SR set_qni_from_qi_sb_scalar().
        CALL trcr_get_index(izerror, 'QNICE', izqc)
        IF (izerror /= 0) THEN
          yzerrmsg = trcr_errorstr(izerror)
          CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
        ENDIF
        DO k = 1, ke
          DO j = jstartpar, jendpar
            DO i = istartpar, iendpar
              ztrbd(i,j,k,izqc) = set_qni_from_qi_sb_scalar(ztrbd(i,j,k,izqi))
            ENDDO
          ENDDO
        ENDDO
#endif
...
#ifdef MESSY
! Subsection 4.1b : lateral boundary relaxation (MESSy)
!----------------------------------------------------
#ifdef OLD
  DO jt = 1, ntrac_gp
      IF (ti_gp(jt)%tp%meta%cask_i(I_RELAX) == ON) THEN
         WHERE(xt_bd(istartpar:iendpar,jstartpar:jendpar,jt,1:ke,nbd1) >= 0._ireals)
            ztracbd(istartpar:iendpar,jstartpar:jendpar,jt,1:ke) = &
              zemzdr*xt_bd(istartpar:iendpar,jstartpar:jendpar,jt,1:ke,nbd1) + &
              zdtird*xt_bd(istartpar:iendpar,jstartpar:jendpar,jt,1:ke,nbd2)
         ELSEWHERE
            ztracbd(istartpar:iendpar,jstartpar:jendpar,jt,1:ke) = &
                 xt_array(nnew)%PTR(istartpar:iendpar,jstartpar:jendpar,jt,1:ke)
         END WHERE
          DO j = jstartpar, jendpar
             DO i = istartpar, iendpar
                xt_array(nnew)%PTR(i,j,jt,k) = xt_array(nnew)%PTR(i,j,jt,k) &
                     - zmys(i,j)*(xt_array(nnew)%PTR(i,j,jt,k)-ztracbd(i,j,jt))
             ENDDO
          ENDDO
      ELSE 
         ztracbd(istartpar:iendpar,jstartpar:jendpar,jt,1:ke) = &
...
#else
   DO jt = 1, ntrac_gp
      IF (ti_gp(jt)%tp%meta%cask_i(I_LBC) == T_LBC_ZERO) THEN
         ztracbd(istartpar:iendpar,jstartpar:jendpar,jt,1:ke) = 0._ireals
      ELSE IF (ti_gp(jt)%tp%meta%cask_i(I_RELAX) >= ON) THEN
         WHERE(xt_bd(istartpar:iendpar,jstartpar:jendpar,jt,1:ke,nbd1) >= 0._ireals)
            ztracbd(istartpar:iendpar,jstartpar:jendpar,jt,1:ke) = &
               zemzdr*xt_bd(istartpar:iendpar,jstartpar:jendpar,jt,1:ke,nbd1) +&
                zdtird*xt_bd(istartpar:iendpar,jstartpar:jendpar,jt,1:ke,nbd2)
      ...
      DO j = jstartpar, jendpar
         DO i = istartpar, iendpar
            xt_array(nnew)%PTR(i,j,jt,1:ke) = xt_array(nnew)%PTR(i,j,jt,1:ke) &
               - zmys(i,j)*(xt_array(nnew)%PTR(i,j,jt,1:ke)-ztracbd(i,j,jt,1:ke))
         ENDDO
      ENDDO
   END DO ! tracer loop

   IF (idx_qi < 0 .OR. idx_qc < 0) THEN
   ...
   ENDIF
   IF (idx_qi > 0) THEN
      IF (ti_gp(idx_qi)%tp%meta%cask_i(I_LBC) /= T_LBC_FILE .AND. &
           ti_gp(idx_qi)%tp%meta%cask_i(I_RELAX) == T_RELAX_FULL) THEN 
         DO k = 1, ke
            DO j = jstartpar, jendpar
               DO i = istartpar, iendpar
                  IF (t_bd(i,j,k,nbd2) /= undef) THEN
                     ztracbd(i,j,idx_qi,k) = 0.0_ireals
                     IF ( ztbd(i,j,k) < 248.15_ireals ) THEN
                        ztracbd(i,j,idx_qi,k) = ztracbd(i,j,idx_qc,k)
                        ztracbd(i,j,idx_qc,k) = 0.0_ireals
                     ENDIF
                  ENDIF
               ENDDO
            ENDDO
   ...
   DO jt = 1, ntrac_gp
      IF (ti_gp(jt)%tp%meta%cask_i(I_RELAX) == T_RELAX_FULL) THEN
         DO k = 1,ke
            DO j = jstartpar, jendpar
               DO i = istartpar, iendpar
                  xt_array(nnew)%PTR(i,j,jt,k) = &
                       xt_array(nnew)%PTR(i,j,jt,k) - zmys(i,j)* &
                       (xt_array(nnew)%PTR(i,j,jt,k)-ztracbd(i,j,jt,k))
               ENDDO
            ENDDO
         END DO
      ELSE IF (ti_gp(jt)%tp%meta%cask_i(I_RELAX) == T_RELAX_INFLOW) THEN
         DO k= 1,ke
            DO j = jstartpar, jendpar
               DO i = istartpar, iendpar
                  xt_array(nnew)%PTR(i,j,jt,k) = &
                       xt_array(nnew)%PTR(i,j,jt,k) - zmyq(i,j,k)* &
                       (xt_array(nnew)%PTR(i,j,jt,k)-ztracbd(i,j,jt,k))
               ENDDO
            ENDDO
         ENDDO
...
! Subsection 4.2b : Rayleigh damping (MESSy)
!----------------------------------------
   DO jt = 1,ntrac_gp
      IF (ti_gp(jt)%tp%meta%cask_i(I_DAMP) == ON .AND. &
           lspubc .AND. itype_spubc == 1) THEN
            DO k = 1 , ke_rd
               DO j = jstartpar, jendpar
                  DO i = istartpar, iendpar
                     xt_array(nnew)%PTR(i,j,jt,k) = &
                          xt_array(nnew)%PTR(i,j,jt,k) - ztaur(k) &
                          *(xt_array(nnew)%PTR(i,j,jt,k)-ztracbd(i,j,jt,k))
                  ENDDO
               ENDDO
            ENDDO
...
!------------------------------------------------------------------------------
  ...
  IF (lspubc .AND. (itype_spubc == 2)) THEN 
...
#ifdef MESSY
      DO jt = 1, ntrac_gp
         IF (ti_gp(jt)%tp%meta%cask_i(I_DAMP) == ON) &
              field_tmp(:,:, k + (n_xt(jt)-1)*ke_rd) =  &
              xt_array(nnew)%ptr(:,:,jt,k)
      END DO
#endif
    ENDDO
    ...
    DO iztrcr = 1, trcr_get_ntrcr()
      IF ( izdamp(iztrcr) == T_DAMP_ON .OR. izdamp_forced(iztrcr) ) THEN
        num_var = num_var + 1
      ENDIF
    ENDDO
...
#ifdef MESSY
    DO jt = 1, ntrac_gp
       IF (ti_gp(jt)%tp%meta%cask_i(I_DAMP) == ON)  THEN
          DO  k = 1, ke_rd
             DO j = jstartpar, jendpar 
                DO i = istartpar, iendpar
                   xt_array(nnew)%ptr(i,j,jt,k) =                 &
                        xt_array(nnew)%ptr(i,j,jt,k)              &
                        - ztaur(k)*(xt_array(nnew)%ptr(i,j,jt,k)- &
                        field_tmp(i,j, k + (n_xt(jt)-1)*ke_rd))
...
!------------------------------------------------------------------------------
  ...
  IF (llhn .OR. llhnverif) THEN
    ...
    ! saturation adjustment. Relaxation runs after latent heat nudging.
    ! Entire latent heat rate has to be restored on the former timesteps
    DO k = 1, ke
  ...
  IF (lcond) THEN
   IF (itype_gscp < 100 .OR. l2mom_satads) THEN
    ...
    IF (izerror /= 0) THEN
      ...
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get( izerror, 'QC', ptr_tlev=nnew, ptr=qc_new )
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
#else
   IF (.NOT. ASSOCIATED(qc_new)) THEN
      CALL get_tracer(izerror, GPTRSTR, 'QV', pxt=qv_new)
      CALL tracer_halt(yzroutine, izerror)
      CALL get_tracer(izerror, GPTRSTR, 'QC', pxt=qc_new)
      CALL tracer_halt(yzroutine, izerror)
   ENDIF
#endif
...
IF ( .NOT. l2tls ) THEN
  IF (izdebug > 10) THEN
    PRINT *, '        Asselin time filtering'
  ENDIF

  DO  k = 1, ke
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        zdd = epsass*( u(i,j,k  ,nnew) - 2.0_ireals*u(i,j,k  ,nnow) + u(i,j,k  ,nold) )
        u(i,j,k,nnow)   =  u(i,j,k  ,nnow) + alphaass * zdd
        u(i,j,k,nnew)   =  u(i,j,k  ,nnew) + (alphaass - 1.0_ireals) * zdd
        zdd = epsass*( v(i,j,k  ,nnew) - 2.0_ireals*v(i,j,k  ,nnow) + v(i,j,k  ,nold) )
        v(i,j,k,nnow)   =  v(i,j,k  ,nnow) + alphaass * zdd
        v(i,j,k,nnew)   =  v(i,j,k  ,nnew) + (alphaass - 1.0_ireals) * zdd
        zdd = epsass*( w(i,j,k+1,nnew) - 2.0_ireals*w(i,j,k+1,nnow) + w(i,j,k+1,nold) )
        w(i,j,k+1,nnow) =  w(i,j,k+1,nnow) + alphaass * zdd
        w(i,j,k+1,nnew) =  w(i,j,k+1,nnew) + (alphaass - 1.0_ireals) * zdd
      ENDDO
    ENDDO

    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        zdd = epsass*( pp(i,j,k,nnew) - 2.0_ireals*pp(i,j,k,nnow) + pp(i,j,k,nold) )
        pp(i,j,k,nnow) =  pp(i,j,k,nnow) + alphaass * zdd
        pp(i,j,k,nnew) =  pp(i,j,k,nnew) + (alphaass - 1.0_ireals) * zdd
      ENDDO
    ENDDO

    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        zdd = epsass*(  t(i,j,k,nnew) - 2.0_ireals* t(i,j,k,nnow) +  t(i,j,k,nold) )
        t(i,j,k,nnow)  =   t(i,j,k,nnow) + alphaass * zdd
        t(i,j,k,nnew)  =   t(i,j,k,nnew) + (alphaass - 1.0_ireals) * zdd
      ENDDO
    ENDDO
  ENDDO
#ifdef MESSY
    DO jt = 1,ntrac_gp
       IF (ti_gp(jt)%tp%meta%cask_i(I_ADVECT) == ON) THEN
          DO  k = 1, ke
             DO j = jstartpar, jendpar
                DO i = istartpar, iendpar
                   zdd = epsass*( xt_array(nnew)%ptr(i,j,jt,k)  &
                        - 2.*xt_array(nnow)%ptr(i,j,jt,k)       &
                        + xt_array(nold)%ptr(i,j,jt,k) )
                   xt_array(nnow)%ptr(i,j,jt,k) =   &
                        xt_array(nnow)%ptr(i,j,jt,k) + alphaass * zdd
                   xt_array(nnew)%ptr(i,j,jt,k) = xt_array(nnew)%ptr(i,j,jt,k) &
                        + (alphaass - 1.0_ireals) * zdd
                ENDDO
             ENDDO
          ENDDO
       ENDIF
  ...
  DO iztrcr = 1, trcr_get_ntrcr()
    ...
    IF ( izsp_adv_lf(iztrcr) == 1_iintegers .AND. izadv(iztrcr) == T_ADV_ON ) THEN
      ...
      DO  k = 1, ke
        DO j = jstartpar, jendpar
          DO i = istartpar, iendpar
            zdd = epsass*( ztrcr_new(i,j,k) - 2._ireals*ztrcr_now(i,j,k)      &
                         + ztrcr_old(i,j,k) )
  ...
  IF (lcond) THEN
    ...
    ENDIF
#else
    IF (.NOT. ASSOCIATED(qc_now)) THEN
       CALL get_tracer(izerror, GPTRSTR, 'QV', pxtm1=qv_now)
       CALL tracer_halt(yzroutine, izerror)
       CALL get_tracer(izerror, GPTRSTR, 'QC', pxtm1=qc_now)
       CALL tracer_halt(yzroutine, izerror)
    ENDIF
#endif 
...
#endif
!------------------------------------------------------------------------------
! End of module procedure sardass
!------------------------------------------------------------------------------

END SUBROUTINE sardass
