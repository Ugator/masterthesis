...
#else
! COSMO
USE data_tracer,              ONLY: idt_qv, idt_qc
! MESSy/BMIL
USE messy_main_tracer_mem_bi, ONLY: xt_array, xtte
#endif
...
!------------------------------------------------------------------------------
  ...
  CALL trcr_get(izerror, 'QV', ptr_tlev = nvar, ptr = qv)
  IF (izerror /= 0) THEN
  ...
  IF (izerror /= 0) THEN
    ...
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QC', ptr_tlev = nvar, ptr = qc)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  ...
  ! this subroutine is only called with nnow
    qv      => xt_array(nvar)%ptr(:,:,idt_qv,:)
    qc      => xt_array(nvar)%ptr(:,:,idt_qc,:)
    qv_tens => xtte(:,:,idt_qv,:)
    qc_tens => xtte(:,:,idt_qc,:)
#endif
...
!US old scheme    CALL turbtran(nvar,dt)
    ...
    CALL turbtran(dt, lstfnct, iini, nprv, ntur, ntim, nvor,                   &
                  ...
                  tke, tkvm, tkvh, rcld,                                       &
#else
                  tke, tkvm(:,:,2:ke+1), tkvh(:,:,2:ke+1), rcld,               &
#endif
...
#ifndef MESSY
                  tke=tke, tkvm=tkvm, tkvh=tkvh, rcld=rcld,                   &
#else
                  tke=tke, tkvm=tkvm(:,:,2:ke+1), tkvh=tkvh(:,:,2:ke+1),      &
                  rcld=rcld,                                                  &
#endif
                  u_tens=utens, v_tens=vtens, t_tens=ttens, qv_tens=qv_tens,  &
                  qc_tens=qc_tens, tketens=tketens,                           &
