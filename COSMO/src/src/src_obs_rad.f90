...
!-------------------------------------------------------------------------------
  ...
  CALL trcr_get(ierrstat, 'QV', ptr_tlev = nnow, ptr=qv)
  IF (ierrstat /= 0) THEN
    yerror = trcr_errorstr(ierrstat)
    CALL model_abort(my_cart_id, ierrstat, yerror, yzroutine)
  ENDIF
  CALL trcr_get(ierrstat, 'QC', ptr_tlev=nnow, ptr=qc)
  IF (ierrstat /= 0) THEN
    yerror = trcr_errorstr(ierrstat)
    CALL model_abort(my_cart_id, ierrstat, yerror, yzroutine)
  ENDIF
  CALL trcr_get(ierrstat, 'QG', ptr_tlev=nnow, ptr=qg)
  IF (ierrstat /= 0 .AND. ierrstat /= T_ERR_NOTFOUND) THEN
    yerror = trcr_errorstr(ierrstat)
    CALL model_abort(my_cart_id, ierrstat, yerror, yzroutine)
  ENDIF
  CALL trcr_get(ierrstat, 'QI', ptr_tlev=nnow, ptr=qi)
  IF (ierrstat /= 0 .AND. ierrstat /= T_ERR_NOTFOUND) THEN
    yerror = trcr_errorstr(ierrstat)
    CALL model_abort(my_cart_id, ierrstat, yerror, yzroutine)
  ENDIF
  CALL trcr_get(ierrstat, 'QS', ptr_tlev=nnow, ptr=qs)
  IF (ierrstat /= 0 .AND. ierrstat /= T_ERR_NOTFOUND) THEN
    yerror = trcr_errorstr(ierrstat)
    CALL model_abort(my_cart_id, ierrstat, yerror, yzroutine)
  ENDIF
