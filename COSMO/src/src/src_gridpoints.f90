...
#else
!COSMO
USE data_tracer,              ONLY: idt_qc, idt_qv, idt_qi
! MESSy/BMIL
USE messy_main_tracer_mem_bi, ONLY: xt_array
#endif
...
!------------------------------------------------------------------------------
  ...
  CALL trcr_get(nstat, 'QV', ptr_tlev = ntl, ptr = qv)
  IF (nstat /= 0) THEN
    yerrmsg = trcr_errorstr(nstat)
    CALL model_abort(my_cart_id, nstat, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(nstat, 'QC', ptr_tlev = ntl, ptr = qc)
  IF (nstat /= 0) THEN
    yerrmsg = trcr_errorstr(nstat)
    CALL model_abort(my_cart_id, nstat, yerrmsg, yroutine)
  ENDIF
  CALL trcr_get(nstat, 'QI', ptr_tlev = ntl, ptr = qi)
  IF (nstat /= 0 .AND. nstat /= T_ERR_NOTFOUND) THEN
    yerrmsg = trcr_errorstr(nstat)
    CALL model_abort(my_cart_id, nstat, yerrmsg, yroutine)
  ENDIF
#else
  ! the subroutine is called with ntl =  nnow only.
  qv => xt_array(ntl)%ptr(:,:,idt_qv,:)
  qc => xt_array(ntl)%ptr(:,:,idt_qc,:)
  IF (idt_qi > 0) qi => xt_array(ntl)%ptr(:,:,idt_qi,:)
#endif
...
IF (lgplong) THEN
  ...
  DO  mzg = 1, ngp
    ...
    WRITE (nugpunit(mzg), '(A,A,A,F10.1,A,I10,A)')                           &
    ...
    IF (nzpa.EQ.1) THEN
      ...
      IF (lpreslev) THEN
         ...
         DO k=ke,1,-1
            ...
            ! US: this only works for vertical coordinate type = 2,3!
            IF ((vcoord_in%vert_coord(k) > vcoord_in%vcflat) .OR. flat) THEN
...
!------------------------------------------------------------------------------
  ...
  ALLOCATE ( gpsmlt   (ngp, nstepsgp) , STAT=istat )  ; gpsmlt = 0.0
  ...
  ELSE IF (lgplong .OR. lgpspec) THEN
    ...
    ALLOCATE ( gpswdir_s  (ngp, nstepsgp) , STAT=istat );  gpswdir_s = 0.0
    ALLOCATE ( gpswdifd_s (ngp, nstepsgp) , STAT=istat );  gpswdifd_s = 0.0
    ALLOCATE ( gpswdifu_s (ngp, nstepsgp) , STAT=istat );  gpswdifu_s = 0.0
