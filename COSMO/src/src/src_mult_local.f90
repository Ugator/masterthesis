...
!   nmloi            ,& ! index of report in the obs. increment record
    ...
    ivrsa  , ivrse   ,& ! index limits for 'ivrs' when getting additional info
    ivrsd            ,& ! index interval for 'ivrs' when getting additional info
...
! (qcvsond(4 --> 500hPa) = 3.4m/s ;   qctsond(3 --> 700hPa) = 0.7K)
  ...
  qcupr  = 0.85_ireals * (qcc(1) + qcvf(1) *qcvsond(4))
  qctpr  = 0.85_ireals * (qcc(3) + qcvf(3) *qctsond(3))
  qcqpr  = 0.85_ireals *  qcc(4)

  lwrite = (MOD( ntstep,MAX( 1, 6/NINT(tconbox/dt) ) ) == 0)
  lwruv  = (lwrite) .AND. (zwtml(icml,itim,1) > 0.8_ireals) .AND. (lwonl)
  lwrt   = (lwrite) .AND. (zwtml(icml,itim,2) > 0.8_ireals) .AND. (lwonl)
  lwrq   = (lwrite) .AND. (zwtml(icml,itim,3) > 0.8_ireals) .AND. (lwonl)
...
!           .AND. (MIN( topobs(1) , topobs(3) , topobs(4) ) < 1049.9_ireals)   &
!      .OR.       (modespr > 0) .AND. (      lretv)                            &
!           .AND. (MIN( topobtv(1), topobtv(3), topobtv(4)) < 1049.9_ireals)
...
! WHILE-loop over remaining observation levels below current model level
      ...
      DO WHILE ((zobbdy(kobs,nbtlop) >= zlopml(icml,km)) .AND. (.NOT. ltop))
        ...
        IF (      ((lextu) .OR. (lextt) .OR. (lextq))                          &
            .AND. (     (zobbdy(kobs,nbtlop)+0.0001_ireals < zloprev)          &
...
! if obs. increment exists: increase number of obs. increment levels
      ...
      IF ((km >= mdatop) .AND. (km <= mdabot)) THEN
        ...
        IF ((lexisto) .AND. (klev >= 1) .AND. (kobtyp == nsattv)) THEN
          klvi    = ktopoi + klev
          IF (oiml(noilp,klvi) < zlopml(icml,km) +0.0001_ireals)               &
            klev = klev - 1
        ENDIF   
        klev    = klev + 1
        klvi    = ktopoi + klev
...
! check if the profile is stable, and needs to be modified
  ...
  DO klev = 2 , numlev
    klvi  = ktopoi + klev
    oiml (noiz,klvi) = MAX( oiml(noiz,klvi) , oiml(noiz,klvi-1) +0.1_ireals )
  ENDDO
  lmono = .TRUE.
...
! ----------------
  ...
  REAL    (KIND=ireals   )   ,  PARAMETER ::  &
    c1000r =  0.001_ireals  ,& ! difference limit to continue iteration  
                               ! (if this limit is too large (e.g. 0.01) then
                               !  observation increments are set to 0 too often)
    thriq  =  2.0_ireals    ,& ! minimum observed IWV value for assimilation 
    zfrmin = 0.60_ireals       ! minimum value of zfracb to consider
...
!-------------------------------------------------------------------------------
  ...
  ! =====================
    ...
    IF ((zyqc(1) >= epsy) .AND. (ntotqc < maxqcp)) THEN
      ...
      oyqc (ntotqc, 2) = ogpbdy(ngpob,nbgp)   * 0.01_ireals
...
! Station pressure in hPa
  pgps   = ogpbdy (ngpob,nbgp) / 100._ireals
...
! Prepare for printing info every half hour at GPS time
  IF ((ntstep <= 1) .OR.(ntstep==48) .OR.(ntstep==138) .OR.(ntstep==228)) THEN
    IF (ke >= 13) THEN
      ...
      zprbuf (2) = REAL ( iter, ireals )
      zprbuf (3) = REAL ( isat, ireals )
      zprbuf (4) = ziqgps
      zprbuf (5) = ziqint
      zprbuf (6) = ziqobs
      zprbuf (7) = ziqite
...
!   nmloi            ,& ! index of report in the obs. increment record
  ...
  REAL    (KIND=ireals   ) ::  &
    ...
    zczrifl          ,& ! initial values for horiz. correl. scales at cut-offs
  ...
  LOGICAL                  ::  &
    ...
    lthij            ,& ! spreading at current station is not along model levels
    lveridat         ,& ! fill simulated obs body (for writing to feedback file)
      ...
      IF (nmlob > 0) THEN
        ...
        DO ivrs = 1, 3
          ...
          IF (      (     ((lraso           ) .AND. (gnudg  (ivar) > epsy))    &
                     .OR. ((kobtyp == nairep) .AND. (gnudgar(ivar) > epsy)))   &
...
!                 , NINT( ogphed(MAX(1,igpadm(ista,1)),nhtime) *100._ireals )  &
!                 , NINT( ogphed(MAX(1,igpadm(ista,2)),nhtime) *100._ireals )  &
...
!       IF ('zcut' < orography) THEN lower level limit is known & set to 'ke+1'.
          IF ((msprpar == 1) .AND. (kthr > ktp) .AND. (kthr <= ke)) THEN
            zupper  = zcut + hhl(io,jo,ke+1) + 400.0_ireals
...
! IF (ntpy >= 2)  fmultw  = REAL ( kwtyp(nwtyp+1) - 1, ireals )
...
! QC threshold: 0.15 * IWV sat   (independently of data density 'zwsum(2)')
      ...
      qctiq    = (c1 - (c1 - qcftsiv) *MIN( zwsum(4) /5.0_ireals , c1 )) * qctiq
...
!           (if .not.lreset in mult_org_localinfo) !
        ...
        IF (ntotqc < maxqcp) THEN
          ...
          oyqc (ntotqc, 2) = omlbdy(nmlob,iltop,nbtp)  / 100.0_ireals
...
!           myqc (ntotqc, 1) = mogphd(ngpob,nhobtp)
            IF (.NOT. lwrqc) myqc(ntotqc,1) = 0
            oyqc (ntotqc, 1) = ogphed(ngpob,nhtime)
            oyqc (ntotqc, 2) = ogpbdy(ngpob,nbgp)   / 100.0_ireals
            oyqc (ntotqc, 3) = ogphed(ngpob,nhjlat)
            oyqc (ntotqc, 4) = ogphed(ngpob,nhilon)
            oyqc (ntotqc, 5) = qctiq
            oyqc (ntotqc, 6) = zoiciv(istaqc,itic) + zmodiv(istaqc)
...
!           myqc (ntotqc, 1) = motvhd(ntvob,nhobtp)
            ...
            oyqc (ntotqc, 2) = otvbdy(ntvob,1,nbvp)   / 100.0_ireals
...
! ----------------
! REAL (KIND = ireals)     , PARAMETER  :: &
!   c300hpa = 30000.0_ireals      ! 300 hPa  in [Pa]
...
ELEMENTAL REAL FUNCTION ftd  ( zpv, b1, b2w, b3, b4w )
  !---------------------------------------------------
  REAL    (KIND=ireals)     , INTENT (IN)  ::  zpv, b1, b2w, b3, b4w
  REAL    (KIND=ireals)                    ::  zlogpv
  !---------------------------------------------------------------------------
