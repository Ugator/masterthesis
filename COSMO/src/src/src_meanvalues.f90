...
!ROA REMARK: qv is at nnew, the others at nnow. Isn't it a bug?
    ...
    CALL trcr_get(nzerror, 'QV', ptr_tlev = nnew, ptr = qv)
    IF (nzerror /= 0) THEN
      yerrmsg = trcr_errorstr(nzerror)
      CALL model_abort(my_cart_id, nzerror, yerrmsg, yroutine)
    ENDIF
    CALL trcr_get(nzerror, 'QC', ptr_tlev = nnow, ptr = qc)
    IF (nzerror /= 0) THEN
      yerrmsg = trcr_errorstr(nzerror)
      CALL model_abort(my_cart_id, nzerror, yerrmsg, yroutine)
    ENDIF
    CALL trcr_get(nzerror, 'QI', ptr_tlev = nnow, ptr = qi)
    IF (nzerror /= 0 .AND. nzerror /= T_ERR_NOTFOUND) THEN
      yerrmsg = trcr_errorstr(nzerror)
      CALL model_abort(my_cart_id, nzerror, yerrmsg, yroutine)
    ENDIF
    CALL trcr_get(nzerror, 'QR', ptr_tlev = nnow, ptr = qr)
    IF (nzerror /= 0 .AND. nzerror /= T_ERR_NOTFOUND) THEN
      yerrmsg = trcr_errorstr(nzerror)
      CALL model_abort(my_cart_id, nzerror, yerrmsg, yroutine)
    ENDIF
    CALL trcr_get(nzerror, 'QS', ptr_tlev = nnow, ptr = qs)
    IF (nzerror /= 0 .AND. nzerror /= T_ERR_NOTFOUND) THEN
      yerrmsg = trcr_errorstr(nzerror)
      CALL model_abort(my_cart_id, nzerror, yerrmsg, yroutine)
    ENDIF
    CALL trcr_get(nzerror, 'QG', ptr_tlev = nnow, ptr = qg)
    IF (nzerror /= 0 .AND. nzerror /= T_ERR_NOTFOUND) THEN
      yerrmsg = trcr_errorstr(nzerror)
      CALL model_abort(my_cart_id, nzerror, yerrmsg, yroutine)
    ENDIF
#ifdef TWOMOM_SB
    CALL trcr_get(nzerror, 'QH', ptr_tlev = nnow, ptr = qh)
    IF (nzerror /= 0 .AND. nzerror /= T_ERR_NOTFOUND) THEN
      yerrmsg = trcr_errorstr(nzerror)
      CALL model_abort(my_cart_id, nzerror, yerrmsg, yroutine)
    ENDIF
...
#else
    IF (idx_qv < 0) THEN
     CALL get_tracer(nzerror, GPTRSTR, 'QV', idx=idx_qv)
     CALL tracer_halt(yroutine,nzerror)

     CALL get_tracer(nzerror, GPTRSTR, 'QC', idx=idx_qc)
     CALL tracer_halt(yroutine,nzerror)

     CALL get_tracer(nzerror, GPTRSTR, 'QI', idx=idx_qi)
     IF (nzerror /= TR_NEXIST) CALL tracer_halt(yroutine,nzerror)

     CALL get_tracer(nzerror, GPTRSTR, 'QR', idx=idx_qr)
     IF (nzerror /= TR_NEXIST) CALL tracer_halt(yroutine,nzerror)

     CALL get_tracer(nzerror, GPTRSTR, 'QS', idx=idx_qs)
     IF (nzerror /= TR_NEXIST) CALL tracer_halt(yroutine,nzerror)

     CALL get_tracer(nzerror, GPTRSTR, 'QG', idx=idx_qg)
     IF (nzerror /= TR_NEXIST) CALL tracer_halt(yroutine,nzerror)
    ENDIF
    ...
    IF (idx_qi >0 ) qi => xt_array(nnow)%ptr(:,:,idx_qi,:)
    IF (idx_qr >0 ) qr => xt_array(nnow)%ptr(:,:,idx_qr,:)
    IF (idx_qs >0 ) qs => xt_array(nnow)%ptr(:,:,idx_qs,:)
    IF (idx_qg >0 ) qg => xt_array(nnow)%ptr(:,:,idx_qg,:)
#endif
...
! Retrieve the required microphysics tracers
  ...
  IF (izerror /= 0) THEN
    ...
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QI', ptr_tlev = nnow, ptr = qi)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QG', ptr_tlev = nnow, ptr = qg)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QR', ptr_tlev = nnow, ptr = qr)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QS', ptr_tlev = nnow, ptr = qs)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
#ifdef TWOMOM_SB
  CALL trcr_get(izerror, 'QH', ptr_tlev = nnow, ptr = qh)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
...
#ifndef MESSY
  ! retrieve the required microphysics tracers
  CALL trcr_get(izerror, 'QV', ptr_tlev = ntlev, ptr = qv)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QC', ptr_tlev = ntlev, ptr = qc)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QI', ptr_tlev = ntlev, ptr = qi)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QG', ptr_tlev = ntlev, ptr = qg)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QR', ptr_tlev = ntlev, ptr = qr)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QS', ptr_tlev = ntlev, ptr = qs)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
    ...
    IF (idx_qv < 0) THEN
     ...
     IF (izerror /= TR_NEXIST) CALL tracer_halt(yzroutine,izerror)
    ENDIF
    qv => xt_array(ntlev)%ptr(:,:,idx_qv,:)
    qc => xt_array(ntlev)%ptr(:,:,idx_qc,:)
    IF (idx_qi >0 ) qi => xt_array(ntlev)%ptr(:,:,idx_qi,:)
    IF (idx_qr >0 ) qr => xt_array(ntlev)%ptr(:,:,idx_qr,:)
    IF (idx_qs >0 ) qs => xt_array(ntlev)%ptr(:,:,idx_qs,:)
    IF (idx_qg >0 ) qg => xt_array(ntlev)%ptr(:,:,idx_qg,:)
#endif
