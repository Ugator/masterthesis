...
!     trcr_get_block       retrieve block of tracer data              4250-4299
...
!
! Current Code Owner: C2SM/ETH, Anne Roches
!  phone:  +41  44  632 7975
!  fax:    -
...
INTEGER (KIND=iintegers) :: &
  i_status = T_STAT_START              ! status of tracer module
...
!------------------------------------------------------------------------------
  ...
  ! initialize arrays
  trcr_data      = 0.0_ireals
  trcr_data_bd   = 0.0_ireals
  trcr_data_tens = 0.0_ireals
...
!------------------------------------------------------------------------------
  ...
  DO i_trcr = 1, n_trcr
    ...
    IF (itmp /= 0_iintegers) RETURN
    ierr = 4155_iintegers
    IF ( i3 < 1_iintegers .OR. &     
         i3 > num_gribtabs ) RETURN
    ...
    var(i1,i2,i3) = ar_des(                                                 &
      yname, 110, 0, 0, 1.0, 0.0, 0, 4, ke,                                 &
      trcr_data(:,:,:,i_trcr,:), trcr_data_bd(:,:,:,trcr(i_trcr)%idx_bd,:), &
      NULL(), NULL(), NULL(), 1,                                            &
      yunits, ystandard_name, ylong_name, ' '                               &
     )
...
SUBROUTINE trcr_get_byindex ( ierr, idx_trcr, ptr, ptr_bd, ptr_tens, ptr_tlev )
!------------------------------------------------------------------------------
!
! Description:
!
...
! --------------------
 ...
 REAL (KIND=ireals), POINTER,              OPTIONAL :: ptr(:,:,:)      ! pointer to tracer
 REAL (KIND=ireals), POINTER,              OPTIONAL :: ptr_bd(:,:,:,:) ! pointer to tracer boundary 
 REAL (KIND=ireals), POINTER,              OPTIONAL :: ptr_tens(:,:,:) ! pointer to tracer tendency 
...
!------------------------------------------------------------------------------
  ...
  CALL trcr_get_byindex( ierr, i_trcr, ptr, ptr_bd, ptr_tens, ptr_tlev)
  IF ( ierr /= 0_iintegers ) RETURN
...
SUBROUTINE trcr_get_block( ierr, idx_start, idx_end, ptr, ptr_bd, ptr_tens, ptr_tlev )
!------------------------------------------------------------------------------
!
! Description:
!
...
!ROA TODO: this n+1 is provisory. We need to find a cleaner solution later on.
...
SUBROUTINE trcr_meta_def_pointer2(ierr,yname,ydefault,iidx,lprotect)
  ...
  INTEGER (KIND=iintegers), INTENT(OUT) :: ierr
  CHARACTER(LEN=*), INTENT(IN)    :: yname
  REAL (KIND=ireals), POINTER, INTENT(IN) :: ydefault(:,:)
  INTEGER, OPTIONAL, INTENT(OUT)  :: iidx
  LOGICAL, OPTIONAL, INTENT(IN)   :: lprotect
  ierr = 0_iintegers
  CALL metadata_define(mtrcr, yname, ydefault, iidx, lprotect)
...
SUBROUTINE trcr_meta_get_r8_byindind(ierr,itrcr,imeta,ydata)
  ...
  INTEGER, INTENT(IN) :: itrcr
  INTEGER, INTENT(IN) :: imeta
  DOUBLE PRECISION, INTENT(OUT) :: ydata
  ierr = 0_iintegers
  CALL trcr_check_index(ierr, itrcr)
  IF (ierr == 0_iintegers) THEN
    CALL metadata_get(mtrcr,itrcr,imeta,ydata)
...
SUBROUTINE trcr_meta_get_ptr2_byindind(ierr,itrcr,imeta,ydata)
  ...
  INTEGER, INTENT(IN) :: itrcr
  INTEGER, INTENT(IN) :: imeta
  REAL (KIND=ireals), POINTER, INTENT(OUT) :: ydata(:,:)
  ierr = 0_iintegers
  CALL trcr_check_index(ierr, itrcr)
  IF (ierr == 0_iintegers) THEN
    CALL metadata_get(mtrcr,itrcr,imeta,ydata)
...
SUBROUTINE trcr_meta_set_ptr2_byindind(ierr,itrcr,imeta,ydata)
  ...
  INTEGER, INTENT(IN) :: itrcr
  INTEGER, INTENT(IN) :: imeta
  REAL (KIND=ireals), POINTER, INTENT(IN) :: ydata(:,:)
  ierr = 0_iintegers
  CALL trcr_check_index(ierr, itrcr)
  IF (ierr == 0_iintegers) THEN
    CALL metadata_set(mtrcr,itrcr,imeta,ydata)
...
#ifdef MESSY
  ! trcr_get_datastruct errors  (4900-4949)
  CASE(4900)
    zstr = 'TRCR_GET_DATASTRUCT CALLED OUT OF SEQUENCE'
  CASE(4901)
    zstr = 'INDEX ARRAY idx_trcr BADLY DIMENSIONNED'
  CASE(4902)
    zstr = 'INDEX ARRAY idx_bd BADLY DIMENSIONNED'
#endif
...
#ifdef MESSY
!==============================================================================
!+ Subroutine "trcr_get_datastruct" in "src_tracer" to get back the tracer struct.
!------------------------------------------------------------------------------

SUBROUTINE trcr_get_datastruct(ierr, ptr_trcr, ptr_bd, ptr_tens, idx_trcr, idx_bd)
!------------------------------------------------------------------------------
!
! Description:
!
! This routine gives access to the internal data structures used to store
! the tracers and their related fields (tendency and boundary). This routine
! should not be used and is subject to change if internals of the tracer module
...
! --------------------
 ...
 INTEGER (KIND=iintegers),    INTENT(OUT)           :: ierr               ! error status
 REAL (KIND=ireals), POINTER,              OPTIONAL :: ptr_trcr(:,:,:,:,:)! pointer to tracer structure
 REAL (KIND=ireals), POINTER,              OPTIONAL :: ptr_bd(:,:,:,:,:)  ! pointer to tracer boundary structure 
 REAL (KIND=ireals), POINTER,              OPTIONAL :: ptr_tens(:,:,:,:)  ! pointer to tracer tendency structure
 INTEGER (KIND=iintegers),    INTENT(OUT), OPTIONAL :: idx_trcr(:)        ! index for accesing tracer and tendency structure
 INTEGER (KIND=iintegers),    INTENT(OUT), OPTIONAL :: idx_bd(:)          ! index for accesing boundary structure
...
!------------------------------------------------------------------------------
! Begin Subroutine trcr_get_datastruct
!------------------------------------------------------------------------------

 
  ! initialize pointer (for safetey)
  IF (PRESENT(ptr_trcr)) NULLIFY(ptr_trcr)
  IF (PRESENT(ptr_bd  )) NULLIFY(ptr_bd  )
  IF (PRESENT(ptr_tens)) NULLIFY(ptr_tens)
  ...
  ierr = 4900_iintegers
  IF ( i_status /= T_STAT_DEFINE .AND. i_status /= T_STAT_ALLOC ) RETURN
  IF ( ( PRESENT(ptr_trcr) .OR. PRESENT(ptr_bd) .OR. PRESENT(ptr_tens)             &
         .AND. i_status /= T_STAT_ALLOC ) RETURN


  ! provide pointer (if requested)
  IF ( PRESENT(ptr_trcr) ) THEN
    ptr_trcr => trcr_data(:,:,:,:,:)
  ENDIF
  ...
  ! provide tendency pointer (if requested)
  IF (PRESENT(ptr_tens)) THEN
    ptr_tens => trcr_data_tens(:,:,:,:)
  ENDIF

  ! check indices (if requested)
  IF (PRESENT(idx_trcr)) THEN
    ierr = 4901_iintegers
    IF (SIZE(idx_trcr) < n_trcr) RETURN
    idx_trcr(:) = trcr(:)%idx_trcr
  ENDIF
  IF (PRESENT(idx_bd)) THEN
    ierr = 4902_iintegers
    IF (SIZE(idx_bd)   < n_trcr  ) RETURN
    idx_bd(:) = trcr(:)%idx_bd
  ENDIF
...
!------------------------------------------------------------------------------
! End of module procedure trcr_get_datastruct
!------------------------------------------------------------------------------
  
END SUBROUTINE trcr_get_datastruct
