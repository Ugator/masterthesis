...
!   - constant surface parameters (read from ASCII-file
!     or constant values for the whole domain)
!   - artificial inital and boundary data for the model:
!     read profiles from ASCII-file or use analytical formulas
...
!   ingredients for idealized simulations may be combined by
...
!   in the lowest 100 hPa, if ladd_noise_tw = .true. and are simple
...
!------------------------------------------------------------------------------
  ...
  USE vgrid_refatm_utils,       ONLY: reference_atmosphere, reference_atmosphere_2,      &
                                      ...
                                      vcoord_in, refatm_in, nfltvc_in, svc1_in, svc2_in, &
...
#else
USE messy_main_tracer_mem_bi, ONLY: xt_array, xt_bd, xtte &
                                  , GPTRSTR, ntrac_gp
...
!------------------------------------------------------------------------------
  ...
  ! Maximum number of height levels allowed in namelist (NOTE: for grib output only nvevels <= 200 possible)
  INTEGER(KIND=iintegers), PARAMETER :: nvcoordvec_max = 999
  ...
  REAL(KIND=ireals) :: exp_galchen, &   ! if ivctype = 2/3 and zspacing_type = 'galchen' this defines the
  ...
  REAL (KIND=ireals)         ::        &
       vcoordvec(999) ! vector to specify vcoord explicitly in namelist
...
CONTAINS
  ...
  SUBROUTINE get_tracers
    ...
    CALL trcr_get(ierror, 'QV', ptr_tlev=nnew, ptr=qv, ptr_bd=qv_bd)
    IF ( ierror /= 0_iintegers ) THEN
      yerrmsg = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QC', ptr_tlev=nnew, ptr=qc, ptr_bd=qc_bd)
    IF ( ierror /= 0_iintegers ) THEN
      yerrmsg = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QI', ptr_tlev=nnew, ptr=qi, ptr_bd=qi_bd)
    IF ( ierror /= 0_iintegers .AND. ierror /= T_ERR_NOTFOUND ) THEN
      yerrmsg = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QR', ptr_tlev=nnew, ptr=qr, ptr_bd=qr_bd)
    IF ( ierror /= 0_iintegers .AND. ierror /= T_ERR_NOTFOUND ) THEN
      yerrmsg = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QS', ptr_tlev=nnew, ptr=qs, ptr_bd=qs_bd)
    IF ( ierror /= 0_iintegers .AND. ierror /= T_ERR_NOTFOUND ) THEN
      yerrmsg = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
   CALL trcr_get(ierror, 'QG', ptr_tlev=nnew, ptr=qg, ptr_bd=qg_bd)
    IF ( ierror /= 0_iintegers .AND. ierror /= T_ERR_NOTFOUND ) THEN
      yerrmsg = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
#ifdef TWOMOM_SB
    CALL trcr_get(ierror, 'QH', ptr_tlev=nnew, ptr=qh)
    IF ( ierror /= 0_iintegers .AND. ierror /= T_ERR_NOTFOUND ) THEN
      yerrmsg = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QNCLOUD', ptr_tlev=nnew, ptr=qnc)
    IF ( ierror /= 0_iintegers .AND. ierror /= T_ERR_NOTFOUND ) THEN
      yerrmsg = trcr_errorstr(ierror) 
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QNRAIN', ptr_tlev=nnew, ptr=qnr)
    IF ( ierror /= 0_iintegers .AND. ierror /= T_ERR_NOTFOUND ) THEN
      yerrmsg = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QNICE', ptr_tlev=nnew, ptr=qni)
    IF ( ierror /= 0_iintegers .AND. ierror /= T_ERR_NOTFOUND ) THEN
      yerrmsg = trcr_errorstr(ierror) 
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QNSNOW', ptr_tlev=nnew, ptr=qns)
    IF ( ierror /= 0_iintegers .AND. ierror /= T_ERR_NOTFOUND ) THEN
      yerrmsg = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QNGRAUPEL', ptr_tlev=nnew, ptr=qng)
    IF ( ierror /= 0_iintegers .AND. ierror /= T_ERR_NOTFOUND ) THEN
      yerrmsg = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QNHAIL', ptr_tlev=nnew, ptr=qnh)
    IF ( ierror /= 0_iintegers .AND. ierror /= T_ERR_NOTFOUND ) THEN
      yerrmsg = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerrmsg, yzroutine)
    ENDIF
...
#else
    IF (idx_qi < 0) THEN
     CALL get_tracer(ierror, GPTRSTR, 'QV', idx=idx_qv)
     CALL tracer_halt(yzroutine,ierror)

     CALL get_tracer(ierror, GPTRSTR, 'QC', idx=idx_qc)
     CALL tracer_halt(yzroutine,ierror)

     CALL get_tracer(ierror, GPTRSTR, 'QI', idx=idx_qi)
     IF (ierror /= TR_NEXIST) CALL tracer_halt(yzroutine,ierror)

     CALL get_tracer(ierror, GPTRSTR, 'QR', idx=idx_qr)
     IF (ierror /= TR_NEXIST) CALL tracer_halt(yzroutine,ierror)

     CALL get_tracer(ierror, GPTRSTR, 'QS', idx=idx_qs)
     IF (ierror /= TR_NEXIST) CALL tracer_halt(yzroutine,ierror)

     CALL get_tracer(ierror, GPTRSTR, 'QG', idx=idx_qg)
     IF (ierror /= TR_NEXIST) CALL tracer_halt(yzroutine,ierror)
    ENDIF
    ...
    ENDIF
    IF (idx_qr >0 ) THEN
       qr    => xt_array(nnew)%ptr(:,:,idx_qr,:)
       qr_bd => xt_bd(:,:,idx_qr,:,:)
    ENDIF
    IF (idx_qs >0 ) THEN
       qs    => xt_array(nnew)%ptr(:,:,idx_qs,:)
       qs_bd => xt_bd(:,:,idx_qs,:,:)
    ENDIF
    IF (idx_qg >0 ) THEN
       qg    => xt_array(nnew)%ptr(:,:,idx_qg,:)
       qg_bd => xt_bd(:,:,idx_qg,:,:)
    ENDIF
#endif
...
#ifdef MESSY
  ! This needs to be done, as no pointers are allowed in namelists and
  ! for CHANNEL output the variables below need to have the POINTER attribute

 USE data_modelconfig,  ONLY: p0sl_mc    => p0sl    &
                            , t0sl_mc    => t0sl    &
                            , dt0lp_mc   => dt0lp   &
                            , delta_t_mc => delta_t &
                            , h_scal_mc  => h_scal  &
 ...
 REAL(KIND=ireals) :: p0sl, t0sl, dt0lp, delta_t, h_scal &
                    , vcflat, svc1, svc2, bvref
#endif
    ...
    INTEGER (KIND=iintegers)   ::       &
         ivctype, ivctype_d,     & ! type of vertical coordinate
         irefatm, irefatm_d        ! 1: old reference atm. based on dT/dlnp = const
    ! 2: new reference atm. with asymptotically isothermal stratosphere

    REAL (KIND=ireals)         ::       &
         p0sl, p0sl_d,         & ! reference pressure at sea level
         t0sl, t0sl_d,         & ! reference temperature at sea level
    ...
    REAL(KIND=ireals) :: exp_galchen_d, & ! if ivctype = 2/3 and zspacing_type = 'galchen' this defines the
                                ! exponent in the Gal-Chen formula
         vcflat, vcflat_d, &      ! height were model levels become flat
    ...
    REAL(KIND=ireals) :: bvref, bvref_d   ! Constant Brunt-Vaisala-Frequency for irefatm=3 [1/s]
...
!!!defaulttag   used by the ed-script ed_addnl.in which automatically adds nlparams
    ...
    NAMELIST /ARTIFCTL/ ivctype, irefatm, p0sl, t0sl, dt0lp, delta_t, h_scal, &
         ...
         t_soil_c , t_snow_c , wf_soil_c , w_i_c , w_snow_c , itype_soil_tw, h_ice_c, t_ice_c , &
...
!!!adddefaulttag   used by the ed-script ed_addnl.in which automatically adds nlparams
    ...
    rasofile_d(:) = ' '; rasofile = 'noname.dat'
    ...
    IF (my_world_id == 0) THEN
      ...
      IF ( nlfile_exists ) THEN
        ...
        IF(iz_err /= 0) THEN
          ...
          iz_err = -3
        ELSE
          READ (nuin, artifctl, IOSTAT=iz_err)
          IF (iz_err > 0) THEN
...
! Set the parameters read for the vertical grid and the reference atmosphere to
! the new structures vcoord_in, refatm_in
vcoord_in%ivctype     = ivctype
vcoord_in%ivcoord_id  = 0
vcoord_in%nlevels     = ke+1
vcoord_in%vcflat      = vcflat
!US this is done in gen_ini_data at the beginning
!IF     (izvctype == 1) THEN
!  DO k=1,ke+1
!    vcoord_in%sigm_coord(k) = vcoordvec(k)
!  ENDDO
!ELSEIF (izvctype == 2 .OR. izvctype == 3) THEN
!  DO k=1,ke+1
!    vcoord_in%vert_coord(k) = vcoordvec(k)
!  ENDDO
!ENDIF
  ...
  SUBROUTINE gen_ini_data 
    ...
    IF ( vcoord_in%ivctype == 1 ) THEN ! Pressure based hybrid coordinate
      ! vcoord(k) cooresponds to pressure above sea-level
      ! normalized by POSL
      SELECT CASE (TRIM(ADJUSTL(zspacing_type)))
      CASE('predefined', 'PREDEFINED', 'Predefined')
        IF (ke == 20) THEN 
          vcoord_in%vcflat = 0.020
          DO k = 1, ke1
            vcoord_in%sigm_coord(k) = zvco_p20(k)
          ENDDO
        ELSEIF (ke == 32) THEN 
          vcoord_in%vcflat = 0.220
          DO k = 1, ke1
            vcoord_in%sigm_coord(k) = zvco_p32(k)
          ENDDO
        ELSEIF (ke == 35) THEN 
          vcoord_in%vcflat = 0.220
          DO k = 1, ke1
            vcoord_in%sigm_coord(k) = zvco_p35(k)
          ENDDO
        ELSEIF (ke == 40) THEN 
          vcoord_in%vcflat = 0.220
          DO k = 1, ke1
            vcoord_in%sigm_coord(k) = zvco_p40(k)
          ENDDO
        ELSE
          WRITE (*,*) '***   No vertical coordinates are pre-specified    ***'
          WRITE (*,*) '***  for ke=',ke,' layers and ivctype=',vcoord_in%ivctype,'  ***'
          WRITE (*,*) '*** Please edid subroutine '//TRIM(yzroutine)//' to proceed ***'
          STOP
        ENDIF
      CASE ('vcoordvec', 'VCOORDVEC', 'Vcoordvec')
        ! if vcoord is specified in namelist, copy it
        vcoord_in%sigm_coord(1:ke+1) = vcoordvec(1:ke+1)
      CASE default
        WRITE (*,*) '***   No vertical coordinates of type',TRIM(ADJUSTL(zspacing_type)),'    ***'
        WRITE (*,*) '***   defined for ivctype=',vcoord_in%ivctype,'  ***'
        WRITE (*,*) '***   Please edid subroutine '//TRIM(yzroutine)//' to proceed ***'
        CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
      END SELECT

    ELSE IF ( vcoord_in%ivctype == 2 ) THEN ! Height-based hybrid Gal-Chen coordinate
      ! vcoord(k) cooresponds to nonnormalized height
      ! above sea-level (z=0)
      SELECT CASE (TRIM(ADJUSTL(zspacing_type)))
      CASE('predefined', 'PREDEFINED', 'Predefined')
        IF (ke == 20) THEN
          vcoord_in%vcflat = 11357.0
          DO k = 1, ke1
            vcoord_in%vert_coord(k) = zvco_z20(k)
          ENDDO
        ELSEIF (ke == 32) THEN
          vcoord_in%vcflat = 11357.0
          DO k = 1, ke1
            vcoord_in%vert_coord(k) = zvco_z32(k)
          ENDDO
        ELSEIF (ke == 35) THEN
          vcoord_in%vcflat = 11357.0
          DO k = 1, ke1
            vcoord_in%vert_coord(k) = zvco_z35(k)
          ENDDO
        ELSEIF (ke == 40) THEN
          vcoord_in%vcflat = 11357.0
          DO k = 1, ke1
            vcoord_in%vert_coord(k) = zvco_z40(k)
          ENDDO
        ELSE
          WRITE (*,*) '***   No vertical coordinates are pre-specified    ***'
          WRITE (*,*) '***  for ke=',ke,' layers and ivctype=',vcoord_in%ivctype,'  ***'
    ...
    ELSE IF ( vcoord_in%ivctype == 3 ) THEN ! SLEVE coordinate
      ...
      CASE('predefined', 'PREDEFINED', 'Predefined')
        IF (ke == 20) THEN
          DO k = 1, ke1
            vcoord_in%vert_coord(k) = zvco_z20(k)
          ENDDO
          vcoord_in%vcflat = vcoord_in%vert_coord(1)
        ELSEIF (ke == 32) THEN
          DO k = 1, ke1
            vcoord_in%vert_coord(k) = zvco_z32(k)
          ENDDO
...
!US    IF ( ANY(soiltyp < 1.0_ireals) .OR. ANY(soiltyp >= 9.0_ireals) ) THEN
    ...
    IF (vcoord_in%ivctype == 3) THEN
      ...
      ! split topo on PE 0
      IF (my_cart_id == 0) THEN
        CALL sleve_split_oro(hsurf_tot, hsurfs_tot, ie_tot, je_tot, nfltvc_in,  &
             0_iintegers, svc1_in, svc2_in, vcoord_in%vcflat, nuspecif,      &
    ...
    CASE (2)
      CALL reference_atmosphere_2                                                 &
           ( hhl, p0, p0hl, rho0, t0, t0hl, dp0, hsurf, hsurfs, ie, je, ke,       &
             refatm_in, vcoord_in, svc1_in, svc2_in, r_d, g, .FALSE.)
    ...
    CALL k_index_of_pressure_levels(  refatm_in%p0sl, vcoord_in%sigm_coord, ke, llm,      &
             klv950, klv850, klv800, klv700, klv500, klv400, klv300 )
    ...
    IF ( .NOT.l2tls ) THEN
     ...
     DO iztrcr = 1, ntrac_gp
        ...
        ztrcr_now => xt_array(nnow)%ptr(:,:,iztrcr,:)
#endif
        ! copy new to now 
        ztrcr_now(:,:,:)  = ztrcr(:,:,:)
  ...
  CONTAINS
    ...
    SUBROUTINE check_vcoord()
      ...
      IF (lspubc) THEN
        ...
        IF (rdheight < vcoord_in%vcflat) THEN
      ...
      IF (vcoord_in%ivctype == 1) THEN
        IF (vcoord_in%sigm_coord(1) <= 0.0_ireals .OR. vcoord_in%sigm_coord(ke+1) /= 1.0_ireals) THEN
          WRITE (*,*) ' SRC_ARTIFDATA: ERROR  *** Wrong values for vcoord: ', &
               vcoord_in%sigm_coord(1:ke+1),' *** '
          WRITE (*,'(a,i3,a)') '        *** must contain ke+1 increasing values ', &
               'between >0.0 and 1.0 for ivctype = ',vcoord_in%ivctype,' !!! *** '
          CALL model_abort (my_world_id, 10006, &
               'ERROR in vert. coordinate specification', &
               'src_artifdata.f90')
          RETURN
        END IF
      ELSE IF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
        IF (vcoord_in%vert_coord(ke+1) /= 0.0_ireals) THEN
          WRITE (*,*) ' ERROR  *** Wrong value for vcoord(ke+1): ',vcoord_in%vert_coord(ke+1),' *** '
      ...
      IF (vcoord_in%ivctype == 1) THEN
        ...
        CASE (1)
          ...
          IF (refatm_in%dt0lp > 0.0_ireals) THEN
            hmax_refatm = -refatm_in%t0sl/refatm_in%dt0lp
            IF (LOG(vcoord_in%sigm_coord(1)) < hmax_refatm) THEN
      ...
      ELSE IF (vcoord_in%ivctype == 2 .OR. vcoord_in%ivctype == 3) THEN
        ...
        CASE (1)
          ...
          IF (refatm_in%dt0lp > 0.0_ireals) THEN
            ...
            hmax_refatm = r_d*refatm_in%t0sl**2/(2.0*refatm_in%dt0lp*g)
            IF (vcoord_in%vert_coord(1) >= hmax_refatm) THEN
              WRITE (*,'(a)') ' SRC_ARTIFDATA: ERROR in vert. coordinate: '// &
        ...
        CASE (3)
          ...
          bvref_tconst = SQRT(g**2 / (refatm_in%t0sl*cp_d))
          IF (refatm_in%bvref < bvref_tconst) THEN
            hmax_refatm = -g/refatm_in%bvref**2 * LOG(1.0-(refatm_in%bvref/bvref_tconst)**2)
            IF (vcoord_in%vert_coord(1) >= hmax_refatm) THEN
              WRITE (*,'(a)') ' SRC_ARTIFDATA: ERROR in vert. coordinate: '// &
                   'vcoord(1) > maximum allowed VALUE ', &
                   hmax_refatm,' for reference atmosphere! Break!'
...
!!! before convection is triggered.
      ...
      ELSE
        ...
        IF ((ntstep-1)*dt/3600 < hcond_on .AND. (lcond .OR. lgsp)) THEN
...
#ifndef MESSY
          CALL trcr_get( izerror, 'QV', ptr_tlev = nnow, ptr = ztrcr_now )
          IF ( izerror /= 0_iintegers ) THEN
            yzerrmsg = trcr_errorstr( izerror )
          ...
          IF (idx_qv < 0) THEN
             ...
             CALL tracer_halt('gen_bound_data', izerror)
          ENDIF
          ztrcr_now => xt_array(nnow)%ptr(:,:,idx_qv,:)
#endif
          DO k=1,ke
            DO j=1,je
              DO i=1,ie
  ...
  SUBROUTINE read_ascii_field_2d(dateiname, shift_i, shift_j, &
    ...
    IF (my_cart_id == 0 .OR. num_compute == 1) THEN
      ...
      ! Format: 1 header line, 1 line with field dimension and then one long 
      !         column with the data (first index varies first).

      READ (freieunit, *)                       ! jump over one header line
      READ (freieunit, *) ie_in, je_in          ! get field dimensions
    ...
    IF (my_cart_id == 0 .OR. num_compute == 1) THEN
      ...
      !?????? Das mit den shift_i shift_j ist noch seeeehr unklar !!!!!!!!!!!!!
  ...
  SUBROUTINE read_raso(rasofile, zml, hhl, temperatur, relfeuchte, &
    ...
    ! Number of header lines expected in the data file:
    num_headerlines = 4
...
!===============================================================================
  ...
  SUBROUTINE set_tempdist(nx)
...
#ifndef MESSY
    ! Retrieve the microphysics tracers
    CALL trcr_get( izerror, 'QV', ptr_tlev = nx, ptr = zqv )
    IF ( izerror /= 0_iintegers ) THEN
      yzerrmsg = trcr_errorstr( izerror )
    ...
    IF (idx_qv < 0) THEN
       CALL get_tracer(izerror, GPTRSTR, 'QV', idx=idx_qv)
       CALL tracer_halt('set_tempdist',izerror)
    ENDIF
    zqv => xt_array(nx)%ptr(:,:,idx_qv,:)
#endif
...
!!!=============================================================================
  ...
  !
  !     NOTE: makes only sense if soil model is turned off (lsoil=.FALSE.).
  !
  !======================================================================================
...
!!!=============================================================================
  ...
  SUBROUTINE artif_heatrate_dist(nx)
    ...
    IF ( izerror /= 0_iintegers ) THEN
      ...
      CALL model_abort( my_cart_id, izerror, yzerrmsg, 'set_tempdist' )
    ENDIF
    CALL trcr_get( izerror, 'QV', ptr_tens = zqv_tens )
    IF ( izerror /= 0_iintegers ) THEN
      yzerrmsg = trcr_errorstr( izerror )
      CALL model_abort( my_cart_id, izerror, yzerrmsg, 'set_tempdist' )
    ENDIF
          ...
          IF ( zfak < 1.0 ) THEN
            ...
            IF ( beta < 1.0 ) THEN
              f_xyz(i,j,k) = COS(pi*zfak/2.0)**2 + &
                   ...
                   ((p0(i,j,k)+pp(i,j,k,nnew))/refatm_in%p0sl)**rdocp
            ELSE
              f_xyz(i,j,k) = COS(pi*zfak/2.0)**2
            END IF
          END IF
...
!!$    INTEGER*4, ALLOCATABLE :: seed(:)
    ...
    DO i=1,k
      seed(i) = MOD(INT(i2/11*13*((MOD(i,5)+i)*i) + i2*(iseed/1000.0) + &
           my_cart_id*i2*0.95/num_compute, KIND=KIND(i2)), i2)
    END DO
