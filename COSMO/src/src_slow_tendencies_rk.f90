...
#ifdef MESSY
USE messy_main_tracer_mem_bi,   ONLY: xt_array, xtte, ntrac_gp, GPTRSTR &
                                    , ti_gp, ON, I_VDIFF, I_HDIFF
...
!------------------------------------------------------------------------------
  ...
  izerror   = 0_iintegers
  yzerrmsg  = ''
  yzroutine = 'init_complete_tendencies'
...
#ifdef MESSY
 REAL (KIND=ireals), TARGET :: qv_p1(ie,je,ke)
#endif
...
!------------------------------------------------------------------------------
  ...
  qv_p1(1:ie,1:je,1:ke) = xt_array(nnew)%ptr(:,:,idx_qv,:) &
        + xtte(:,:,idx_qv,:)
  qv_new => qv_p1
#endif
...
! Description:
!   This procedure is only used in src_runge_kutta and calculates the
!   final updates for the prognostic tracers at time level n+1 (nnew). 
!   The vertical diffusion as the last slow tendency 
!   is computed here for all tracers the vertical diffusion acts on. 
!
! Method:
!   Using the previously calculated updates (on the timelevel nnew) for the
!   tracers from advection, physics, chemistry  and other processes, the 
!   vertical diffusion is solved by a vertically implicit scheme (modified 
!   Crank-Nicolson). 
!
!------------------------------------------------------------------------------
...
REAL    (KIND=ireals   ) ::  &
  ...
  zxtcor  (ie,je   ),    & !
  zc      (ie,je,ke),    & ! Upper main diagonal of ...
  zd2_xt  (ie,je,ke),    & ! Right hand side of ...
  ze      (ie,je,ke)       ! Soluton vector of ...

REAL    (KIND=ireals   ) :: xt_start(ie,je,ke)
REAL    (KIND=ireals   ) :: fac = 0._ireals
INTEGER (KIND=iintegers) :: jt
INTEGER (KIND=iintegers) :: status
CHARACTER(LEN=*), PARAMETER :: yzroutine ='complete_tendencies_tracer'
REAL(KIND=ireals), DIMENSION(:,:,:), POINTER :: qv_new => NULL()
REAL(KIND=ireals), DIMENSION(:,:,:), POINTER :: qv_now => NULL()
! End of header
!==============================================================================
...
!------------------------------------------------------------------------------
  ...
  IF ( lvertdiff ) THEN
     ...
     IF (idx_qv <= 0) THEN
        CALL get_tracer(status, GPTRSTR, 'QV',  idx=idx_qv)
        CALL tracer_halt(yzroutine,status)
     ENDIF
     do_jt: DO jt =1, ntrac_gp
    !--------------------------------------------------------------------------
    ! Section 2a: Setup of tridiagonal matrix systems resulting from the
    !             implicit numerical formulation of diffusion of tracers
    !--------------------------------------------------------------------------

    if_vdiff: IF (ti_gp(jt)%tp%meta%cask_i(I_VDIFF) == ON)  THEN
       ! switch surface contribution of qv
       IF (jt == idx_qv) THEN
          fac = 1._ireals
       ELSE
          fac = 0._ireals
    ...
    ! Top layer
      DO j = jstart, jend
        DO i = istart, iend
          ...
          zcs        = zgct*a2t(2)
          zbg        = zzdtr - zcg
          zd2g_xt    = zzdtr * xt_array(nnew)%PTR(i,j,jt,1)       &
                       + xtte(i,j,jt,1)                           &
                       - zcs * ( xt_array(nnow)%PTR(i,j,jt,2)     &
                                  - xt_array(nnow)%PTR(i,j,jt,1) )
          zc (i,j,1) = zcg / zbg
          zd2_xt(i,j,1) = zd2g_xt/zbg
        ENDDO
      ENDDO
      ...
      DO k = 2, ke-1
        DO j = jstart, jend
          DO i = istart, iend
            ...
            zcs        = zgct*a2t(k+1)
            zbg        = zzdtr - zag - zcg
            zd2g_xt    = zzdtr * xt_array(nnew)%PTR(i,j,jt,k)       &
                         + xtte(i,j,jt,k)                           &
                         - zas * ( xt_array(nnow)%PTR(i,j,jt,k-1)   &
                                 - xt_array(nnow)%PTR(i,j,jt,k) )   &
                         - zcs * ( xt_array(nnow)%PTR(i,j,jt,k+1)   &
                                 - xt_array(nnow)%PTR(i,j,jt,k) )
            zz         = 1.0_ireals / ( zbg - zag*zc(i,j,k-1) )
            zc (i,j,k) = zcg * zz
            zd2_xt(i,j,k) = ( zd2g_xt -zag*zd2_xt(i,j,k-1) ) * zz
          ENDDO
        ENDDO
      ENDDO
      ...
      DO j = jstart, jend
        DO i = istart, iend
          ...
          zcs        = zgct*za2t_surf
          zbg        = zzdtr - zag - zcg
          zd2g_xt    = zzdtr * xt_array(nnew)%PTR(i,j,jt,ke)          &
                       + xtte(i,j,jt,ke)                              &
                       - zas * ( xt_array(nnow)%PTR(i,j,jt,ke-1)      &
                                 - xt_array(nnow)%PTR(i,j,jt,ke) )    &
! um_ak_20120716+
                       + zcs *  xt_array(nnow)%PTR(i,j,jt,ke)  &     
                       - fac * zgct * qv_s(i,j,nnow)
! um_ak_20120716-
          zz         = 1.0_ireals / ( zbg  - zag*zc(i,j,ke-1) )
          xt_array(nnew)%PTR(i,j,jt,ke) = ( zd2g_xt - zag*zd2_xt(i,j,ke-1) )* zz
        ENDDO
      ENDDO
        ...
        DO j = jstart, jend
          DO i = istart, iend
            xt_array(nnew)%PTR(i,j,jt,k) = &
                 zd2_xt(i,j,k) - zc(i,j,k)*xt_array(nnew)%PTR(i,j,jt,k+1)
          ENDDO
        ENDDO
      ENDDO
   ELSE !if_vdiff   
      DO k = 1, ke
         DO j = jstart, jend
            DO i = istart, iend
               ! this works as xt_array(nnew) was set to xt_array(nnow)
               xt_array(nnew)%PTR(i,j,jt,k) = &
                    xt_array(nnew)%PTR(i,j,jt,k) + dt * xtte(i,j,jt,k)
            ENDDO
         ENDDO
      ENDDO
   ENDIF if_vdiff
  ...
  !----------------------------------------------------------------------------
  ! Section 4: Massflux correction scheme for removing negative qv, qc values
  !            (if required)
  !----------------------------------------------------------------------------
  ! for MESSy => correct in TRACER_PDEF if required/wanted
  !----------------------------------------------------------------------------
  ...
  IF (idx_qv < 0) THEN
     ...
     CALL tracer_halt(yzroutine,status)
  ENDIF
  qv_new => xt_array(nnew)%ptr(:,:,idx_qv,:)
  qv_now => xt_array(nnow)%ptr(:,:,idx_qv,:)
...
REAL    (KIND=ireals   )  :: zxth1(ie,je,ntrac_gp,ke), zxth2(ie,je,ntrac_gp,ke)
#endif
...
#ifdef MESSY
        DO jt = 1, ntrac_gp
           zhfdx     = ( xt_array(nnow)%ptr(i+1,j,jt,k) &
                         - xt_array(nnow)%ptr(i,j,jt,k) ) * eddlon
           zxth1(i,j,jt,k) =  - zhfkh * zhfdx
        ENDDO
#endif
      ENDDO
    ENDDO
...
#ifdef MESSY
        DO jt = 1, ntrac_gp
           zhfdy     = ( xt_array(nnow)%ptr(i,j+1,jt,k) &
                         - xt_array(nnow)%ptr(i,j,jt,k) ) * eddlat
           zxth2(i,j,jt,k) =  - zhfkh * zhfdy
        ENDDO
#endif
      ENDDO
    ENDDO
...
#ifdef MESSY
            DO jt = 1, ntrac_gp
               zhfdx     = 0.25 * ( ( xt_array(nnow)%ptr(i,j,jt,k+1)     &
                                    - xt_array(nnow)%ptr(i,j,jt,k-1) )   &
                                   + ( xt_array(nnow)%ptr(i+1,j,jt,k+1)  &
                                    - xt_array(nnow)%ptr(i+1,j,jt,k-1) ) )
               zxth1(i,j,jt,k) = zxth1(i,j,jt,k) - zhfkh * zhfdx
            ENDDO
#endif
          ENDDO
        ENDDO
...
#ifdef MESSY
            DO jt = 1, ntrac_gp
               zhfdy     = 0.25 * ( ( xt_array(nnow)%ptr(i,j,jt,k+1)     &
                                    - xt_array(nnow)%ptr(i,j,jt,k-1) )   &
                                   + ( xt_array(nnow)%ptr(i,j+1,jt,k+1)  &
                                    - xt_array(nnow)%ptr(i,j+1,jt,k-1) ) )
               zxth2(i,j,jt,k) = zxth2(i,j,jt,k) - zhfkh * zhfdy
            ENDDO
#endif
          ENDDO
        ENDDO
...
#ifdef MESSY
            DO jt = 1, ntrac_gp
               zhfdx     = 0.5 * ( ( xt_array(nnow)%ptr(i,j,jt,k+1)     &
                                    - xt_array(nnow)%ptr(i,j,jt,k) )   &
                                   + ( xt_array(nnow)%ptr(i+1,j,jt,k+1)  &
                                    - xt_array(nnow)%ptr(i+1,j,jt,k) ) )
               zxth1(i,j,jt,k) = zxth1(i,j,jt,k) - zhfkh * zhfdx
            ENDDO
#endif
...
#ifdef MESSY
            DO jt = 1, ntrac_gp
               zhfdy     = 0.5 * ( ( xt_array(nnow)%ptr(i,j,jt,k+1)     &
                                    - xt_array(nnow)%ptr(i,j,jt,k) )   &
                                   + ( xt_array(nnow)%ptr(i,j+1,jt,k+1)  &
                                    - xt_array(nnow)%ptr(i,j+1,jt,k) ) )
               zxth2(i,j,jt,k) = zxth2(i,j,jt,k) - zhfkh * zhfdy
            ENDDO
#endif
          ENDDO
...
#ifdef MESSY
            DO jt = 1, ntrac_gp
               zhfdx     = 0.5 * ( ( xt_array(nnow)%ptr(i,j,jt,k)     &
                                    - xt_array(nnow)%ptr(i,j,jt,k-1) )   &
                                   + ( xt_array(nnow)%ptr(i+1,j,jt,k)  &
                                    - xt_array(nnow)%ptr(i+1,j,jt,k-1) ) )
               zxth1(i,j,jt,k) = zxth1(i,j,jt,k) - zhfkh * zhfdx
          ...
          DO i = istart, iend
            ...
            DO jt = 1, ntrac_gp
               zhfdx     = 0.5 * ( ( xt_array(nnow)%ptr(i,j,jt,k)     &
                                   ...
                                   + ( xt_array(nnow)%ptr(i,j+1,jt,k)  &
                                    - xt_array(nnow)%ptr(i,j+1,jt,k-1) ) )
               zxth2(i,j,jt,k) = zxth2(i,j,jt,k) - zhfkh * zhfdx
            ENDDO
#endif
          ENDDO
        ENDDO
...
#ifdef MESSY
            DO jt = 1, ntrac_gp
               IF (ti_gp(jt)%tp%meta%cask_i(I_HDIFF) == ON)        &
                    xtte(i,j,jt,k) = xtte(i,j,jt,k) - zarhor *     &
             ( zcrlatr * ( zxth1(i,j,jt,k) - zxth1(i-1,j,jt  ,k) ) * eddlon  &
                       + ( zxth2(i,j,jt,k) - zxth2(i  ,j-1,jt,k) ) * eddlat )
            ENDDO
#endif
      ENDDO
    ENDDO
        ...
        DO j = jstart, jend
          ...
          DO jt= 1, ntrac_gp
             IF (ti_gp(jt)%tp%meta%cask_i(I_HDIFF) == ON)        &
                  xtte(i,j,jt,k) = xtte(i,j,jt,k)  - zarhor * (                &
                  zcrlatr *           dzeta_dlam(i,j,k)                        &
                   * 0.25 * ( ( zxth1(i  ,j,jt,k+1) - zxth1(i  ,j,jt,k-1) )    &
                            + ( zxth1(i-1,j,jt,k+1) - zxth1(i-1,j,jt,k-1) ) )  &
                     +            dzeta_dphi(i,j,k)                               &
                   * 0.25 * ( ( zxth2(i,j,jt,k+1) - zxth2(i,j,jt,k-1) )        &
                            + ( zxth2(i,j-1,jt,k+1) - zxth2(i,j-1,jt,k-1) ) ) )
          ENDDO
#endif
          ENDDO
        ENDDO
...
! UB<<
            ...
            xtte(i,j,jt,k) = xtte(i,j,jt,k) - zarhor * (                        &
              zcrlatr          * dzeta_dlam(i,j,k)                             &
                    * 0.5  * ( ( zxth1(i  ,j,jt,k+1) - zxth1(i  ,j,jt,k) )     &
                             + ( zxth1(i-1,j,jt,k+1) - zxth1(i-1,j,jt,k) ) )   &
                  +              dzeta_dphi(i,j,k)                             &
                    * 0.5  * ( ( zxth2(i,j  ,jt,k+1) - zxth2(i,j  ,jt,k) )     &
                             + ( zxth2(i,j-1,jt,k+1) - zxth2(i,j-1,jt,k) ) ) )
        ...
        DO j = jstart, jend
          ...
          DO jt= 1, ntrac_gp
             IF (ti_gp(jt)%tp%meta%cask_i(I_HDIFF) == ON)        &
                  xtte(i,j,jt,k) = xtte(i,j,jt,k) - zarhor * (                &
                  zcrlatr          * dzeta_dlam(i,j,k)                        &
                    * 0.5  * ( ( zxth1(i  ,j,jt,k) - zxth1(i  ,j,jt,k-1) )    &
                             + ( zxth1(i-1,j,jt,k) - zxth1(i-1,j,jt,k-1) ) )  &
                  +              dzeta_dphi(i,j,k)                            &
                    * 0.5  * ( ( zxth2(i,j  ,jt,k) - zxth2(i,j  ,jt,k-1) )    &
                             + ( zxth2(i,j-1,jt,k) - zxth2(i,j-1,jt,k-1) ) ) )
         ENDDO
#endif
          ENDDO
        ENDDO
