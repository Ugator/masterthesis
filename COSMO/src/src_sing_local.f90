...
! time boxes
! ----------
    tmaxbox = INT( tconbox/dt   + c1-epsy ) * dt
    tminbox = INT( tconbox/dt       +epsy ) * dt
    IF (rmod( tconbox,dt,epsy ) < c2*epsy) THEN
      cqcwbox = INT( dtqc  /tconbox + c1-epsy ) * tconbox * c05
    ELSE
      cqcwbox = INT( (dtqc  +tmaxbox)/dt - epsy ) * dt * c05
    ENDIF
...
! -----------------------------------------------
    zcosign = SIGN( c1 , vcoord_in%vert_coord(1) - vcoord_in%vert_coord(2) )
    ktp = 0
    DO kk = 1 , ke+1
      IF (zcosign*vcoord_in%vert_coord(kk) >= zcosign*vcoord_in%vcflat-epsy) ktp = kk
    ENDDO
...
! ------------------------------------------------------
    ...
    DO kk = 1 , nqclev
      DO mv = 1 , nqclev
        qczcorl(kk,mv) = nzcorl(kk,mv) * 0.001_ireals
      ENDDO
    ENDDO
    IF (qcvf(4) > epsy) THEN
      qctf   (1) = qctf  (1) * 0.50_ireals
      qctf   (3) = qctf  (3) * 0.50_ireals
      qctf   (4) = qctf  (4) * 0.17_ireals
      qctfsu (1) = qctfsu(1) * 0.50_ireals
      qctfsu (3) = qctfsu(3) * 0.50_ireals
      qctfsu (4) = qctfsu(4) * 0.17_ireals
    ENDIF
    DO kk = 1 , 4
      qctfpr (kk  ) = qctf  (kk)
...
! for reference pressure   p0sl = 10E5_ireals
      IF (      (vcoord_in%sigm_coord(kk)  <= 0.85_ireals)                 &
          .AND. (vcoord_in%sigm_coord(kk+1) > 0.85_ireals)) kml850 = kk
      IF (      (vcoord_in%sigm_coord(kk)  <= 0.70_ireals)                 &
          .AND. (vcoord_in%sigm_coord(kk+1) > 0.70_ireals)) kml700 = kk
      IF (      (vcoord_in%sigm_coord(kk)  +                               &
                 vcoord_in%sigm_coord(MAX(kk-i1,i1)) <= 0.601_ireals)      &
          .AND. (vcoord_in%sigm_coord(kk+1) +                              &
                 vcoord_in%sigm_coord(kk  )           > 0.601_ireals)) kml300 = kk
    ENDDO
        ...
        DO itic = 1, 2
          ...
          IF (((lrepsu) .OR. (lrepps)) .AND. (ntotys < maxysu)) THEN
            ...
            IF (lrepps) THEN
              ...
              oysu (ntotys, 3)  = - osgbdy(nsgob,nbsvip) / 100.0_ireals
              oysu (ntotys, 4)  =   zoips (icps,itic)    / 100.0_ireals
            ENDIF
            oysu (ntotys, 2)  =   osgbdy(nsgob,nbsp ) / 100.0_ireals
            ...
            oysu (ntotys,18)  =   REAL ( mosghd(nsgob,nhitot), ireals )
            oysu (ntotys,19)  =   REAL ( mosghd(nsgob,nhjtot), ireals )
          ENDIF
        ENDDO
      ENDDO
      DO ista = 1 , nmlsta
        ...
        DO itic = 1, 2
          ...
          IF ((lrepps) .AND. (ntotys < maxysu)) THEN
            ...
            oysu (ntotys, 2)  =   omlbdy(nmlob,1,nbsp) / 100.0_ireals
            oysu (ntotys, 3)  = - omlhed(nmlob,nhtvip) / 100.0_ireals
            oysu (ntotys, 4)  =   zoips (icps,itic)    / 100.0_ireals
            DO nvys = 5 , 16
              oysu (ntotys,nvys) = rmdi
            ENDDO
            oysu (ntotys,17)  =   omlhed(nmlob,nhtime)
            oysu (ntotys,18)  =   REAL ( momlhd(nmlob,nhitot), ireals )
            oysu (ntotys,19)  =   REAL ( momlhd(nmlob,nhjtot), ireals )
          ENDIF
        ENDDO
      ENDDO
...
!   for these observation code types !)
  ...
  IF (     (kcdtyp == nwp_eu) .OR. (kcdtyp == nra_eu)                          &
    ...
    wtuka = MIN( 0.5_ireals, wtukrsa )
    wtuke = MIN( 0.2_ireals, wtukrse )
    tipmx = 0.5_ireals
  ELSE
    wtuka = wtukrsa
    wtuke = wtukrse
  ...
  IF (     (momlhd(nmlob,nhobtp) == ntemp)                                     &
    ...
    lactive =       (omlhed(nmlob,nhtime) +MAX(wtuke ,tipmx ) > aiwthr)        &
              ...
              .AND. (MAX( momlhd(nmlob,nhuexi) *gnudg(1)                       &
                        , (1- ABS( momlhd(nmlob,nhobtp)-ntemp )) *gnudg(2)     &
...
!           .AND. (ogphed(ngpob,nhtime) -wtuka-c05*c05*dtchkps < wtboxe)
  ...
  lqcflag = ((lqcflag) .OR. (      (lqcall) .AND. (liwvssc)))                  &
                             .AND. (ogphed(ngpob,nhtime) +.2_ireals > aiwthr)  &
                             .AND. (ogphed(ngpob,nhtime) -.2_ireals < wtboxe)
...
loop_over_sing_aircraft_reports:  DO nsgob = 1 , nsgobe
  ...
  IF ((lactive) .OR. (lqcflag)) THEN
!   IF ( IBITS( mosghd(nsgob,nhschr) , nvpsbp , nvscoc ) == 0 ) THEN
    ...
    ELSE
      ...
      IF ((lfirst) .OR. (rmod( ntstep*dt,c3600,epsy ) < tconbox)) THEN
        ...
        zpobpr = osghed(nsgob,nhtime) / 100.0_ireals
        IF (lwonl)                                                             &
...
! for single-level reports
    IF (     ((lfirst) .OR. (rmod( ntstep*dt,21600.0_ireals,epsy ) < tconbox)) &
        .OR. (aiwthr >= MIN( nudgend,nstop )*dtdeh -1.1_ireals)) THEN
      nsgpra = 1
      nsgprd = 1
    ELSE
...
! IF ((omyk1 >= epsy) .OR. (omyk2 >= epsy)) THEN
  IF (((zqct(1) > rmdich) .OR. (zqct(2) > rmdich)) .AND. (icps < maxpso)) THEN
    ...
    IF ((omykps(icps,1) > c0) .AND. (ABS(zoips(icps,1)) > 999._ireals))        &
      PRINT *,'DPML1 ', ystidps(icps), zoips(icps,1), omykps(icps,1)
    IF ((omykps(icps,2) > c0) .AND. (ABS(zoips(icps,2)) > 999._ireals))        &
          ...
          IF (ntpscor >= 1) THEN
            ...
            IF (MOD(ntpscor,2) == 1) THEN
              ...
              zsprcor =   cfitop * zqmass *zqmass                              &
                        * EXP(  (c1-zqmass3) *0.25_ireals / MAX(c1 , zqmass3) )
            ELSE
...
! perform the refined quality control check
        ...
        IF (ixnvrz /= MAX( mflgqc, mflgqc_bc )) THEN
          ...
          IF (ntotqc < maxqcp) THEN
            ...
            myqc (ntotqc, 1) = momlhd(nmlob,nhcode)
            oyqc (ntotqc, 1) = omlhed(nmlob,nhtime)
            oyqc (ntotqc, 2) = ABS( omlhed(nmlob,nhtvip) ) / 100.0_ireals
            oyqc (ntotqc, 3) = omlhed(nmlob,nhjlat)
            oyqc (ntotqc, 4) = omlhed(nmlob,nhilon)
            oyqc (ntotqc, 6) = omlhed(nmlob,nhtvip) / 100.0_ireals
            ! keep it without 'ABS': neg. value indicates rejected by indiv. QC
!           oyqc (ntotqc, 6) = ABS( omlhed(nmlob,nhtvip) ) / 100.0_ireals
            IF (ixnvrz /= mflgqc) THEN
              myqc (ntotqc, 2) = 15
              oyqc (ntotqc, 5) = qctps              / 100.0_ireals
              oyqc (ntotqc, 7) = zpsmod             / 100.0_ireals
              oyqc (ntotqc, 8) = zbias              / 100.0_ireals
              oyqc (ntotqc, 9) = om2ps
            ELSE
              myqc (ntotqc, 2) = 20
              oyqc (ntotqc, 5) = qctps_bc           / 100.0_ireals
              oyqc (ntotqc, 7) = zpsmod_bc          / 100.0_ireals
              oyqc (ntotqc, 8) = zbias_bc           / 100.0_ireals
...
!         qctps  =  qcfctnd *qccsu(2)  +  qcftend *ABS( osgbdy(nsgob,nbspst) )
        ...
        IF (     (ibit1( mosgbd(nsgob,nbsqcf),nvrz  ) /= mflgqc   )            &
          ...
          IF (      (   MAX( ibit1( mosgbd(nsgob,nbsqcf),nvrz  )               &
            ...
            myqc (ntotqc, 1) = mosghd(nsgob,nhcode)
            oyqc (ntotqc, 1) = osghed(nsgob,nhtime)
            oyqc (ntotqc, 2) = osgbdy(nsgob,nbsp)   / 100.0_ireals
            oyqc (ntotqc, 3) = osghed(nsgob,nhjlat)
            oyqc (ntotqc, 4) = osghed(nsgob,nhilon)
            oyqc (ntotqc, 6) = osgbdy(nsgob,nbsp)   / 100.0_ireals
            ...
            IF (ibit1( mosgbd(nsgob,nbsqcf),nvrz ) /= mflgqc) THEN
              myqc (ntotqc, 2) = 15
              oyqc (ntotqc, 5) = qctps              / 100.0_ireals
              oyqc (ntotqc, 7) = zpsvim             / 100.0_ireals
              oyqc (ntotqc, 8) = zbias              / 100.0_ireals
              oyqc (ntotqc, 9) = om2ps
            ELSE
              myqc (ntotqc, 2) = 20
              oyqc (ntotqc, 5) = qctps_bc           / 100.0_ireals
              oyqc (ntotqc, 7) = zpsvim_bc          / 100.0_ireals
              oyqc (ntotqc, 8) = zbias_bc           / 100.0_ireals
              oyqc (ntotqc, 9) = om2ps_bc
            ENDIF
            IF (.NOT. ((lqcall) .AND. (ABS( c3600*timdif+epsy ) <= cqcwbox)))  &
...
! ----------------
  ...
  REAL    (KIND=ireals   )  , PARAMETER  ::  &
    qqdts  =  5.0_ireals    ! Gaussian radius of T-2m increments used for
...
! check tcm and gz0
  IF ((lphys) .AND. (ltur)) THEN
    zz0o  =  gz0(io,jo) / g
    zcm   =  MAX( tcm(io,jo) , 5.0E-4_ireals )
  ELSE
    zz0o  =  MAX( gz0(io,jo) , 1.0E-3_ireals ) / g
    zcm   =  1.0E-4_ireals
  ENDIF
  z10 = 10.0_ireals
...
! instable case
    ...
    zobop  =  c1 - zsqcm/akt * LOG( c1 + (EXP( akt/zsqcm ) -c1)                &
                          *MAX(0.0_ireals,zzke-z10) *zz0o /(zzke *(z10+zz0o)))
  ENDIF
  xke    =  xsl / zobop
  yke    =  ysl / zobop
...
! check tcm, tch and gz0
    IF ((lphys) .AND. (ltur)) THEN
      zz0o  =  gz0(io,jo) / g
      zch   =  MAX( tch(io,jo) , 4.0E-5_ireals )
      zcm   =  MAX( tcm(io,jo) , 5.0E-4_ireals )
    ELSE
      zz0o  =  MAX( gz0(io,jo) , 1.0E-3_ireals ) / g
      zch   =  1.0E-4_ireals
...
! other variables
    ...
    IF (zhs <= zh) THEN
! stable case
      zfext = 4.0_ireals / (c1 + 4.5_ireals /(zzke -c05))
    ELSE
! instable case
...
!-------------------------------------------------------------------------------
  ...
  loop_over_variables:  DO ivrs = 1, 3
    ...
    IF ((kb > 0) .AND. (gnudgar(ivar) > epsy)) THEN
      DO itim = itima , itime
        nsgob  = isgadm(ista,itim)
        IF (      (      BTEST( mosgbd(nsgob,nbserr), nvrx ))                  &
            .AND. (.NOT. BTEST( mosgbd(nsgob,nbsqcf), nvrx ))) THEN
...
!                   .OR. (osgbdy(MAX(nsgob,i1),nbsxer) <= rmdich)              &
!                   .OR. (mosghd(MAX(nsgob,i1),nhpass) /= 0)) THEN
      IF (     (nsgob == 0) .OR. (kb == 0) .OR. (gnudgar(ivar) <= epsy)        &
          .OR. (mosghd(MAX(nsgob,i1),nhpass) /= 0)                             &
          .OR. (.NOT. BTEST( mosgbd(MAX(nsgob,i1),nbserr), nvrx ))) THEN
        wtv = c0
      ELSE
    ...
    IF ((zwtua(icua,1,ivrs) >= epsy) .OR. (zwtua(icua,2,ivrs) >= epsy)) THEN
      ...
      DO nbotop = 1 , 2
        ...
        IF ((kthr < 0) .OR. (nbotop == 2)) THEN
          zlopco = LOG( MAX( a_p(io,jo,MIN(ABS(kthr),ke)) , 1000.0_ireals ) )
          ilva = 1
