...
ELSEIF (yaction == 'compute') THEN
  ...
  IF (lsynsat .AND. (nsat_steps(nsat_next) == ntstep) ) THEN
    ...
    CALL trcr_get(ierror, 'QV', ptr_tlev = nnow, ptr=qv)
    IF (ierror /= 0) THEN
      yerror = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerror, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QC', ptr_tlev=nnow, ptr=qc)
    IF (ierror /= 0) THEN
      yerror = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerror, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QG', ptr_tlev=nnow, ptr=qg)
    IF (ierror /= 0 .AND. ierror /= T_ERR_NOTFOUND) THEN
      yerror = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerror, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QI', ptr_tlev=nnow, ptr=qi)
    IF (ierror /= 0 .AND. ierror /= T_ERR_NOTFOUND) THEN
      yerror = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerror, yzroutine)
    ENDIF
    CALL trcr_get(ierror, 'QS', ptr_tlev=nnow, ptr=qs)
    IF (ierror /= 0 .AND. ierror /= T_ERR_NOTFOUND) THEN 
      yerror = trcr_errorstr(ierror)
      CALL model_abort(my_cart_id, ierror, yerror, yzroutine)
    ENDIF  
#else
    IF (.NOT. ASSOCIATED(qs) ) THEN
     CALL get_tracer(ierror, GPTRSTR, 'QV',  pxtm1=qv)
     CALL tracer_halt(yzroutine,ierror)
     CALL get_tracer(ierror, GPTRSTR, 'QC',  pxtm1=qc)
     CALL tracer_halt(yzroutine,ierror)
     CALL get_tracer(ierror, GPTRSTR, 'QG',  pxtm1=qg)
     IF (ierror /= TR_NEXIST) CALL tracer_halt(yzroutine,ierror)
     CALL get_tracer(ierror, GPTRSTR, 'QI',  pxtm1=qi)
     IF (ierror /= TR_NEXIST) CALL tracer_halt(yzroutine,ierror)
     CALL get_tracer(ierror, GPTRSTR, 'QS',  pxtm1=qs)
     IF (ierror /= TR_NEXIST) CALL tracer_halt(yzroutine,ierror)
    ENDIF
#endif
