...
USE data_fields,         ONLY:                                               &
       ...
       aer_su_bd, aer_du_bd, aer_bc_bd, aer_or_bd, aer_ss_bd
...
#ifdef MESSY
! COSMO
USE data_tracer,           ONLY: idt_qv, idt_qc, idt_qi
! MESSy/BMIL
...
USE messy_main_tracer_mem_bi, ONLY: ti_gp, xt, xtf, xtm1, xtte &
                                  , ntrac_gp, xt_bd, xt_array
USE messy_main_data_bi,       ONLY: L_IS_CLIENT
USE messy_main_mpi_bi,        ONLY: exchg_trac_boundaries,exchg_trac_boundaries2
! MESSy/SMCL
USE messy_main_timer,      ONLY: lstop, lbreak
USE messy_main_tracer,     ONLY: I_LBC, T_LBC_ZEROGRAD, T_LBC_ZERO &
                               , T_INI_FILE, T_LBC_FILE, I_INITIAL
#endif
...
#ifndef MESSY
  ! tracer fields
  ! in case of MESSY, these have to be allocated before call to organize_allocation, 
  ! so that they can be used in the MESSY memory organization
...
#ifdef MESSY
!    CALL messy_vdiff     ! Here ART EMIS is called
...
!------------------------------------------------------------------------------
  ...
  IF (izerror /= 0) THEN
    ...
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QC', ptr_tlev = nx0, ptr = qc_nx0)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QI', ptr_tlev = nnew, ptr = qi_new)
  ...
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    ...
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QI', ptr_tlev = nx0, ptr = qi_nx0)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
...
#ifdef TWOMOM_SB
  CALL trcr_get(izerror, 'QNICE', ptr_tlev = nx0, ptr = qni_now)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
...
!CDIR COLLAPSE
  ...
  IF (ntrac_gp > 0 ) THEN
     ! set new time levels:
     xt_array(nnew)%PTR => xt
     ! the cyclic changes in COSMO is applied hard coded for the tracer here
     IF (l2tls) THEN
        xtm1 (:,:,:,:) = xt (:,:,:,:)
        ! xt is set below including the boundary data
        xt_array(nnow)%PTR => xtm1
     ELSE
        xtm1(:,:,:,:) = xtf (:,:,:,:) ! nnow => nold
        xtf (:,:,:,:) = xt  (:,:,:,:) ! nnew => nnow
        ! xt is set below including the boundary data
        xt_array(nnow)%PTR => xtf
        xt_array(nold)%PTR => xtm1
     ENDIF

     ! set tracer boundary fields
     WHERE (xt_bd (:,:,:,:,nbd1) >= 0._ireals)
        xt_array(nnew)%PTR(:,:,:,:) = &
             z1 *xt_bd (:,:,:,:,nbd1) + z2*xt_bd (:,:,:,:,nbd2)
     END WHERE

     ! set tracer tendencies to zero
     xtte(:,:,:,:) = 0.0_ireals
  ENDIF 

  qv_new => xt_array(nnew)%ptr(:,:,idt_qv,:)
  qv_now => xt_array(nnow)%ptr(:,:,idt_qv,:)
  qc_new => xt_array(nnew)%ptr(:,:,idt_qc,:)
  qc_now => xt_array(nnow)%ptr(:,:,idt_qc,:)
  qc_nx0 => xt_array(nx0)%ptr(:,:,idt_qc,:)
  IF (idt_qi > 0) THEN
     qi_new => xt_array(nnew)%ptr(:,:,idt_qi,:)
     qi_now => xt_array(nnow)%ptr(:,:,idt_qi,:)
     qi_nx0 => xt_array(nx0)%ptr(:,:,idt_qi,:)
  ENDIF
#endif
...
#ifndef MESSY
  CALL trcr_meta_get(izerror, 'QI', T_LBC_ID, izlbcqi)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  ...
  izlbcqi = 0
  IF (idt_qi > 0) THEN
     izlbcqi = ti_gp(idt_qi)%tp%meta%cask_i(I_LBC)
     izanaqi = ti_gp(idt_qi)%tp%meta%cask_i(I_INITIAL)
  ENDIF
#endif
...
!CDIR COLLAPSE
  ...
  CALL trcr_meta_get(izerror, 'QI', T_INI_ID, izanaqi)
  IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
...
CHARACTER(LEN=25) :: yzroutine = 'exchange_leapfrog'
  ...
  DO jt=1,ntrac_gp
     ...
     CALL exchg_boundaries                                                   &
          ...
          xt_array(nnow)%PTR(:,:,jt,:), xt_array(nnew)%PTR(:,:,jt,:))
  ENDDO
#endif
...
CHARACTER (LEN=25) :: yzroutine='exchange_runge_kutta'
  ...
  DO jt = 1, ntrac_gp
    ...
    CALL exchg_boundaries                                                 &
      ...
      my_cart_neigh, lperi_x, lperi_y, l2dim,                             &
      20000+nexch_tag, .FALSE., ncomm_type, izerror, yzerrmsg,            &
      xt_array(nnew)%PTR(:,:,jt,:), xt_array(nnow)%PTR(:,:,jt,:) )
  ENDDO
#endif
...
CHARACTER(LEN=25) :: yzroutine = 'set_trcr_special_bc'
  ...
  DO iztrcr = 1, ntrac_gp
     IF (ti_gp(iztrcr)%tp%meta%cask_i(I_LBC) == T_LBC_ZEROGRAD) THEN
        !.OR. ti_gp(iztrcr)%tp%meta%cask_i(BD_FORCED?) THEN
    ...
    ELSEIF ( izlbc(iztrcr) == T_LBC_ZERO .AND.                                &
      ...
      ENDIF
#else
    ELSE IF(ti_gp(iztrcr)%tp%meta%cask_i(I_LBC) == T_LBC_ZERO) THEN
