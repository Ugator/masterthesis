...
!   nvhqfl = 10  ,& ! report flags (rds) on lat/long/date/time/alt
                    !   (only if reading from AOF)           (see ODR: 'nhqofl')
  ...
  INTEGER (KIND=iintegers) , PARAMETER  :: &
    ...
    nvbcwg = 14  ,& ! combined cloud and weather group (set of classes)
                    !                                        (see ODR: 'nbscwg')
    nvbcfw = 15  ,& ! from AOF   : cloud, weather etc flags  (see ODR: 'nbscfw')
                    ! from NetCDF: weather and ground group  (see ODR: 'nbswwe')
    nvbpr  = 16  ,& ! precipitation amount                            [1/10 mm]
    nvbtn  = 17  ,& ! minimum temperature (at 2m during past 12 hrs)  [1/10 K]
    nvbtx  = 18  ,& ! maximum temperature (at 2m during past 12 hrs)  [1/10 K]
    nvbtg  = 19  ,& ! ground temperature (min. T-5cm in past 12 hrs)  [1/10 K]
    nvbfgu = 20  ,& ! max. wind speed of gusts                        [m/s]
    nvbfme = 21  ,& ! max. wind speed of 10 minute mean wind          [m/s]
  ...
  REAL    (KIND = ireals)    , PARAMETER  :: &
    g_bufr  =     9.807          ,& ! gravity acceleration as used in BUFR / VOF
    c1000r  =     0.001_ireals   ,& !
    c100r   =     0.01_ireals    ,& !
    c10r    =     0.1_ireals     ,& !
    c10     =    10.0_ireals     ,& !
    ...
    c1000   =  1000.0_ireals     ,& !
    c3600   =  3600.0_ireals     ,& !
    csmall  = 0.00001_ireals        ! less than half a second [hrs]
...
! -------------
  ...
  CHARACTER (LEN=14)         ::  &
    yakdat1  ! actual date in the form   yyyymmddhhmmss
...
!   iodrerr            ,& ! ODR obs. error flag code
    ...
    nveria             ,& ! start  of verification period in timesteps
    ...
    kruntyp            ,& ! = 2: NUDGE ; = 1: NUDGCAST or FORECAST
...
!     odrbufp(ipos+17) = NINT( zdd * c10  )
    ...
    ENDIF
    IF ((osgbdy(iob,nbspr) > epsy) .AND. (osgbdy(iob,nbspr) < 0.09_ireals)) THEN
      odrbufp(ipos+nvbpr ) = - 1
    ELSEIF (osgbdy(iob,nbspr ) > rmdich) THEN
      odrbufp(ipos+nvbpr ) = NINT( osgbdy(iob,nbspr ) * c10 )
    ENDIF
    ...
    IF (osgbdy(iob,nbstg ) > rmdich)                                           &
      odrbufp(ipos+nvbtg ) = NINT( osgbdy(iob,nbstg ) * c10 )
    IF (osgbdy(iob,nbsfgu) > rmdich)                                           &
      odrbufp(ipos+nvbfgu) = NINT( osgbdy(iob,nbsfgu) )
    IF (osgbdy(iob,nbsfme) > rmdich)                                           &
      odrbufp(ipos+nvbfme) = NINT( osgbdy(iob,nbsfme) )
    IF (osgbdy(iob,nbsrad) > rmdich)                                           &
      odrbufp(ipos+nvbrad) = NINT( osgbdy(iob,nbsrad) * 0.0001_ireals )
...
!!  IF (osgbdy(iob,nbsper) > rmdich) iodrerr = iodrerr + 16
!!  IF (osgbdy(iob,nbscer) > rmdich) iodrerr = iodrerr + 32
!   odrbufp(ipos+nvberf) = iodrerr
    ...
    odrbufp(ipos+nvbcfw) = mosgbd(iob,nbscfw)
...
!   odrbufp(ipos+ 1) = 15 + 13* momlhd(iob,nhnlev)
    ...
    IF (lvofoi) THEN
      IF (ABS( dmlhed(nso_ps,iob) ) > (ABS(nodev)-c05)) odrbufp(ipos+nvhqcp) = 4
    ENDIF
...
!   ----------------------------------------------
    IF (lvofoi) THEN
      ...
      IF (ABS( dmlhed(nso_ps,iob) ) < (ABS( nodev )-c05))                      &
...
!     =================
      ...
      tveria  = tveria - epsy + 0.00005_ireals
      tverie  = tverie + epsy - 0.00005_ireals
...
!         ntrun  = INT( hverend +1000.99999_ireals ) - 1000
!         trun   = hverend - ntrun 
...
!         =================
        ...
        WRITE( nuverif,'('' 1'',3X,I2.2,2X,A14,2X,F9.4,4X,F7.2,4X,I3,5X,A11)') &
               kruntyp, ygrbt, trun, zdphir, ke_tot, ydescr
      ENDIF
      WRITE( nuverif,'('' '')' )
    ENDIF
    mboxnow = -9999
  ...
  ! --------------------------------------------
        ...
        ELSE
          ...
          IF (      (odrbufa(indst+nvbtn ) <   0)                              &
              ...
              .AND. (odrbufa(indst+nvbtg ) <   0)                              &
              .AND. (odrbufa(indst+nvbfgu) <   0)                              &
              .AND. (odrbufa(indst+nvbfme) <   0)                              &
              .AND. (odrbufa(indst+nvbrad) <   0))  nlev  =  - 1
          IF (      (odrbufa(indst+nvbcl ) <   0) .AND. (nlev == -1)           &
              .AND. (odrbufa(indst+nvbvis) <   0)                              &
              .AND. (odrbufa(indst+nvbcwg) <   0)                              &
!             .AND. (odrbufa(indst+nvbcfw) <   1)                              &
              .AND. (odrbufa(indst+nvbpr ) <  -1))  nlev  =  - 2
        ENDIF
        DO icl = 1 , ilstidv
...
! body of single-level report
        ...
        ENDIF
!       IF ((nlev == -1) .AND. (odrbufa(ipos+nvbpr) /= noval))                 &
...
! -----------------------------------------
  ...
  IF ((lvofoi) .AND. (MOD(mveripr,2) == 0)) THEN
...
ELEMENTAL REAL FUNCTION rmod  ( ztti, ziv )
  !----------------------------------------
  REAL    (KIND=ireals)     , INTENT (IN)  ::  ztti, ziv
  !  uses parameter:                           epsy
  !---------------------------------------------------------------------------
