...
#else
USE data_tracer,              ONLY : idt_qv, idt_qc, idt_qs &
...
USE messy_main_tracer_mem_bi, ONLY: xtte, xt_array
#endif
...
! ----------------
  REAL    (KIND=ireals   ), PARAMETER ::  &
    ...
    c_w   = 4200.0_ireals, & ! specific heat of water Flocode
...
! select timelevel and timestep for calculations
  ...
  CALL trcr_get(izerror, 'QV', ptr_tlev = nnew, ptr = qv, ptr_tens = qv_tens)
  IF (izerror /= 0) THEN
  ...
  ENDIF
#else
  qv      => xt_array(nnew)%ptr(:,:,idt_qv,:)
  qv_nx   => xt_array(nx)%ptr(:,:,idt_qv,:)
  qv_tens => xtte(:,:,idt_qv,:)
  
  qc    => xt_array(nnew)%ptr(:,:,idt_qc,:)
  qc_nx => xt_array(nx)%ptr(:,:,idt_qc,:) 
  
  qi_nx => xt_array(nx)%ptr(:,:,idt_qi,:)
    
  IF (ldini) THEN
     qr_now => xt_array(nnow)%ptr(:,:,idt_qr,:)
     IF (.NOT. l2tls) qr_old => xt_array(nold)%ptr(:,:,idt_qr,:)
     qs_now => xt_array(nnow)%ptr(:,:,idt_qs,:)
     IF (.NOT. l2tls) qs_old => xt_array(nold)%ptr(:,:,idt_qs,:)
  ENDIF
#endif 
...
#ifndef MESSY
! retrieve the required microphysics tracers (at corresponding timelevel)
CALL trcr_get(izerror, 'QV', ptr_tlev = nu, ptr = qv)
IF (izerror /= 0) THEN
  yzerrmsg = trcr_errorstr(izerror)
  CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
ENDIF
CALL trcr_get(izerror, 'QC', ptr_tlev = nu, ptr = qc)
IF (izerror /= 0) THEN
  yzerrmsg = trcr_errorstr(izerror)
  CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
ENDIF
CALL trcr_get(izerror, 'QR', ptr_tlev = nu, ptr = qr)
IF (izerror /= 0) THEN
  yzerrmsg = trcr_errorstr(izerror)
  CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
ENDIF
#else
  qv => xt_array(nu)%ptr(:,:,idt_qv,:)
  qc => xt_array(nu)%ptr(:,:,idt_qc,:)
  qr => xt_array(nu)%ptr(:,:,idt_qr,:)
#endif
...
IF (izerror /= 0) THEN
  ...
  CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
ENDIF
CALL trcr_get(izerror, 'QS', ptr_tlev = nu, ptr = qs)
IF (izerror /= 0) THEN
  yzerrmsg = trcr_errorstr(izerror)
  CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
ENDIF
...
! -------------
  ...
  REAL    (KIND=ireals   ) ::  &
    ...
    p1,                & !
    c_s,               & !
    qcg,tg,qvg,qrg, qsg,qig,ppg, alf,bet,m2s,m3s,hlp
...
#ifndef MESSY
!Flos Entropie-tracer
  CALL trcr_get(izerror, 'T_ENTR', ptr_tlev = nx, ptr = t_entr)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
!ende flocode
  ...
  ENDIF
#else
  t_entr => xt_array(nnew)%ptr(:,:,idt_qv,:)
  qv     => xt_array(nnew)%ptr(:,:,idt_qv,:)
  qc     => xt_array(nnew)%ptr(:,:,idt_qc,:)
  qi     => xt_array(nnew)%ptr(:,:,idt_qi,:)
  qr     => xt_array(nnew)%ptr(:,:,idt_qr,:)
  qs     => xt_array(nnew)%ptr(:,:,idt_qs,:)
#endif
...
!CDIR END
  ...
  ! t        temperature
  ! p0       ref pressure
  ! pp       deviation from ref pressure
  ! r_d      gas constant dry air R0
  ! r_v      gas constant water vapor R1
  ! lh_v     latent heat of vapourization l21
  ! lh_f     latent heat of fusion l32
  ! fpvsw(T) sat vap pressure over water p21
  ! fpvsi(T) sat vap pressure over ice p31
  ! cp_d     specific heat of dry air
  ! c_w     specific heat of water
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        ...
        r3=qig+qsg
        p1=r1*r_v/r_d*ppg
        c_s=cp_d+c_w*(r1+r2+r3)
        !flocode
        t_entr(i,j,k)=tg*(((ppg-p1)/100000.)**(-r_d/c_s))*EXP((lh_v+r_v*tg*LOG(fpvsw(tg)/p1))*r1/(c_s*tg)-(lh_f+r_v*tg*LOG(fpvsi(tg)/fpvsw(tg)))*r3/(c_s*tg))
        !flocode ende
...
! -------------
  ...
  REAL    (KIND=ireals   ) ::  &
    zdh, zlnqrk, &
    zln1o2, zlnqsk, zlnqgk, zlnlogmi, zar, zn0r, &!Flo r1, r2, r3, p1, c_s
    r1,                & !
    r2,                & !
    r3,                & !
    p1,                & !
